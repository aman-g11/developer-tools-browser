...
function getMarkdownRenderer(conversation) {
  ...
  if (context instanceof AiAssistanceModel.PerformanceAgent.PerformanceTraceContext) {
    ...
    return new PerformanceAgentMarkdownRenderer();
  } else if (Greendev.Prototypes.instance().isEnabled("emulationCapabilities") && conversation?.type === AiAssistanceModel.AiHistoryStorage.ConversationType.STYLING && SDK.TargetManager.TargetManager.instance().primaryPageTarget()?.model(SDK.DOMModel.DOMModel)) {
...
export var ViewState = /* @__PURE__ */ ((ViewState2) => {
...
function defaultView(input, output, target) {
  ...
  if (Root.Runtime.hostConfig.devToolsAiAssistanceV2?.enabled || Greendev.Prototypes.instance().isEnabled("breakpointDebuggerAgent")) {
    const shouldShowWalkthrough = input.state === "chat-view" /* CHAT_VIEW */ && input.walkthrough.isExpanded;
...
export class AiAssistancePanel extends UI.Panel.Panel {
  ...
  async #getPanelViewInput() {
    ...
    if (this.#aidaAvailability !== Host.AidaClient.AidaAccessPreconditions.AVAILABLE || !this.#aiAssistanceEnabledSetting?.getIfNotDisabled() || blockedByAge) {
      return {
  ...
  #getDefaultConversationType() {
    ...
    } else if (isSourcesPanelVisible && this.#conversation?.type === AiAssistanceModel.AiHistoryStorage.ConversationType.BREAKPOINT) {
  ...
  wasShown() {
    ...
    this.#selectedBreakpoint = createBreakpointContext(UI.Context.Context.instance().flavor(Workspace.UISourceCode.UILocation));
    ...
    UI.Context.Context.instance().addFlavorChangeListener(
      ...
      this.#handleBreakpointFlavorChange
    );
    UI.Context.Context.instance().addFlavorChangeListener(
      LighthousePanel.LighthousePanel.ActiveLighthouseReport,
  ...
  #getChatInputPlaceholder() {
    ...
    switch (this.#conversation.type) {
      case AiAssistanceModel.AiHistoryStorage.ConversationType.STYLING:
        return this.#conversation.selectedContext ? lockedString(UIStringsNotTranslate.inputPlaceholderForStyling) : lockedString(UIStringsNotTranslate.inputPlaceholderForStylingNoContext);
      case AiAssistanceModel.AiHistoryStorage.ConversationType.FILE:
        return this.#conversation.selectedContext ? lockedString(UIStringsNotTranslate.inputPlaceholderForFile) : lockedString(UIStringsNotTranslate.inputPlaceholderForFileNoContext);
      case AiAssistanceModel.AiHistoryStorage.ConversationType.NETWORK:
        return this.#conversation.selectedContext ? lockedString(UIStringsNotTranslate.inputPlaceholderForNetwork) : lockedString(UIStringsNotTranslate.inputPlaceholderForNetworkNoContext);
      case AiAssistanceModel.AiHistoryStorage.ConversationType.PERFORMANCE: {
        const perfPanel = UI.Context.Context.instance().flavor(TimelinePanel.TimelinePanel.TimelinePanel);
        if (perfPanel?.hasActiveTrace()) {
          return this.#conversation.selectedContext ? lockedString(UIStringsNotTranslate.inputPlaceholderForPerformanceTrace) : lockedString(UIStringsNotTranslate.inputPlaceholderForPerformanceTraceNoContext);
        }
      ...
      case AiAssistanceModel.AiHistoryStorage.ConversationType.ACCESSIBILITY:
        return this.#conversation.selectedContext ? lockedString(UIStringsNotTranslate.inputPlaceholderForAccessibility) : lockedString(UIStringsNotTranslate.inputPlaceholderForAccessibilityNoContext);
      case AiAssistanceModel.AiHistoryStorage.ConversationType.NONE:
  ...
  #populateHistoryMenu(contextMenu) {
    const historicalConversations = AiAssistanceModel.AiHistoryStorage.AiHistoryStorage.instance().getHistory().map(
      (serializedConversation) => AiAssistanceModel.AiConversation.AiConversation.fromSerializedConversation(serializedConversation)
    );
      ...
      for await (const data of items) {
        ...
        switch (data.type) {
          case AiAssistanceModel.AiAgent.ResponseType.USER_QUERY: {
            ...
            if (isSidebarWalkthroughOpen || Greendev.Prototypes.instance().isEnabled("breakpointDebuggerAgent") && this.#conversation?.type === AiAssistanceModel.AiHistoryStorage.ConversationType.BREAKPOINT) {
    ...
    switch (actionId) {
      ...
      case "drjones.sources-panel-context": {
        void (async () => {
          ...
          void widget2.handleAction(actionId, opts);
        })();
        return true;
      }
    }
