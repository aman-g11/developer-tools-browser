...
function renderConsentViewContents(hostConfig) {
  ...
  if (hostConfig.devToolsAiAssistancePerformanceAgent?.enabled) {
    consentViewContents = uiI18n.getFormatLocalizedString(
      str_,
      UIStrings.turnOnForStylesRequestsPerformanceAndFiles,
      { PH1: settingsLink }
    );
  ...
  render(
    html`
      ...
      <div class="disabled-view">
        ...
        <div>
          ${input.aidaAvailability === Host.AidaClient.AidaAccessPreconditions.AVAILABLE ? renderConsentViewContents(input.hostConfig) : renderAidaUnavailableContents(input.aidaAvailability)}
        </div>
      </div>
    `,
    target
