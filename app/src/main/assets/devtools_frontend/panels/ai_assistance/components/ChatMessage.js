...
export var ChatMessageEntity = /* @__PURE__ */ ((ChatMessageEntity2) => {
...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  if (message.entity === "user" /* USER */) {
    const imageInput = message.imageInput && "inlineData" in message.imageInput ? renderImageChatMessage(message.imageInput.inlineData) : Lit.nothing;
    Lit.render(html`
...
function renderTitle(step) {
  const paused = step.requestApproval ? html`<span class="paused">${lockedString(UIStringsNotTranslate.paused)}: </span>` : Lit.nothing;
...
function renderStepCode(step) {
  ...
  const codeHeadingText = step.output && !step.canceled ? lockedString(UIStringsNotTranslate.codeExecuted) : lockedString(UIStringsNotTranslate.codeToExecute);
...
function renderStepDetails({
  ...
  const contextDetails = step.contextDetails ? html`${Lit.Directives.repeat(
    step.contextDetails,
    (contextDetail) => {
    ...
    <div class="network-request-preview">
      <div class="network-request-header">
        <div class="network-request-icon">
          ${resourceType.isImage() ? html`<img src=${networkRequest.imageUrl ?? networkRequest.url} alt=${filename} />` : html`<devtools-icon name=${iconName} style=${Lit.Directives.styleMap({
    color: color ?? ""
  })}></devtools-icon>`}
        </div>
