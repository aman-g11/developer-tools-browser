...
export class AXNodePropertyTreeElement extends UI.TreeOutline.TreeElement {
  ...
  static createSimpleValueElement(type, value) {
    let valueElement;
    if (!type || type === Protocol.Accessibility.AXValueType.ValueUndefined || type === Protocol.Accessibility.AXValueType.ComputedString) {
  ...
  appendValueElement(value) {
    if (value.type === Protocol.Accessibility.AXValueType.Idref || value.type === Protocol.Accessibility.AXValueType.Node || value.type === Protocol.Accessibility.AXValueType.IdrefList || value.type === Protocol.Accessibility.AXValueType.NodeList) {
...
export const StringProperties = /* @__PURE__ */ new Set([
...
export class AXValueSourceTreeElement extends AXNodePropertyTreeElement {
  ...
  appendRelatedNodeListValueElement(value) {
    ...
    if (value.type === Protocol.Accessibility.AXValueType.IdrefList || value.type === Protocol.Accessibility.AXValueType.Idref) {
  ...
  update() {
    ...
    if (this.source.attributeValue) {
      ...
      const valueElement = AXNodePropertyTreeElement.createSimpleValueElement(
        Protocol.Accessibility.AXValueType.ValueUndefined,
        i18nString(UIStrings.notSpecified)
      );
    ...
    switch (reason) {
      ...
      case "ariaHiddenSubtree": {
        ...
        reasonElement = uiI18n.getFormatLocalizedString(
          str_,
          UIStrings.placeholderIsPlaceholderOnAncestor,
      ...
      case "presentationalRole": {
        ...
        reasonElement = uiI18n.getFormatLocalizedString(str_, UIStrings.elementHasPlaceholder, { PH1: rolePresentationSpan });
        break;
      }
