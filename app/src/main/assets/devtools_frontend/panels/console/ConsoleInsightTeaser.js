...
export class ConsoleInsightTeaser extends UI.Widget.Widget {
  ...
  maybeGenerateTeaser() {
    const startGeneratingTeaser = () => {
      if (!this.#isInactive && Common.Settings.Settings.instance().moduleSetting("console-insight-teasers-enabled").get()) {
          ...
          if (this.#eventListeners.length === 0) {
            this.#eventListeners = [
              this.#builtInAi.addEventListener(
                AiAssistanceModel.BuiltInAi.Events.DOWNLOAD_PROGRESS_CHANGED,
    ...
    this.#view(
      {
        ...
        isInactive: this.#isInactive || !Common.Settings.Settings.instance().moduleSetting("console-insight-teasers-enabled").get(),
