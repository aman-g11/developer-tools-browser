...
const elementToMessage = /* @__PURE__ */ new WeakMap();
...
export class ConsoleViewMessage {
      ...
      switch (this.message.type) {
        ...
        case Protocol.Runtime.ConsoleAPICalledEventType.Clear:
          ...
          UI.Tooltip.Tooltip.install(
            messageElement,
            i18nString(UIStrings.clearAllMessagesWithS, {
  ...
  formatAsNetworkRequest() {
    ...
    if (this.message.level === Protocol.Log.LogEntryLevel.Error) {
      ...
      const fragment = this.linkifyWithCustomLinkifier(messageText, (text, url, lineNumber, columnNumber) => {
        const linkElement = url === request.url() ? Components.Linkifier.Linkifier.linkifyRevealable(
          request,
          url,
          request.url(),
          void 0,
  ...
  buildMessageWithStackTrace(runtimeModel) {
    ...
    if (this.message.type === Protocol.Runtime.ConsoleAPICalledEventType.Trace && Common.Settings.Settings.instance().moduleSetting("console-trace-expand").get()) {
  ...
  buildMessageHelper(target, stackTrace, icon) {
    ...
    UI.ARIAUtils.setLabel(
      contentElement,
      `${messageElement.textContent} ${i18nString(UIStrings.stackMessageCollapsed)}`
    );
  ...
  format(rawParameters) {
    ...
    const shouldFormatMessage = SDK.RemoteObject.RemoteObject.type(parameters[0]) === "string" && (this.message.type !== SDK.ConsoleModel.FrontendMessageType.Result || this.message.level === Protocol.Log.LogEntryLevel.Error);
  ...
  formatParameterAsFunction(originalFunction, includePreview) {
    const result = document.createElement("span");
    void SDK.RemoteObject.RemoteFunction.objectAsFunction(originalFunction).targetFunction().then(formatTargetFunction.bind(this));
    return result;
  ...
  formatAsAccessorProperty(object, propertyPath, isArrayEntry) {
    ...
    function onInvokeGetterClick(result) {
      ...
      if (wasThrown) {
        ...
        UI.Tooltip.Tooltip.install(element, object2.description);
        ...
        if (type !== "function" && object2.description) {
          if (type === "string" || subtype === "regexp" || subtype === "trustedtype") {
            description = Platform.StringUtilities.trimMiddle(object2.description, maxLength);
          } else {
            description = Platform.StringUtilities.trimEndWithMaxLength(object2.description, maxLength);
          }
        }
  ...
  onKeyDown(event) {
    if (UI.UIUtils.isEditing() || !this.elementInternal || !this.elementInternal.hasFocus() || this.elementInternal.hasSelection()) {
      return;
    }
  ...
  shouldShowInsights() {
    if (this.message.source === Common.Console.FrontendMessageSource.ConsoleAPI && this.message.stackTrace?.callFrames[0]?.url === "") {
      return false;
    }
    if (this.message.messageText === "" || this.message.source === Common.Console.FrontendMessageSource.SELF_XSS) {
      return false;
    }
    return this.message.level === Protocol.Log.LogEntryLevel.Error || this.message.level === Protocol.Log.LogEntryLevel.Warning;
  }
  shouldShowTeaser() {
    if (!this.shouldShowInsights()) {
      return false;
    }
    if (!Common.Settings.Settings.instance().moduleSetting("console-insight-teasers-enabled").getIfNotDisabled() || !AiAssistanceModel.BuiltInAi.BuiltInAi.instance().isEventuallyAvailable()) {
      return false;
    }
  ...
  shouldRenderAsWarning() {
    return (this.message.level === Protocol.Log.LogEntryLevel.Verbose || this.message.level === Protocol.Log.LogEntryLevel.Info) && (this.message.source === Protocol.Log.LogEntrySource.Violation || this.message.source === Protocol.Log.LogEntrySource.Deprecation || this.message.source === Protocol.Log.LogEntrySource.Intervention || this.message.source === Protocol.Log.LogEntrySource.Recommendation);
  }
  updateMessageIcon() {
    if (this.messageIcon) {
  ...
  async expandInlineStackFrames(debuggerModel, prefix, suffix, url, lineNumber, columnNumber, stackTrace, insertBefore) {
    ...
    for (let f = 0; f < frames.length; ++f) {
      ...
      const scriptLocationLink = this.linkifier.linkifyScriptLocation(
        ...
        url,
        lineNumber,
        { columnNumber, inlineFrameIndex: f }
      );
  ...
  createScriptLocationLinkForSyntaxError(debuggerModel, exceptionDetails) {
    ...
    const scriptLocationLink = this.linkifier.linkifyScriptLocation(
      debuggerModel.target(),
      exceptionDetails.scriptId || null,
      url,
      lineNumber,
      {
        columnNumber,
  ...
  tryFormatAsError(string, exceptionDetails) {
    ...
    for (let i = 0; i < linkInfos.length; ++i) {
      ...
      void this.expandInlineStackFrames(
        debuggerModel,
        prefixWithoutFunction,
        suffix,
        link.url,
        link.lineNumber,
        link.columnNumber,
  ...
  static tokenizeMessageText(string) {
    ...
    const results = TextUtils.TextUtils.Utils.splitStringByRegexes(string, tokenizerRegexes2);
...
const MaxLengthToIgnoreHighlighter = 1e4;
let maxTokenizableStringLength = 1e4;
let longStringVisibleLength = 5e3;
