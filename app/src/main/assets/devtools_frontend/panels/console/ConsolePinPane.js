...
export class ConsolePinPane extends UI.Widget.VBox {
  ...
  contextMenuEventFired(event) {
      ...
      if (targetPinElement) {
        ...
        if (targetPin instanceof ConsolePinPresenter) {
          contextMenu.editSection().appendItem(
            i18nString(UIStrings.removeExpression),
    ...
    contextMenu.editSection().appendItem(
      i18nString(UIStrings.removeAllExpressions),
...
export const DEFAULT_VIEW = (input, output, target) => {
  const deleteIconLabel = input.expression ? i18nString(UIStrings.removeExpressionS, { PH1: input.expression }) : i18nString(UIStrings.removeBlankExpression);
  const deleteRef = createRef();
  const editorRef = createRef();
  const isError = input.result && !("error" in input.result) && input.result?.exceptionDetails && !SDK.RuntimeModel.RuntimeModel.isSideEffectFailure(input.result);
  render(html`
    <style>${consolePinPaneStyles}</style>
...
export var ConsolePinEvent = /* @__PURE__ */ ((ConsolePinEvent2) => {
