...
export class ConsolePrompt extends Common.ObjectWrapper.eventMixin(
  UI.Widget.Widget
) {
  ...
  async handleEnter(forceEvaluate) {
    if (this.#selfXssWarningShown && (this.text() === i18nString(UIStrings.allowPasting) || this.text() === `'${i18nString(UIStrings.allowPasting)}'`)) {
      Common.Console.Console.instance().log(this.text());
      this.editor.dispatch({
        changes: { from: 0, to: this.editor.state.doc.length },
        scrollIntoView: true
      });
      Common.Settings.Settings.instance().createSetting("disable-self-xss-warning", false, Common.Settings.SettingStorageType.SYNCED).set(true);
  ...
  async evaluateCommandInConsole(executionContext, message, expression, useCommandLineAPI) {
    ...
    await executionContext.target().model(SDK.ConsoleModel.ConsoleModel)?.evaluateCommandInConsole(executionContext, message, expression, useCommandLineAPI);
  }
