...
export class AnimationUI {
  ...
  constructor(animation, timeline, parentElement) {
    ...
    if (!this.#animation.viewOrScrollTimeline()) {
      UI.UIUtils.installDragHandle(
        this.#activeIntervalGroup,
        this.mouseDown.bind(this, "AnimationDrag" /* ANIMATION_DRAG */, null),
      ...
      AnimationUI.installDragHandleKeyboard(
        this.#activeIntervalGroup,
        this.keydownMove.bind(this, "AnimationDrag" /* ANIMATION_DRAG */, null)
      );
    }
  ...
  drawAnimationLine(iteration, parentElement) {
    ...
    cache.animationLine.setAttribute(
      "x2",
      (this.duration() * this.#timeline.pixelTimeRatio() + Options.AnimationMargin).toFixed(2)
    );
  }
  drawDelayLine(parentElement) {
    ...
    const leftMargin = Math.min(
      this.#timeline.width(),
      (this.delayOrStartTime() + this.duration() * this.#animation.source().iterations()) * this.#timeline.pixelTimeRatio()
    );
    ...
    this.#endDelayLine.setAttribute(
      "x2",
      forwardsFill ? (this.#timeline.width() - leftMargin + margin).toFixed(2) : (this.#animation.source().endDelay() * this.#timeline.pixelTimeRatio() + margin).toFixed(2)
    );
  }
  drawPoint(iteration, parentElement, x, keyframeIndex, attachEvents) {
    ...
    const circle = UI.UIUtils.createSVGChild(
      parentElement,
      "circle",
      keyframeIndex <= 0 ? "animation-endpoint" : "animation-keyframe-point"
    );
    ...
    UI.ARIAUtils.setLabel(
      circle,
      keyframeIndex <= 0 ? i18nString(UIStrings.animationEndpointSlider) : i18nString(UIStrings.animationKeyframeSlider)
    );
    if (keyframeIndex <= 0) {
    ...
    let eventType;
    if (keyframeIndex === 0) {
      eventType = "StartEndpointMove" /* START_ENDPOINT_MOVE */;
    } else if (keyframeIndex === -1) {
      eventType = "FinishEndpointMove" /* FINISH_ENDPOINT_MOVE */;
    } else {
      eventType = "KeyframeMove" /* KEYFRAME_MOVE */;
    }
    if (!this.animation().viewOrScrollTimeline()) {
      UI.UIUtils.installDragHandle(
        circle,
        this.mouseDown.bind(this, eventType, keyframeIndex),
        this.mouseMove.bind(this),
        this.mouseUp.bind(this),
        "ew-resize"
      );
  ...
  renderKeyframe(iteration, keyframeIndex, parentElement, leftDistance, width, easing) {
    function createStepLine(parentElement2, x, strokeColor) {
      const line = UI.UIUtils.createSVGChild(parentElement2, "line");
    ...
    if (!cache[keyframeIndex]) {
      const svg = bezier ? UI.UIUtils.createSVGChild(parentElement, "path", "animation-keyframe") : UI.UIUtils.createSVGChild(parentElement, "g", "animation-keyframe-step");
    ...
    if (easing === "linear") {
      ...
      group.setAttribute(
        "d",
        ["M", 0, height, "L", 0, 5, "L", width.toFixed(2), 5, "L", width.toFixed(2), height, "Z"].join(" ")
      );
    } else if (bezier) {
      group.style.fill = this.#color;
  ...
  renderTransition() {
    ...
    this.renderKeyframe(
      ...
      0,
      activeIntervalGroup,
      Options.AnimationMargin,
      this.duration() * this.#timeline.pixelTimeRatio(),
    ...
    this.drawPoint(
      0,
      activeIntervalGroup,
      this.duration() * this.#timeline.pixelTimeRatio() + Options.AnimationMargin,
      -1,
      true
    );
  }
  ...
  delayOrStartTime() {
    ...
    if (this.#mouseEventType === "AnimationDrag" /* ANIMATION_DRAG */ || this.#mouseEventType === "StartEndpointMove" /* START_ENDPOINT_MOVE */) {
  ...
  duration() {
    ...
    if (this.#mouseEventType === "FinishEndpointMove" /* FINISH_ENDPOINT_MOVE */) {
      duration += this.#movementInMs;
    } else if (this.#mouseEventType === "StartEndpointMove" /* START_ENDPOINT_MOVE */) {
      duration -= this.#movementInMs;
    }
  ...
  offset(i) {
    ...
    if (this.#mouseEventType === "KeyframeMove" /* KEYFRAME_MOVE */ && i === this.#keyframeMoved) {
  ...
  mouseUp(event) {
    ...
    if (this.#mouseEventType === "KeyframeMove" /* KEYFRAME_MOVE */) {
...
export const Colors = /* @__PURE__ */ new Map([
