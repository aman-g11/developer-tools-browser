...
const nodeUIsByNode = /* @__PURE__ */ new WeakMap();
...
export class AnimationTimeline extends UI.Widget.VBox {
  ...
  constructor(toolbarView = DEFAULT_TOOLBAR_VIEW) {
    ...
    const noAnimationsPlaceholder = new UI.EmptyWidget.EmptyWidget(
      i18nString(UIStrings.waitingForAnimations),
      i18nString(UIStrings.animationDescription)
    );
    ...
    const noEffectSelectedPlaceholder = new UI.EmptyWidget.EmptyWidget(
      i18nString(UIStrings.noEffectSelected),
    ...
    SDK.TargetManager.TargetManager.instance().addModelListener(
      ...
      SDK.DOMModel.Events.NodeRemoved,
      (ev) => this.markNodeAsRemoved(ev.data.node),
      this,
      { scoped: true }
    );
  ...
  wasShown() {
    ...
    for (const animationModel of SDK.TargetManager.TargetManager.instance().models(
      SDK.AnimationModel.AnimationModel,
      { scoped: true }
    )) {
  ...
  removeEventListeners(animationModel) {
    animationModel.removeEventListener(
      SDK.AnimationModel.Events.AnimationGroupStarted,
      this.animationGroupStarted,
      this
    );
    animationModel.removeEventListener(
      SDK.AnimationModel.Events.AnimationGroupUpdated,
      this.animationGroupUpdated,
      this
    );
  ...
  createHeader() {
    ...
    this.#controlButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.replayTimeline),
      "replay",
      void 0,
    ...
    this.#gridHeader.setAttribute(
      "jslog",
      `${VisualLogging.timeline("animations.grid-header").track({ drag: true, click: true })}`
    );
    UI.UIUtils.installDragHandle(
      this.#gridHeader,
      this.scrubberDragStart.bind(this),
      this.scrubberDragMove.bind(this),
      this.scrubberDragEnd.bind(this),
      null
    );
  ...
  animationGroupUpdated({
    ...
    void this.#animationGroupUpdatedThrottler.schedule(async () => {
      ...
      if (group.isScrollDriven()) {
        ...
        if (animationNode) {
          const scrollRange = group.scrollOrientation() === Protocol.DOM.ScrollOrientation.Vertical ? await animationNode.verticalScrollRange() : await animationNode.horizontalScrollRange();
          const scrollOffset = group.scrollOrientation() === Protocol.DOM.ScrollOrientation.Vertical ? await animationNode.scrollTop() : await animationNode.scrollLeft();
          if (scrollRange !== null) {
...
var ControlState = /* @__PURE__ */ ((ControlState2) => {
