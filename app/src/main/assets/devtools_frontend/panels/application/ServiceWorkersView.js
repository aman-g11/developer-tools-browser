...
export class ServiceWorkersView extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    const fallbackToNetwork = new UI.Toolbar.ToolbarSettingCheckbox(
      ...
      i18nString(UIStrings.bypassTheServiceWorkerAndLoad)
    );
  ...
  modelAdded(serviceWorkerManager) {
    ...
    this.eventListeners.set(serviceWorkerManager, [
      ...
      this.securityOriginManager.addEventListener(
        SDK.SecurityOriginManager.Events.SecurityOriginAdded,
...
export class Section {
  ...
  constructor(manager, section, registration) {
    ...
    this.pushNotificationDataSetting = Common.Settings.Settings.instance().createLocalSetting(
      "push-data",
      i18nString(UIStrings.testPushMessageFromDevtools)
    );
    this.syncTagNameSetting = Common.Settings.Settings.instance().createLocalSetting("sync-tag-name", "test-tag-from-devtools");
    this.periodicSyncTagNameSetting = Common.Settings.Settings.instance().createLocalSetting("periodic-sync-tag-name", "test-tag-from-devtools");
    ...
    this.updateButton = UI.UIUtils.createTextButton(
      ...
      this.updateButtonClicked.bind(this),
    ...
    this.deleteButton = UI.UIUtils.createTextButton(i18nString(UIStrings.unregister), this.unregisterButtonClicked.bind(this), {
      variant: Buttons.Button.Variant.TEXT,
    ...
    this.createSyncNotificationField(
      i18nString(UIStrings.pushString),
      this.pushNotificationDataSetting.get(),
      i18nString(UIStrings.pushData),
      this.push.bind(this),
      "push-message"
    );
    this.createSyncNotificationField(
      i18nString(UIStrings.syncString),
      this.syncTagNameSetting.get(),
      i18nString(UIStrings.syncTag),
      this.sync.bind(this),
      "sync-tag"
    );
    this.createSyncNotificationField(
      i18nString(UIStrings.periodicSync),
      this.periodicSyncTagNameSetting.get(),
      i18nString(UIStrings.periodicSyncTag),
      (tag) => this.periodicSync(tag),
      "periodic-sync-tag"
    );
  ...
  createSyncNotificationField(label, initialValue, placeholder, callback, jslogContext) {
    ...
    const button = UI.UIUtils.createTextButton(label, void 0, { jslogContext });
    button.type = "submit";
  ...
  updateSourceField(version) {
    ...
    if (this.registration.errors.length) {
      ...
      UI.ARIAUtils.setLabel(
        errorsLabel,
        i18nString(UIStrings.sRegistrationErrors, { PH1: this.registration.errors.length })
      );
  ...
  update() {
    ...
    if (active) {
      ...
      if (active.isRunning() || active.isStarting()) {
        const stopButton = UI.UIUtils.createTextButton(
          ...
          this.stopButtonClicked.bind(this, active.id),
          { jslogContext: "stop" }
        );
    ...
    if (waiting) {
      const waitingEntry = this.addVersion(
        ...
        i18nString(UIStrings.sWaitingToActivate, { PH1: waiting.id })
      );
