...
const DEFAULT_VIEW = (input, output, target) => {
  render(html`
    ...
    ${input.allowed.length > 0 && input.disallowed.length > 0 ? html`<devtools-report-divider class="subsection-divider"></devtools-report-divider>` : nothing}
    ${renderDisallowed(
    input.disallowed,
    input.showDetails,
    input.onToggleShowDetails,
    input.onRevealDOMNode,
    input.onRevealHeader
  )}
...
export class PermissionsPolicySection extends UI.Widget.Widget {
  ...
  async performUpdate() {
    ...
    const disallowedData = this.#showDetails ? await Promise.all(disallowed.map(async (policy) => {
      ...
      const linkTargetDOMNode = await (blockReason === Protocol.Page.PermissionsPolicyBlockReason.IframeAttribute && frame?.getOwnerDOMNodeOrDocument() || void 0);
      const resource = frame?.resourceForURL(frame.url);
      const linkTargetRequest = blockReason === Protocol.Page.PermissionsPolicyBlockReason.Header && resource?.request || void 0;
