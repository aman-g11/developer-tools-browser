...
export class StorageMetadataView extends LegacyWrapper.LegacyWrapper.WrappableComponent {
  ...
  async renderReportContent() {
    ...
    const origin = this.#storageKey.origin;
    const ancestorChainHasCrossSite = Boolean(this.#storageKey.components.get(SDK.StorageKeyManager.StorageKeyComponent.ANCESTOR_CHAIN_BIT));
    ...
    return html`
        ${isIframeOrEmbedded ? html`${this.key(i18nString(UIStrings.origin))}
            ${this.value(html`<div class="text-ellipsis" title=${origin}>${origin}</div>`)}` : nothing}
        ${topLevelSite || topLevelSiteIsOpaque ? this.key(i18nString(UIStrings.topLevelSite)) : nothing}
        ...
        ${hasNonce || topLevelSiteIsOpaque ? this.key(i18nString(UIStrings.isOpaque)) : nothing}
        ${hasNonce ? this.value(i18nString(UIStrings.yes)) : nothing}
        ${topLevelSiteIsOpaque ? this.value(i18nString(UIStrings.yesBecauseTopLevelIsOpaque)) : nothing}
  ...
  async #deleteBucket() {
    ...
    const ok = await UI.UIUtils.ConfirmDialog.show(
      ...
      i18nString(UIStrings.confirmBucketDeletion, { PH1: this.#storageBucket.bucket.name || "" }),
      this,
