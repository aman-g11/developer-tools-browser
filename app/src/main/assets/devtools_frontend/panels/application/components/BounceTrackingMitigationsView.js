...
export var ScreenStatusType = /* @__PURE__ */ ((ScreenStatusType2) => {
  ScreenStatusType2["INITIALIZING"] = "Initializing";
  ScreenStatusType2["RUNNING"] = "Running";
  ScreenStatusType2["RESULT"] = "Result";
  ScreenStatusType2["DISABLED"] = "Disabled";
  return ScreenStatusType2;
})(ScreenStatusType || {});
const renderForceRunButton = (input) => {
  const isMitigationRunning = input.screenStatus === "Running" /* RUNNING */;
  return html`
    <devtools-button
...
const renderDeletedSitesOrNoSitesMessage = (input) => {
  ...
  if (input.trackingSites.length === 0) {
    return html`
      ...
      ${input.screenStatus === "Running" /* RUNNING */ ? html`
...
const renderMainFrameInformation = (input) => {
  if (input.screenStatus === "Initializing" /* INITIALIZING */) {
    return Lit.nothing;
  }
  if (input.screenStatus === "Disabled" /* DISABLED */) {
    return html`
      <devtools-report-section>
...
export class BounceTrackingMitigationsView extends UI.Widget.Widget {
  #trackingSites = [];
  #screenStatus = "Initializing" /* INITIALIZING */;
  ...
  constructor(element, view = DEFAULT_VIEW) {
    ...
    if (!mainTarget) {
      ...
      void mainTarget.systemInfo().invoke_getFeatureState({ featureState: "DIPS" }).then((state) => {
        this.#screenStatus = state.featureEnabled ? "Result" /* RESULT */ : "Disabled" /* DISABLED */;
        this.requestUpdate();
      });
    }
  }
