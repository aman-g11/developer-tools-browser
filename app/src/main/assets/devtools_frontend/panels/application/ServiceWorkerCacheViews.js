...
export class ServiceWorkerCacheView extends UI.View.SimpleView {
  ...
  constructor(model, cache) {
    ...
    this.preview = null;
    this.cache = cache;
    const bucketInfo = this.model.target().model(SDK.StorageBucketsModel.StorageBucketsModel)?.getBucketByName(cache.storageBucket.storageKey, cache.storageBucket.name);
    ...
    this.refreshButton = new UI.Toolbar.ToolbarButton(i18nString(UIStrings.refresh), "refresh", void 0, "cache-storage.refresh");
    ...
    this.deleteSelectedButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.deleteSelected),
      "cross",
      void 0,
  ...
  wasShown() {
    ...
    this.model.addEventListener(
      SDK.ServiceWorkerCacheModel.Events.CACHE_STORAGE_CONTENT_UPDATED,
  ...
  showPreview(preview) {
    ...
    if (!preview) {
      preview = new UI.EmptyWidget.EmptyWidget(
        i18nString(UIStrings.noCacheEntrySelected),
        i18nString(UIStrings.selectACacheEntryAboveToPreview)
      );
    }
  ...
  createRequest(entry) {
    ...
    let mimeType = Platform.MimeType.MimeType.PLAIN;
    if (header) {
    ...
    if (!resourceType) {
      resourceType = Common.ResourceType.ResourceType.fromURL(entry.requestURL) || Common.ResourceType.resourceTypes.Other;
    }
...
const networkRequestToPreview = /* @__PURE__ */ new WeakMap();
export class DataGridNode extends DataGrid.DataGrid.DataGridNode {
  ...
  createCell(columnId) {
    ...
    DataGrid.DataGrid.DataGridImpl.setElementText(
      cell,
      value || "",
      /* longText= */
      true,
...
export class RequestView extends UI.Widget.VBox {
  ...
  constructor(request) {
    ...
    this.resourceViewTabSetting = Common.Settings.Settings.instance().createSetting("cache-storage-view-tab", "preview");
    ...
    this.tabbedPane.appendTab(
      "preview",
      i18nString(UIStrings.preview),
      new Network.RequestPreviewView.RequestPreviewView(request)
    );
