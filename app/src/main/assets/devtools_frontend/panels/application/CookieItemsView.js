...
export const DEFAULT_COOKIE_PREVIEW_WIDGET_VIEW = (input, output, target) => {
  ...
  render(
    html`<style>${cookieItemsViewStyles}</style>
  ...
  render(
    ...
    <devtools-widget class="storage-view" ${widget(UI.Widget.VBox, { minimumSize: new Size(0, 50) })}>
      ...
      onRefreshCallback: input.onRefreshItems
        ...
        class=flex-none
        ${UI.Widget.widgetRef(StorageItemsToolbar, (toolbar) => {
      ...
      editable: true
        ...
        <devtools-widget slot="sidebar" ${widget(UI.Widget.VBox, { minimumSize: new Size(0, 50) })}
          ...
          ${input.selectedCookie ? html`<devtools-widget ${widget(CookiePreviewWidget, { cookie: input.selectedCookie })}>
                 </devtools-widget>` : html`<devtools-widget ${widget(UI.EmptyWidget.EmptyWidget, {
...
export class CookieItemsView extends UI.Widget.VBox {
  ...
  constructor(model, cookieDomain, view = DEFAULT_VIEW) {
    ...
    this.onlyIssuesFilterUI = new UI.Toolbar.ToolbarCheckbox(
      i18nString(UIStrings.onlyShowCookiesWithAnIssue),
      i18nString(UIStrings.onlyShowCookiesWhichHaveAn),
      () => {
