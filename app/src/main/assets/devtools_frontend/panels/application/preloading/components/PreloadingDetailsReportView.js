...
export class PreloadingDetailsReportView extends LegacyWrapper.LegacyWrapper.WrappableComponent {
  ...
  async #render() {
    await RenderCoordinator.write("PreloadingDetailsReportView render", () => {
      ...
      const isFallbackToPrefetch = pipeline.getPrerender()?.status === SDK.PreloadingModel.PreloadingStatus.FAILURE && (pipeline.getPrefetch()?.status === SDK.PreloadingModel.PreloadingStatus.READY || pipeline.getPrefetch()?.status === SDK.PreloadingModel.PreloadingStatus.SUCCESS);
      Lit.render(html`
        <style>${preloadingDetailsReportViewStyles}</style>
        <style>${UI.inspectorCommonStyles}</style>
        <devtools-report
  ...
  #url() {
    ...
    let value;
    if (attempt.action === Protocol.Preload.SpeculationAction.Prefetch && attempt.requestId !== void 0 && prefetchStatus !== SDK.PreloadingModel.PreloadingStatus.NOT_TRIGGERED) {
  ...
  #status(isFallbackToPrefetch) {
    assertNotNullOrUndefined(this.#data);
    const attempt = this.#data.pipeline.getOriginallyTriggered();
    const detailedStatus = isFallbackToPrefetch ? i18nString(UIStrings.detailedStatusFallbackToPrefetch) : PreloadingUIUtils.detailedStatus(attempt);
    return html`
