      ...
      return html`
        ...
        if (revealInElements) {
          ...
          input.onRevealInNetwork(ruleSet);
                              ...
                              title=${revealInElements ? i18nString(UIStrings.clickToOpenInElementsPanel) : i18nString(UIStrings.clickToOpenInNetworkPanel)}
                              style=${styleMap({
        border: "none",
        background: "none",
        color: "var(--icon-link)",
        ...
        "padding-inline-end": "0"
                              ...
                              jslog=${VisualLogging.action(revealInElements ? "reveal-in-elements" : "reveal-in-network").track({ click: true })}
                            >
        ...
        "vertical-align": "sub"
                    ...
                    <td>
                      ${ruleSet.errorType !== void 0 ? html`
                        <span style=${styleMap({ color: "var(--sys-color-error)" })}>
                          ${i18nString(UIStrings.errors, { errorCount: 1 })}
                        </span>` : ""} ${ruleSet.errorType !== Protocol.Preload.RuleSetErrorType.SourceIsNotJsonObject && ruleSet.errorType !== Protocol.Preload.RuleSetErrorType.InvalidRulesetLevelTag ? html`
                        <button class="link" role="link"
...
export class RuleSetGrid extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  #revealSpeculationRulesInNetwork(ruleSet) {
    ...
    const request = SDK.TargetManager.TargetManager.instance().scopeTarget()?.model(SDK.NetworkManager.NetworkManager)?.requestForId(ruleSet.requestId) || null;
    if (request === null) {
      return;
    }
