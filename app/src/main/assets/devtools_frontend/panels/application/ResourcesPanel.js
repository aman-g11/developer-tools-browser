...
export class ResourcesPanel extends UI.Panel.PanelWithSidebar {
  ...
  constructor(mode = "default") {
    ...
    this.resourcesLastSelectedItemSetting = Common.Settings.Settings.instance().createSetting("resources-last-selected-element-path", []);
  ...
  showCategoryView(categoryName, categoryHeadline, categoryDescription, categoryLink) {
    ...
    this.categoryView.element.setAttribute(
      "jslog",
      `${VisualLogging.pane().context(Platform.StringUtilities.toKebabCase(categoryName))}`
    );
