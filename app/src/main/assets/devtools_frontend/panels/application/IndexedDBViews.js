...
export class IDBDatabaseView extends ApplicationComponents.StorageMetadataView.StorageMetadataView {
  ...
  update(database) {
    ...
    const bucketInfo = this.model.target().model(SDK.StorageBucketsModel.StorageBucketsModel)?.getBucketByName(database.databaseId.storageBucket.storageKey, database.databaseId.storageBucket.name);
    if (bucketInfo) {
  ...
  async deleteDatabase() {
    const ok = await UI.UIUtils.ConfirmDialog.show(
      ...
      i18nString(UIStrings.confirmDeleteDatabase, { PH1: this.database.databaseId.name }),
      this,
...
export class IDBDataView extends UI.View.SimpleView {
  ...
  createEditorToolbar() {
    ...
    this.pageBackButton = new UI.Toolbar.ToolbarButton(i18nString(UIStrings.showPreviousPage), "triangle-left", void 0, "prev-page");
    ...
    this.pageForwardButton = new UI.Toolbar.ToolbarButton(i18nString(UIStrings.showNextPage), "triangle-right", void 0, "next-page");
