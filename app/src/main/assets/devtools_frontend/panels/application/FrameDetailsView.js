...
const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(html`
    ...
    jslog=${VisualLogging.pane("frames")}>
      ...
      ${input.permissionsPolicies ? widget(ApplicationComponents.PermissionsPolicySection.PermissionsPolicySection, {
...
function maybeRenderAdStatus(adFrameType, adFrameStatus) {
  if (adFrameType === void 0 || adFrameType === Protocol.Page.AdFrameType.None) {
    return nothing;
  }
...
function maybeRenderCreatorAdScriptAncestry(adFrameType, target, adScriptAncestry) {
  ...
  const rows = adScriptAncestry.ancestryChain.map((adScriptId) => {
    return html`<div>
      ${widget(
      Components.Linkifier.ScriptLocationLink,
...
function maybeRenderCoopCoepCSPStatus(info) {
  if (info) {
    return html`
      ...
      info.coep,
      i18n.i18n.lockedString("Cross-Origin Embedder Policy (COEP)"),
      Protocol.Network.CrossOriginEmbedderPolicyValue.None
    )}
          ${maybeRenderCrossOriginStatus(
      info.coop,
      i18n.i18n.lockedString("Cross-Origin Opener Policy (COOP)"),
...
function renderSharedArrayBufferAvailability(frame) {
  if (frame) {
    ...
    if (features) {
      let renderHint2 = function(frame2) {
      ...
      return html`
          ...
          <devtools-report-value title=${tooltipText}>
            ${availabilityText}\xA0${renderHint2(frame)}
          </devtools-report-value>
        `;
    }
  }
...
export class FrameDetailsReportView extends UI.Widget.Widget {
  ...
  set frame(frame) {
    ...
    if (rawCreationStackTrace) {
      void Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().createStackTraceFromProtocolRuntime(rawCreationStackTrace, creationTarget).then((creationStackTrace) => {
  ...
  async performUpdate() {
    ...
    if (result && result.ancestryChain.length > 0) {
      ...
      const debuggerModel = firstScript?.debuggerId ? await SDK.DebuggerModel.DebuggerModel.modelForDebuggerId(firstScript.debuggerId) : null;
