...
export class StorageView extends UI.Widget.VBox {
  ...
  clearButton;
  throttler = new Common.Throttler.Throttler(1e3);
  constructor() {
    ...
    for (const type of AllStorageTypes) {
      this.settings.set(
        ...
        Common.Settings.Settings.instance().createSetting(
          "clear-storage-" + Platform.StringUtilities.toKebabCase(type),
          true
        )
      );
    }
    ...
    this.clearButton = UI.UIUtils.createTextButton(
      i18nString(UIStrings.clearSiteData),
    ...
    const includeThirdPartyCookiesCheckbox = SettingsUI.SettingsUI.createSettingCheckbox(
      i18nString(UIStrings.includingThirdPartyCookies),
    ...
    this.quotaOverrideCheckbox.setAttribute(
      "jslog",
      `${VisualLogging.toggle("simulate-custom-quota").track({ change: true })}`
    );
    ...
    this.quotaOverrideEditor.setAttribute(
      "jslog",
      `${VisualLogging.textField("quota-override").track({ change: true })}`
    );
    ...
    this.appendItem(
      application,
      i18nString(UIStrings.unregisterServiceWorker),
  ...
  async applyQuotaOverrideFromInputField() {
    ...
    const cutoff = 9e12;
    if (quota >= cutoff) {
      this.quotaOverrideErrorMessage.textContent = i18nString(UIStrings.numberMustBeSmaller, { PH1: cutoff.toLocaleString() });
      return;
    }
  ...
  static clear(target, storageKey, originForCookies, selectedStorageTypes, includeThirdPartyCookies) {
    ...
    if (set.has(Protocol.Storage.StorageType.Cache_storage) || hasAll) {
      const target2 = SDK.TargetManager.TargetManager.instance().primaryPageTarget();
      const model = target2?.model(SDK.ServiceWorkerCacheModel.ServiceWorkerCacheModel);
      if (model) {
