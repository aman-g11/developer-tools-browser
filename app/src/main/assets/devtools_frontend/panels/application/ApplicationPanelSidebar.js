...
((SharedStorageTreeElementDispatcher2) => {
  let Events;
  ((Events2) => {
    Events2["SHARED_STORAGE_TREE_ELEMENT_ADDED"] = "SharedStorageTreeElementAdded";
  })(Events = SharedStorageTreeElementDispatcher2.Events || (SharedStorageTreeElementDispatcher2.Events = {}));
...
export class ApplicationPanelSidebar extends UI.Widget.VBox {
  ...
  constructor(panel) {
    ...
    this.localStorageListTreeElement = new ExpandableApplicationPanelTreeElement(
      panel,
      i18nString(UIStrings.localStorage),
      i18nString(UIStrings.noLocalStorage),
      i18nString(UIStrings.localStorageDescription),
      "local-storage"
    );
    ...
    this.sessionStorageListTreeElement = new ExpandableApplicationPanelTreeElement(
      panel,
      i18nString(UIStrings.sessionStorage),
      i18nString(UIStrings.noSessionStorage),
      i18nString(UIStrings.sessionStorageDescription),
      "session-storage"
    );
    ...
    this.extensionStorageListTreeElement = new ExpandableApplicationPanelTreeElement(
      panel,
      i18nString(UIStrings.extensionStorage),
      i18nString(UIStrings.noExtensionStorage),
      i18nString(UIStrings.extensionStorageDescription),
      "extension-storage"
    );
    ...
    this.cookieListTreeElement = new ExpandableApplicationPanelTreeElement(
      panel,
      i18nString(UIStrings.cookies),
      i18nString(UIStrings.noCookies),
      i18nString(UIStrings.cookiesDescription),
      "cookies"
    );
    ...
    this.backgroundFetchTreeElement = new BackgroundServiceTreeElement(panel, Protocol.BackgroundService.ServiceName.BackgroundFetch);
    backgroundServiceTreeElement.appendChild(this.backgroundFetchTreeElement);
    this.backgroundSyncTreeElement = new BackgroundServiceTreeElement(panel, Protocol.BackgroundService.ServiceName.BackgroundSync);
    ...
    this.notificationsTreeElement = new BackgroundServiceTreeElement(panel, Protocol.BackgroundService.ServiceName.Notifications);
    backgroundServiceTreeElement.appendChild(this.notificationsTreeElement);
    this.paymentHandlerTreeElement = new BackgroundServiceTreeElement(panel, Protocol.BackgroundService.ServiceName.PaymentHandler);
    backgroundServiceTreeElement.appendChild(this.paymentHandlerTreeElement);
    this.periodicBackgroundSyncTreeElement = new BackgroundServiceTreeElement(panel, Protocol.BackgroundService.ServiceName.PeriodicBackgroundSync);
    ...
    this.pushMessagingTreeElement = new BackgroundServiceTreeElement(panel, Protocol.BackgroundService.ServiceName.PushMessaging);
    ...
    if (Root.Runtime.hostConfig.deviceBoundSessionsDebugging?.enabled) {
      ...
      this.deviceBoundSessionsRootTreeElement = new DeviceBoundSessionsRootTreeElement(panel, this.deviceBoundSessionsModel);
    ...
    this.sharedStorageTreeElementDispatcher = new Common.ObjectWrapper.ObjectWrapper();
  ...
  targetAdded(target) {
    ...
    if (interestGroupModel) {
      interestGroupModel.addEventListener(
        InterestGroupModelEvents.INTEREST_GROUP_ACCESS,
        this.interestGroupAccess,
        this
      );
    }
  ...
  extensionStorageModelRemoved(model) {
    ...
    model.removeEventListener(
      ExtensionStorageModelEvents.EXTENSION_STORAGE_REMOVED,
  ...
  async addSharedStorage(sharedStorage) {
    ...
    this.sharedStorageTreeElementDispatcher.dispatchEventToListeners(
      "SharedStorageTreeElementAdded" /* SHARED_STORAGE_TREE_ELEMENT_ADDED */,
  ...
  generateChildren() {
    ...
    for (const section of staticSections) {
      ...
      UI.ARIAUtils.setLabel(
        ...
        i18nString(UIStrings.beforeInvokeAlert, { PH1: child.listItemElement.title })
      );
...
export class IndexedDBTreeElement extends ExpandableApplicationPanelTreeElement {
  ...
  constructor(storagePanel, storageBucket) {
    super(
      storagePanel,
      i18nString(UIStrings.indexeddb),
      i18nString(UIStrings.noIndexeddb),
      i18nString(UIStrings.indexeddbDescription),
      "indexed-db"
    );
    const icon = createIcon("database");
    this.setLeadingIcons([icon]);
  ...
  initialize() {
    SDK.TargetManager.TargetManager.instance().addModelListener(
      IndexedDBModel,
      IndexedDBModelEvents.DatabaseAdded,
      this.indexedDBAdded,
      this,
      { scoped: true }
    );
    SDK.TargetManager.TargetManager.instance().addModelListener(
      IndexedDBModel,
      IndexedDBModelEvents.DatabaseRemoved,
    ...
    SDK.TargetManager.TargetManager.instance().addModelListener(
      IndexedDBModel,
      IndexedDBModelEvents.DatabaseLoaded,
    ...
    SDK.TargetManager.TargetManager.instance().addModelListener(
      IndexedDBModel,
      IndexedDBModelEvents.IndexedDBContentUpdated,
  ...
  handleContextMenuEvent(event) {
    ...
    contextMenu.defaultSection().appendItem(
      i18nString(UIStrings.refreshIndexeddb),
...
export class DOMStorageTreeElement extends ApplicationPanelTreeElement {
  ...
  constructor(storagePanel, domStorage) {
    super(
      storagePanel,
      domStorage.storageKey ? SDK.StorageKeyManager.parseStorageKey(domStorage.storageKey).origin : i18nString(UIStrings.localFiles),
      false,
...
export class ResourcesSection {
  ...
  constructor(storagePanel, treeElement) {
    ...
    frameManager.addEventListener(
      SDK.FrameManager.Events.FRAME_REMOVED,
    ...
    frameManager.addEventListener(
      SDK.FrameManager.Events.RESOURCE_ADDED,
    ...
    if (this.panel.mode !== "node") {
      SDK.TargetManager.TargetManager.instance().addModelListener(
        ...
        SDK.ChildTargetManager.Events.TARGET_CREATED,
        this.windowOpened,
        this,
        { scoped: true }
      );
      SDK.TargetManager.TargetManager.instance().addModelListener(
        SDK.ChildTargetManager.ChildTargetManager,
        SDK.ChildTargetManager.Events.TARGET_INFO_CHANGED,
        this.windowChanged,
        this,
        { scoped: true }
      );
      SDK.TargetManager.TargetManager.instance().addModelListener(
        SDK.ChildTargetManager.ChildTargetManager,
        SDK.ChildTargetManager.Events.TARGET_DESTROYED,
...
export class FrameTreeElement extends ApplicationPanelTreeElement {
  ...
  async frameNavigated(frame) {
    ...
    if (frame.isOutermostFrame()) {
      ...
      for (const target of targets) {
        if (target.type() === SDK.Target.Type.ServiceWorker && SDK.TargetManager.TargetManager.instance().isInScope(target)) {
          const targetId = target.id();
          assertNotMainTarget(targetId);
  ...
  appendResource(resource) {
    ...
    if (!categoryElement) {
      categoryElement = new ExpandableApplicationPanelTreeElement(
        ...
        "",
        i18nString(UIStrings.resourceDescription),
        categoryName,
        categoryName === "Frames"
      );
  ...
  windowOpened(targetInfo) {
    ...
    if (!categoryElement) {
      categoryElement = new ExpandableApplicationPanelTreeElement(
        this.section.panel,
        i18nString(UIStrings.openedWindows),
        "",
        i18nString(UIStrings.openedWindowsDescription),
        categoryKey
      );
  ...
  workerCreated(targetInfo) {
    ...
    const categoryName = targetInfo.type === "service_worker" ? i18n.i18n.lockedString("Service workers") : i18nString(UIStrings.webWorkers);
    let categoryElement = this.categoryElements.get(categoryKey);
    if (!categoryElement) {
      categoryElement = new ExpandableApplicationPanelTreeElement(
        this.section.panel,
        categoryName,
        "",
        i18nString(UIStrings.workerDescription),
        categoryKey
      );
      this.categoryElements.set(categoryKey, categoryElement);
      this.appendChild(categoryElement, FrameTreeElement.presentationOrderCompare);
...
const resourceToFrameResourceTreeElement = /* @__PURE__ */ new WeakMap();
export class FrameResourceTreeElement extends ApplicationPanelTreeElement {
  ...
  constructor(storagePanel, resource) {
    super(
      storagePanel,
      resource.isGenerated ? i18nString(UIStrings.documentNotAvailable) : resource.displayName,
      false,
      "frame-resource"
    );
    this.panel = storagePanel;
  ...
  onselect(selectedByUser) {
    ...
    if (this.resource.isGenerated) {
      this.panel.showCategoryView(
        "",
        i18nString(UIStrings.documentNotAvailable),
        i18nString(UIStrings.theContentOfThisDocumentHasBeen),
        null
      );
    } else {
