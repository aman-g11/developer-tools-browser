    ...
    render(html`
      ...
      onReportSelected: input.onReportSelected
            ...
            <div slot="sidebar" class="vbox" jslog=${VisualLogging.pane("preview").track({ resize: true })}>
              ${input.focusedReport ? widget(SourceFrame.JSONView.SearchableJsonView, { jsonObject: input.focusedReport.body }) : widget(UI.EmptyWidget.EmptyWidget, {
...
export class ReportingApiView extends UI.Widget.VBox {
  ...
  modelAdded(networkManager) {
    ...
    this.#networkManager.addEventListener(
      SDK.NetworkManager.Events.ReportingApiReportUpdated,
  ...
  modelRemoved(networkManager) {
    ...
    this.#networkManager.removeEventListener(
      SDK.NetworkManager.Events.ReportingApiReportAdded,
      this.#onReportAdded,
      this
    );
    this.#networkManager.removeEventListener(
