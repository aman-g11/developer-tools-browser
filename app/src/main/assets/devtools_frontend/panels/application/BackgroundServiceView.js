...
export class BackgroundServiceView extends UI.Widget.VBox {
  ...
  constructor(serviceName, model) {
    ...
    this.securityOriginManager.addEventListener(
      SDK.SecurityOriginManager.Events.MainSecurityOriginChanged,
      () => this.onOriginChanged()
    );
    this.storageKeyManager = this.model.target().model(SDK.StorageKeyManager.StorageKeyManager);
    if (!this.storageKeyManager) {
      throw new Error("StorageKeyManager instance is missing");
    }
    this.storageKeyManager.addEventListener(
      SDK.StorageKeyManager.Events.MAIN_STORAGE_KEY_CHANGED,
  ...
  async setupToolbar() {
    ...
    const clearButton = new UI.Toolbar.ToolbarButton(i18nString(UIStrings.clear), "clear", void 0, "background-service.clear");
    ...
    this.saveButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.saveEvents),
      "download",
      void 0,
  ...
  toggleRecording() {
    ...
    if (isRecording) {
      UI.ARIAUtils.LiveAnnouncer.alert(
        i18nString(UIStrings.recordingSActivity, { PH1: featureName }) + " " + i18nString(UIStrings.devtoolsWillRecordAllSActivity, { PH1: featureName })
      );
      this.preview?.focus();
    }
  }
  ...
  updateRecordButtonTooltip() {
    const buttonTooltip = this.recordButton.isToggled() ? i18nString(UIStrings.stopRecordingEvents) : i18nString(UIStrings.startRecordingEvents);
  ...
  createDataGrid() {
    ...
    dataGrid.addEventListener(
      ...
      (event) => this.showPreview(event.data)
    );
    return dataGrid;
  }
  /**
  ...
  createEventData(serviceEvent) {
    ...
    return {
      ...
      timestamp: UI.UIUtils.formatTimestamp(
  ...
  showPreview(dataNode) {
    ...
    if (this.dataGrid.rootNode().children.length) {
      emptyWidget = new UI.EmptyWidget.EmptyWidget(
        i18nString(UIStrings.noEventSelected),
        i18nString(UIStrings.selectAnEventToViewMetadata)
      );
