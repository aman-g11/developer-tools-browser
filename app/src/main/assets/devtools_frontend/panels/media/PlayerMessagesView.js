...
export class PlayerMessagesView extends UI.Widget.VBox {
  ...
  createDropdown() {
    ...
    UI.ARIAUtils.setLabel(
      ...
      `${i18nString(UIStrings.logLevel)} ${this.messageLevelSelector.defaultTitle()}`
    );
    return dropDownItem;
  }
  createFilterInput() {
    const filterInput = new UI.Toolbar.ToolbarFilter(i18nString(UIStrings.filterByLogMessages), 1, 1);
    filterInput.addEventListener(UI.Toolbar.ToolbarInput.Event.TEXT_CHANGED, (data) => {
      ...
      ([key, value]) => html`<div>${key}: ${value}</div>`
        ...
        <div class="status-error-field-labeled">
          ${error.stack.length !== 0 ? html`<span class="status-error-field-label"
                  ...
                  <div>
                    ${error.stack.map(
      (stackEntry) => html`<div>${stackEntry.file}:${stackEntry.line}</div>`
    )}
                  </div>` : nothing}
        </div>
        <div class="status-error-field-labeled">
