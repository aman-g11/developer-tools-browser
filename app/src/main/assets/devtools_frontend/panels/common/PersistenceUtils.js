...
export class PersistenceUtils {
  static tooltipForUISourceCode(uiSourceCode) {
    ...
    if (binding.network.contentType().isFromSourceMap()) {
      return i18nString(
        UIStrings.linkedToSourceMapS,
        { PH1: Platform.StringUtilities.trimMiddle(binding.network.url(), 150) }
      );
    }
  ...
  static iconForUISourceCode(uiSourceCode) {
    ...
    if (uiSourceCode.project().type() !== Workspace.Workspace.projectTypes.FileSystem || !Common.ParsedURL.schemeIs(uiSourceCode.url(), "file:")) {
      return null;
    }
