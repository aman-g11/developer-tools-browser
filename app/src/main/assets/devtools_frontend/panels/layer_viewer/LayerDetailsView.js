...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  const formatStickyAncestorLayer = (title, layer2) => {
    ...
    return i18nString(UIStrings.stickyAncestorLayersS, { PH1: title, PH2: name, PH3: layer2.id() });
  };
...
export class LayerDetailsView extends Common.ObjectWrapper.eventMixin(
  UI.Widget.Widget
) {
  layerViewHost;
  layerSnapshotMap;
  ...
  performUpdate() {
    ...
    const snapshotSelection = (this.selection && this.selection.type() === Type.SNAPSHOT ? this.selection : layer ? this.layerSnapshotMap.get(layer) : null) || null;
    this.view(
      {
        layer,
        snapshotSelection,
