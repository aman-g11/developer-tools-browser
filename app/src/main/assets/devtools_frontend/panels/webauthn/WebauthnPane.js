...
const i18nTemplate = Lit.i18nTemplate.bind(void 0, str_);
        ...
        ${credentials.length ? repeat(credentials, (c) => c.credentialId, (credential) => html`
          ...
          <tr>
            <td class="center" colspan=6>
              ${i18nTemplate(
    UIStrings.noCredentialsTryCallingSFromYour,
  ...
  return html`
    ...
    ...isCtap2 ? [Protocol.WebAuthn.AuthenticatorTransport.Internal] : []
                ...
                <option value=${option} jslog=${VisualLogging.item(option).track({ click: true })}
                        ...
                        .disabled=${!internalTransportAvailable && option === Protocol.WebAuthn.AuthenticatorTransport.Internal}>
                  ${option}
                </option>`)}
          </select>
        </div>
...
export const DEFAULT_VIEW = (input, output, target) => {
  render(
    ...
    <div class="webauthn-pane flex-auto ${classMap({ enabled: input.enabled })}">
      ...
      input.newAuthenticatorOptions,
      input.internalTransportAvailable,
      input.updateNewAuthenticatorOptions,
      input.addAuthenticator
    )}
    </div>`,
    target
  );
...
export class WebauthnPaneImpl extends UI.Panel.Panel {
  async #addAuthenticator(options) {
    ...
    this.#model.addEventListener(
      SDK.WebAuthnModel.Events.CREDENTIAL_ADDED,
      this.#addCredential.bind(this, authenticatorId)
    );
    this.#model.addEventListener(
      SDK.WebAuthnModel.Events.CREDENTIAL_ASSERTED,
      this.#updateCredential.bind(this, authenticatorId)
    );
    this.#model.addEventListener(
      SDK.WebAuthnModel.Events.CREDENTIAL_UPDATED,
      this.#updateCredential.bind(this, authenticatorId)
    );
    this.#model.addEventListener(
      SDK.WebAuthnModel.Events.CREDENTIAL_DELETED,
      this.#deleteCredential.bind(this, authenticatorId)
    );
  ...
  constructor(view = DEFAULT_VIEW) {
    ...
    this.#availableAuthenticatorSetting = Common.Settings.Settings.instance().createSetting(
  ...
  #updateCredential(authenticatorId, {
    data: event
  }) {
    const authenticator = this.#authenticators.get(authenticatorId);
    if (!authenticator) {
  ...
  #deleteCredential(authenticatorId, {
    data: event
  }) {
    const authenticator = this.#authenticators.get(authenticatorId);
    if (!authenticator) {
  ...
  #updateInternalTransportAvailability() {
    ...
    if (this.#hasInternalAuthenticator && this.#newAuthenticatorOptions.transport === Protocol.WebAuthn.AuthenticatorTransport.Internal) {
