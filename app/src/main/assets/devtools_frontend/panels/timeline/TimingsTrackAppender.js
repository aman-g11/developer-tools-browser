...
export class TimingsTrackAppender {
  ...
  appendTrackAtLevel(trackStartLevel, expanded) {
    ...
    const timestampEvents = this.#parsedTrace.data.UserTimings.timestampEvents.filter(
      (timeStamp) => !Trace.Handlers.ModelHandlers.ExtensionTraceData.extensionDataInConsoleTimeStamp(timeStamp).devtoolsObj
    );
  ...
  #appendTrackHeaderAtLevel(currentLevel, expanded) {
    ...
    const style = buildGroupStyle({
      ...
      collapsible: trackIsCollapsible ? PerfUI.FlameChart.GroupCollapsibleState.IF_MULTI_ROW : PerfUI.FlameChart.GroupCollapsibleState.NEVER
    });
    const group = buildTrackHeader(
      VisualLoggingTrackName.TIMINGS,
      currentLevel,
      i18nString(UIStrings.timings),
      style,
      /* selectable= */
      true,
      expanded
  ...
  */
  ...
  setPopoverInfo(event, info) {
    const isExtensibilityMarker = Trace.Types.Extensions.isSyntheticExtensionEntry(event) && Trace.Types.Extensions.isExtensionPayloadMarker(event.devtoolsObj);
    if (isExtensibilityMarker) {
      info.title = event.devtoolsObj.tooltipText || event.name;
    }
    if (Trace.Types.Events.isMarkerEvent(event) || Trace.Types.Events.isPerformanceMark(event) || Trace.Types.Events.isConsoleTimeStamp(event) || isExtensibilityMarker) {
