...
export class ExtensionTrackAppender {
  ...
  #appendTopLevelHeaderAtLevel(currentLevel, compact, expanded) {
    const style = compact ? buildGroupStyle({ shareHeaderLine: true, collapsible: PerfUI.FlameChart.GroupCollapsibleState.NEVER }) : buildGroupStyle({ shareHeaderLine: false, collapsible: PerfUI.FlameChart.GroupCollapsibleState.ALWAYS });
    const headerTitle = this.#extensionTopLevelTrack.name;
    const jsLogContext = this.#extensionTopLevelTrack.name === "\u{1F170}\uFE0F Angular" ? VisualLoggingTrackName.ANGULAR_TRACK : VisualLoggingTrackName.EXTENSION;
  ...
  setPopoverInfo(event, info) {
    info.title = Trace.Types.Extensions.isSyntheticExtensionEntry(event) && event.devtoolsObj.tooltipText ? event.devtoolsObj.tooltipText : this.titleForEvent(event);
