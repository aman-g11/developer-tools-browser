...
export class TimelineTreeView extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  populateToolbar(toolbar) {
    this.caseSensitiveButton = new UI.Toolbar.ToolbarToggle(i18nString(UIStrings.matchCase), "match-case", void 0, "match-case");
    ...
    this.regexButton = new UI.Toolbar.ToolbarToggle(
      i18nString(UIStrings.useRegularExpression),
      "regular-expression",
      void 0,
      "regular-expression"
    );
    ...
    this.matchWholeWord = new UI.Toolbar.ToolbarToggle(
      i18nString(UIStrings.matchWholeWord),
      "match-whole-word",
      void 0,
      "match-whole-word"
    );
  ...
  #filterChanged() {
    ...
    this.textFilterInternal.setRegExp(
      searchQuery ? Platform.StringUtilities.createSearchRegex(searchQuery, caseSensitive, isRegex, matchWholeWord) : null
    );
    this.refreshTree();
  }
  onShowModeChanged() {
...
((TimelineTreeView2) => {
  let Events;
  ((Events2) => {
    Events2["TREE_ROW_HOVERED"] = "TreeRowHovered";
    Events2["BOTTOM_UP_BUTTON_CLICKED"] = "BottomUpButtonClicked";
    Events2["TREE_ROW_CLICKED"] = "TreeRowClicked";
  })(Events = TimelineTreeView2.Events || (TimelineTreeView2.Events = {}));
...
export class GridNode extends DataGrid.SortableDataGrid.SortableDataGridNode {
  ...
  createValueCell(columnId) {
    ...
    if (showPercents && this.treeView.exposePercentages()) {
      textDiv.createChild("span", "percent-column").textContent = i18nString(UIStrings.percentPlaceholder, { PH1: (value / this.grandTotalTime * 100).toFixed(1) });
    }
    if (maxTime) {
...
const treeNodeToGridNode = /* @__PURE__ */ new WeakMap();
export class AggregatedTimelineTreeView extends TimelineTreeView {
  ...
  displayInfoForGroupNode(node) {
    ...
    switch (this.groupBySetting.get()) {
      case AggregatedTimelineTreeView.GroupBy.Category: {
        ...
        const color2 = category instanceof Trace.Styles.TimelineCategory ? ThemeSupport.ThemeSupport.instance().getComputedValue(category.cssVariable) : category.color;
  ...
  populateToolbar(toolbar) {
    ...
    toolbar.appendToolbarItem(this.splitWidget.createShowHideSidebarButton(
      i18nString(UIStrings.showHeaviestStack),
      i18nString(UIStrings.hideHeaviestStack),
      i18nString(UIStrings.heaviestStackShown),
      i18nString(UIStrings.heaviestStackHidden)
    ));
  }
  ...
  domainByEvent(groupBy, event) {
    ...
    if (!url) {
      ...
      if (groupBy === AggregatedTimelineTreeView.GroupBy.ThirdParties && entity) {
        ...
        const parsedURL2 = Common.ParsedURL.ParsedURL.fromString(firstDomain);
...
export class TimelineStackView extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(treeView) {
    ...
    this.dataGrid.element.addEventListener(
      "mouseleave",
      () => this.dispatchEventToListeners(TimelineStackView.Events.TREE_ROW_HOVERED, null)
    );
...
((TimelineStackView2) => {
  let Events;
  ((Events2) => {
    Events2["SELECTION_CHANGED"] = "SelectionChanged";
    Events2["TREE_ROW_HOVERED"] = "TreeRowHovered";
  })(Events = TimelineStackView2.Events || (TimelineStackView2.Events = {}));
