...
const listFormatter = /* @__PURE__ */ function defineFormatter() {
  let intlListFormat;
  return {
    format(...args) {
      if (!intlListFormat) {
...
export class TimelineHistoryManager {
  ...
  constructor(minimapComponent, isNode) {
    ...
    this.#landingPageTitle = isNode ? i18nString(UIStrings.nodeLandingPageTitle) : i18nString(UIStrings.landingPageTitle);
  ...
  addRecording(newInput) {
    ...
    UI.ARIAUtils.setLabel(
      this.#button.element,
      i18nString(UIStrings.currentSessionSS, { PH1: modelTitle, PH2: buttonTitle })
    );
    this.updateState();
  ...
  #buildTextDetails(metadata, title) {
    ...
    if (metadata) {
      const parts = [
        ...
        metadata.cpuThrottling ? i18nString(UIStrings.dSlowdown, { PH1: metadata.cpuThrottling }) : void 0,
...
const parsedTraceIndexToPerformancePreviewData = /* @__PURE__ */ new Map();
