...
export function getFieldMetrics(parsedTrace, insightSetKey) {
  ...
  const fieldMetricsResults = Trace.Insights.Common.getFieldMetricsForInsightSet(insightSet, parsedTrace.metadata, scope);
  if (!fieldMetricsResults) {
    return null;
  }
  return fieldMetricsResults;
}
const CWV_METRICS_VIEW = (input, _output, target) => {
  const {
    parsedTrace,
    insightSetKey,
  ...
  const localValues = {
    lcp: local?.lcp?.value !== void 0 ? Trace.Helpers.Timing.microToMilli(local?.lcp.value) : void 0,
    inp: local?.inp?.value !== void 0 ? Trace.Helpers.Timing.microToMilli(local?.inp.value) : void 0
  };
  const fieldValues = field && {
    lcp: field.lcp?.value !== void 0 ? Trace.Helpers.Timing.microToMilli(field.lcp.value) : void 0,
    inp: field.inp?.value !== void 0 ? Trace.Helpers.Timing.microToMilli(field.inp.value) : void 0
  };
  ...
  function renderMetricValue(metric, value, relevantEvent) {
    ...
    if (value === null) {
      ...
      classification = Trace.Handlers.ModelHandlers.PageLoadMetrics.scoreClassificationForLargestContentfulPaint(micros);
    } else if (metric === "CLS") {
      ...
      valueText = text;
      valueDisplay = element;
      classification = Trace.Handlers.ModelHandlers.UserInteractions.scoreClassificationForInteractionToNextPaint(micros);
    } else {
      Platform.TypeScriptUtilities.assertNever(metric, `Unexpected metric ${metric}`);
    }
    const title = value !== null ? i18nString(UIStrings.metricScore, { PH1: metric, PH2: valueText, PH3: classification }) : i18nString(UIStrings.metricScoreUnavailable, { PH1: metric });
    return html`
      <button class="metric"
  ...
  if (field) {
    ...
    const lcpEl2 = renderMetricValue("LCP", lcp?.value ?? null, null);
    const inpEl2 = renderMetricValue("INP", inp?.value ?? null, null);
    const clsEl2 = renderMetricValue("CLS", cls?.value ?? null, null);
...
//# sourceMappingURL=CWVMetrics.js.map
