...
export class LayoutShiftDetails extends UI.Widget.Widget {
  ...
  #togglePopover(e) {
    ...
    const event = Trace.Types.Events.isSyntheticLayoutShift(this.#event) ? this.#event : this.#event.events.find((e2) => e2.ts === parseInt(rowEl.getAttribute("data-ts") ?? "", 10));
    this.contentElement.dispatchEvent(
...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(html`
      ...
      <div class="layout-shift-summary-details">
        ...
        ${Trace.Types.Events.isSyntheticLayoutShift(input.event) ? renderLayoutShiftDetails(
    input.event,
