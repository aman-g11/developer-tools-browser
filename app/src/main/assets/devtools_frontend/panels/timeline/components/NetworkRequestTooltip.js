...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  Lit.render(html`
    ...
    <div class="performance-card">
      ...
      ${Trace.Helpers.Network.isSyntheticNetworkRequestEventRenderBlocking(networkRequest) ? html`<div class="render-blocking"> ${i18nString(UIStrings.renderBlocking)} </div>` : Lit.nothing}
      <div class="divider"></div>
...
export class NetworkRequestTooltip extends UI.Widget.Widget {
  ...
  static renderTimings(networkRequest) {
    ...
    const wasThrottled = sdkNetworkRequest && SDK.NetworkManager.MultitargetNetworkManager.instance().appliedRequestConditions(
    ...
    return html`
      <div
        ...
        ${wasThrottled?.urlPattern ? html`<devtools-icon
          class=indicator
          name=watch
          ></devtools-icon>` : html`<span class="indicator"></span>`}
  ...
  performUpdate() {
    ...
    const networkConditions = sdkNetworkRequest && SDK.NetworkManager.MultitargetNetworkManager.instance().appliedRequestConditions(
      sdkNetworkRequest.networkRequest
    );
