...
export class LiveMetricsView extends LegacyWrapper.LegacyWrapper.WrappableComponent {
  ...
  #getPageScopeLabel(pageScope) {
    ...
    if (key) {
      return pageScope === "url" ? i18nString(UIStrings.urlOptionWithKey, { PH1: key }) : i18nString(UIStrings.originOptionWithKey, { PH1: key });
    }
              ...
              <details>
                <summary>
                  <span class="interaction-type">
                    ${interaction.interactionType} ${isInp ? html`<span class="interaction-inp-chip" title=${i18nString(UIStrings.inpInteraction)}>INP</span>` : nothing}
                  </span>
...
class LiveMetricsLogs extends UI.Widget.WidgetElement {
  ...
  createWidget() {
    ...
    const clearButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.clearCurrentLog),
      "clear",
      void 0,
