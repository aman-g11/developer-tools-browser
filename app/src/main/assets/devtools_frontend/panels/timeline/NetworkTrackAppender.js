...
export class NetworkTrackAppender {
  ...
  constructor(flameChartData, events) {
    ...
    ThemeSupport.ThemeSupport.instance().addEventListener(ThemeSupport.ThemeChangeEvent.eventName, () => {
      if (this.#group) {
        ...
        this.#group.style.backgroundColor = ThemeSupport.ThemeSupport.instance().getComputedValue("--sys-color-cdt-base-container");
      }
    });
  }
  group() {
  ...
  #appendTrackHeaderAtLevel(_currentLevel, expanded) {
    ...
    this.#group = buildTrackHeader(
      VisualLoggingTrackName.NETWORK,
      0,
  ...
  #appendEventsAtLevel(events, trackStartLevel) {
    for (let i = 0; i < events.length; ++i) {
      ...
      if (Trace.Types.Events.isSyntheticNetworkRequest(event) && Trace.Helpers.Network.isSyntheticNetworkRequestEventRenderBlocking(event)) {
    ...
    return this.relayoutEntriesWithinBounds(
      events,
      Trace.Types.Timing.Milli(-Infinity),
      Trace.Types.Timing.Milli(Infinity)
    );
  }
  /**
