...
export class SignedExchangeInfoView extends UI.Widget.VBox {
  ...
  constructor(request) {
    ...
    if (signedExchangeInfo.header) {
      ...
      this.responseHeadersItem = headerCategory.createLeaf(this.formatHeader(i18nString(UIStrings.responseHeaders), ""));
      ...
      for (let i = 0; i < header.signatures.length; ++i) {
        ...
        signatureCategory.createLeaf(this.formatHeaderForHexData(
          i18nString(UIStrings.signature),
          signature.signature,
        ...
        if (signature.certUrl) {
          const certURLElement = this.formatHeader(
            i18nString(UIStrings.certificateUrl),
            signature.certUrl,
    ...
    if (signedExchangeInfo.securityDetails) {
      ...
      securityCategory.createLeaf(
        this.formatHeader(i18nString(UIStrings.validFrom), new Date(1e3 * securityDetails.validFrom).toUTCString())
      );
      securityCategory.createLeaf(
        this.formatHeader(i18nString(UIStrings.validUntil), new Date(1e3 * securityDetails.validTo).toUTCString())
      );
