...
import { Events, RequestPanelBehavior } from "./NetworkDataGridNode.js";
...
export class NetworkPanel extends UI.Panel.Panel {
  ...
  constructor(displayScreenshotDelay) {
    ...
    this.networkLogShowOverviewSetting = Common.Settings.Settings.instance().createSetting("network-log-show-overview", true);
    this.networkLogLargeRowsSetting = Common.Settings.Settings.instance().createSetting("network-log-large-rows", false);
    this.networkRecordFilmStripSetting = Common.Settings.Settings.instance().createSetting("network-record-film-strip-setting", false);
    ...
    this.showSettingsPaneSetting = Common.Settings.Settings.instance().createSetting("network-show-settings-toolbar", false);
    ...
    this.overviewPane.addEventListener(
      PerfUI.TimelineOverviewPane.Events.OVERVIEW_PANE_WINDOW_CHANGED,
    ...
    if (Annotations.AnnotationRepository.annotationsEnabled()) {
      ...
      if (dataGrid) {
        PanelCommon.AnnotationManager.instance().initializePlacementForAnnotationType(
          ...
          this.resolveInitialState.bind(this),
  ...
  static instance(opts) {
    if (!networkPanelInstance || opts?.forceNew) {
      networkPanelInstance = new NetworkPanel(opts?.displayScreenshotDelay ?? 1e3);
    }
  ...
  setupToolbarButtons(splitWidget) {
    const searchToggle = new UI.Toolbar.ToolbarToggle(i18nString(UIStrings.search), "search", void 0, "search");
    ...
    const disableCacheCheckbox = new UI.Toolbar.ToolbarSettingCheckbox(
      ...
      i18nString(UIStrings.disableCacheWhileDevtoolsIsOpen),
    ...
    const networkConditionsButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.moreNetworkConditions),
      "network-settings",
      void 0,
      "network-conditions"
    );
    ...
    this.panelToolbar.appendSeparator();
    const importHarButton = new UI.Toolbar.ToolbarButton(i18nString(UIStrings.importHarFile), "import", void 0, "import-har");
    ...
    const exportHarButton = new UI.Toolbar.ToolbarButton(i18nString(UIStrings.exportHarSanitized), "download", void 0, "export-har");
  ...
  createThrottlingConditionsSelect() {
    ...
    MobileThrottling.NetworkThrottlingSelector.NetworkThrottlingSelect.createForGlobalConditions(
      toolbarItem.element,
      i18nString(UIStrings.throttling)
    );
    return toolbarItem;
  }
  ...
  appendApplicableItems(event, contextMenu, target) {
    const appendRevealItem = (request) => {
      contextMenu.revealSection().appendItem(
        ...
        () => UI.ViewManager.ViewManager.instance().showView("network").then(this.networkLogView.resetFilter.bind(this.networkLogView)).then(this.revealAndHighlightRequest.bind(this, request)),
    ...
    const appendRevealItemAndSelect = (request) => {
      contextMenu.revealSection().appendItem(
        i18nString(UIStrings.openInNetworkPanel),
        () => UI.ViewManager.ViewManager.instance().showView("network").then(this.networkLogView.resetFilter.bind(this.networkLogView)).then(this.selectAndActivateRequest.bind(
          this,
  ...
  onUpdateRequest(event) {
    ...
    this.overviewPane.setBounds(
      Trace.Types.Timing.Milli(this.calculator.minimumBoundary() * 1e3),
      Trace.Types.Timing.Milli(this.calculator.maximumBoundary() * 1e3)
    );
