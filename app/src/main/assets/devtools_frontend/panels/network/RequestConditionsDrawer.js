  ...
  return html`
    ...
    ${!originalOrUpgradedURLPattern ? html`
      ...
      <devtools-tooltip variant=rich jslogcontext=url-pattern-warning id=url-pattern-error-${index}>
        ${SDK.NetworkManager.RequestURLPattern.isValidPattern(constructorStringOrWildcardURL) === SDK.NetworkManager.RequestURLPatternValidity.HAS_REGEXP_GROUPS ? i18nString(UIStrings.patternFailedWithRegExpGroups) : i18nString(UIStrings.patternFailedToParse)}
        ${learnMore()}
      </devtools-tooltip>` : nothing}
    <div
      @click=${toggle}
...
export class RequestConditionsDrawer extends UI.Widget.VBox {
  ...
  createEditor() {
    ...
    const learnMore2 = Link.create(PATTERN_API_DOCS_URL, i18nString(UIStrings.learnMore), void 0, "learn-more");
    learnMore2.title = i18nString(UIStrings.learnMoreLabel);
  ...
  static async reveal(appliedConditions) {
    ...
    const conditions = drawer.manager.requestConditions.conditions.find(
      (condition) => condition.ruleIds.has(appliedConditions.appliedNetworkConditionsId) && condition.constructorString && condition.constructorString === appliedConditions.urlPattern
    );
