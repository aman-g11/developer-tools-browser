  ...
  render(
    html`
      ...
      contents: html`<div jslog=${VisualLogging.section("general")}>
            ...
            ${input.request.statusCode ? renderGeneralRow(
        input,
        i18nString(UIStrings.requestMethod),
        input.request.requestMethod,
        "request-method"
      ) : Lit.nothing}
            ${input.request.statusCode ? renderGeneralRow(
        input,
        i18nString(UIStrings.statusCode),
        statusText,
        "status-code",
        statusClasses
      ) : Lit.nothing}
            ${input.request.remoteAddress() ? renderGeneralRow(
        input,
        i18nString(UIStrings.remoteAddress),
        input.request.remoteAddress(),
        "remote-address"
      ) : Lit.nothing}
            ${input.request.referrerPolicy() ? renderGeneralRow(
        input,
        i18nString(UIStrings.referrerPolicy),
        String(input.request.referrerPolicy()),
        "referrer-policy"
      ) : Lit.nothing}
            </div>`
    })}
        ${!input.request?.earlyHintsHeaders || input.request.earlyHintsHeaders.length === 0 ? Lit.nothing : renderCategory({
      ...
      contents: input.showRequestHeadersText && requestHeadersText ? renderRawHeaders(requestHeadersText) : html`
...
export class RequestHeadersView extends UI.Widget.Widget {
  ...
  #addEventListeners() {
    ...
    this.#request?.addEventListener(
      SDK.NetworkRequest.Events.RESPONSE_HEADERS_CHANGED,
    ...
    Common.Settings.Settings.instance().moduleSetting("persistence-network-overrides-enabled").addChangeListener(this.requestUpdate, this);
  }
  wasShown() {
    super.wasShown();
    this.#addEventListeners();
  ...
  #removeEventListeners() {
    ...
    Common.Settings.Settings.instance().moduleSetting("persistence-network-overrides-enabled").removeChangeListener(this.requestUpdate, this);
  }
...
function renderGeneralRow(input, name, value, id, classNames) {
  const isHighlighted = input.toReveal?.section === NetworkForward.UIRequestLocation.UIHeaderSection.GENERAL && name.toLowerCase() === input.toReveal?.header?.toLowerCase();
  return html`
      ...
      <details ?open=${isOpen} @toggle=${onToggle} aria-label=${data.title}>
        ...
        >
          <div class="header-grid-container">
            <div>
              ${data.title}${data.headerCount !== void 0 ? html`<span class="header-count"> (${data.headerCount})</span>` : Lit.nothing}
            </div>
            <div class="hide-when-closed">
              ${data.checked !== void 0 ? html`
