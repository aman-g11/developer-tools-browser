    ...
    render(html`
      ...
      <div class=${rowClasses}>
        <div class=${headerNameClasses}>
          ${this.#header.headerNotSet ? html`<div class="header-badge header-badge-text">${i18n.i18n.lockedString("not-set")}</div> ` : Lit.nothing}
          ${isHeaderNameEditable && !this.#isValidHeaderName ? html`<devtools-icon class="inline-icon disallowed-characters medium" title=${UIStrings.headerNamesOnlyLetters} name='cross-circle-filled'>
            </devtools-icon>` : Lit.nothing}
          ${isHeaderNameEditable && !this.#header.isDeleted ? html`<devtools-editable-span
        ...
        ${showReloadInfoIcon ? html`<devtools-icon name="info" class="row-flex-icon flex-right medium" title=${UIStrings.reloadPrompt}>
          </devtools-icon>` : Lit.nothing}
      </div>
...
export var EditingAllowedStatus = /* @__PURE__ */ ((EditingAllowedStatus2) => {
