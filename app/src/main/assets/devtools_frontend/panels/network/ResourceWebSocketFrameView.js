...
export class ResourceWebSocketFrameView extends ResourceChunkView {
  constructor(request) {
    super(
      ...
      i18nString(UIStrings.webSocketFrame),
      i18nString(UIStrings.filterUsingRegex)
    );
...
const opCodeDescriptions = function() {
  const map = [];
  map[0 /* CONTINUATION_FRAME */] = i18nLazyString(UIStrings.continuationFrame);
  map[1 /* TEXT_FRAME */] = i18nLazyString(UIStrings.textMessage);
  map[2 /* BINARY_FRAME */] = i18nLazyString(UIStrings.binaryMessage);
  map[8 /* CONNECTION_CLOSE_FRAME */] = i18nLazyString(UIStrings.connectionCloseMessage);
  map[9 /* PING_FRAME */] = i18nLazyString(UIStrings.pingMessage);
  map[10 /* PONG_FRAME */] = i18nLazyString(UIStrings.pongMessage);
  return map;
}();
class ResourceFrameNode extends DataGridItem {
  ...
  createCells(element) {
    element.classList.toggle(
      ...
      this.frame.type === SDK.NetworkRequest.WebSocketFrameType.Error
    );
    element.classList.toggle(
      "resource-chunk-view-row-send",
      this.frame.type === SDK.NetworkRequest.WebSocketFrameType.Send
    );
    element.classList.toggle(
      "resource-chunk-view-row-receive",
      this.frame.type === SDK.NetworkRequest.WebSocketFrameType.Receive
    );
