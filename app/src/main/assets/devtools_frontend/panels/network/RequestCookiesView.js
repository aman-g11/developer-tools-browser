  ...
  getResponseCookies() {
    ...
    if (this.request.responseCookies.length) {
      ...
      for (const blockedCookie of this.request.blockedResponseCookies()) {
        ...
        if (parsedCookies && !parsedCookies.length || blockedCookie.blockedReasons.includes(Protocol.Network.SetCookieBlockedReason.SyntaxError) || blockedCookie.blockedReasons.includes(
          Protocol.Network.SetCookieBlockedReason.NameValuePairExceedsMaxSize
        )) {
