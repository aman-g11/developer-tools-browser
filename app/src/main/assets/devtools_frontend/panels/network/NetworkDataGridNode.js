...
export var RequestPanelBehavior = /* @__PURE__ */ ((RequestPanelBehavior2) => {
...
export class NetworkRequestNode extends NetworkNode {
  ...
  static ResponseHeaderStringComparator = NetworkRequestNode.HeaderStringComparator.bind(
    null,
    (req, name) => req.responseHeaderValue(name)
  );
  static RequestHeaderStringComparator = NetworkRequestNode.HeaderStringComparator.bind(
    null,
    ...
    switch (columnId) {
      ...
      case "method": {
        ...
        if (preflightRequest) {
          ...
          cell.appendChild(Components.Linkifier.Linkifier.linkifyRevealable(
            preflightRequest,
            i18nString(UIStrings.preflight),
            void 0,
      ...
      case "initiator-address-space": {
        ...
        this.renderAddressSpaceCell(
          cell,
          clientSecurityState ? clientSecurityState.initiatorIPAddressSpace : Protocol.Network.IPAddressSpace.Unknown
        );
        break;
      }
      case "size": {
    ...
    if (this.requestInternal.failed && !this.requestInternal.canceled && !this.requestInternal.wasBlocked() && !corsErrorStatus) {
      ...
      if (displayShowHeadersLink) {
        this.setTextAndTitleAsLink(
          cell,
          i18nString(UIStrings.blockeds, { PH1: reason }),
          i18nString(UIStrings.blockedTooltip),
          () => {
      ...
      this.setTextAndTitle(
        cell,
        i18nString(UIStrings.corsError),
      ...
      case SDK.NetworkRequest.InitiatorType.PREFLIGHT: {
        ...
        if (initiator.initiatorRequest) {
          ...
          const link = Components.Linkifier.Linkifier.linkifyRevealable(
            ...
            icon,
            void 0,
            i18nString(UIStrings.selectTheRequestThatTriggered),
            "trailing-link-icon",
            "initator-request"
          );
  ...
  renderTimeCell(cell) {
    ...
    if (this.requestInternal.duration > 0) {
      ...
      this.appendSubtitle(
        cell,
        i18n.TimeUtilities.secondsToString(this.requestInternal.latency),
        false,
