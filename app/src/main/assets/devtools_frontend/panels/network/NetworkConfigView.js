...
export class NetworkConfigView extends UI.Widget.VBox {
  ...
  static createUserAgentSelectAndInput(title) {
    ...
    const userAgentMetadataSetting = Common.Settings.Settings.instance().createSetting(
    ...
    userAgentSelectElement.setAttribute(
      "jslog",
      `${VisualLogging.dropDown().track({ change: true }).context(userAgentSetting.name)}`
    );
    ...
    for (const userAgentDescriptor of userAgentGroups) {
      ...
      for (const userAgentVersion of userAgentDescriptor.values) {
        ...
        groupElement.appendChild(UI.UIUtils.createOption(
          ...
          userAgentValue,
          Platform.StringUtilities.toKebabCase(userAgentVersion.title)
        ));
      }
    }
    ...
    otherUserAgentElement.setAttribute(
      "jslog",
      `${VisualLogging.textField().track({ change: true }).context(userAgentSetting.name)}`
    );
  ...
  createAcceptedEncodingSection() {
    ...
    const customAcceptedEncodingSetting = Common.Settings.Settings.instance().createSetting(
      ...
      `${Protocol.Network.ContentEncoding.Gzip},${Protocol.Network.ContentEncoding.Br},${Protocol.Network.ContentEncoding.Deflate}`
    );
    ...
    const autoCheckbox = UI.UIUtils.CheckboxLabel.create(
      ...
      true,
      void 0,
      useCustomAcceptedEncodingSetting.name
    );
    section.appendChild(autoCheckbox);
...
function getUserAgentMetadata(userAgent) {
  for (const userAgentDescriptor of userAgentGroups) {
    for (const userAgentVersion of userAgentDescriptor.values) {
      if (userAgent === SDK.NetworkManager.MultitargetNetworkManager.patchUserAgentWithChromeVersion(userAgentVersion.value)) {
