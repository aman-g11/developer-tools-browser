...
export class NetworkWaterfallColumn extends UI.Widget.VBox {
  ...
  constructor(calculator) {
    ...
    this.popoverHelper = new UI.PopoverHelper.PopoverHelper(this.element, this.getPopoverRequest.bind(this), "network.timing");
  ...
  getPopoverRequest(event) {
    ...
    const useTimingBars = !Common.Settings.Settings.instance().moduleSetting("network-color-code-resource-types").get() && !this.calculator.startAtZero;
    let range;
    let start;
    let end;
    if (useTimingBars) {
      range = NetworkTimeCalculator.calculateRequestTimeRanges(request, 0).find((data) => data.name === NetworkTimeCalculator.RequestTimeRangeNames.TOTAL);
  ...
  drawLayers(context, useTimingBars) {
    for (const entry of this.pathForStyle) {
      ...
      if (style.fillStyle) {
        context.fillStyle = useTimingBars ? ThemeSupport.ThemeSupport.instance().getComputedValue(style.fillStyle) : style.fillStyle;
        context.fill(path);
      }
      context.restore();
    }
  ...
  buildSimplifiedBarLayers(context, node, y) {
    ...
    if (!this.calculator.startAtZero) {
      const queueingRange = NetworkTimeCalculator.calculateRequestTimeRanges(request, 0).find((data) => data.name === NetworkTimeCalculator.RequestTimeRangeNames.TOTAL);
  ...
  buildTimingBarLayers(node, y) {
    ...
    for (const range of ranges) {
      if (range.name === NetworkTimeCalculator.RequestTimeRangeNames.TOTAL || range.name === NetworkTimeCalculator.RequestTimeRangeNames.SENDING || range.end - range.start === 0) {
        continue;
      }
