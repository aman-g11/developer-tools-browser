...
export class RequestInitiatorView extends UI.Widget.VBox {
  ...
  static async createStackTracePreview(request, linkifier, focusableLink) {
    ...
    if (target) {
      stackTrace = await Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().createStackTraceFromProtocolRuntime(initiator.stack, target);
  ...
  async performUpdate() {
    ...
    let stackTrace = null;
    if (rawStack && target) {
      stackTrace = await Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().createStackTraceFromProtocolRuntime(rawStack, target);
    }
    const viewInput = {
      initiatorGraph,
      stackTrace,
