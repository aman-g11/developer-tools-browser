...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  const createParsedParams = (params) => params.map(
    (param) => html`<li role=treeitem @contextmenu=${copyValueContextmenu(i18nString(UIStrings.copyValue), () => decodeURIComponent(param.value), "copy-value")}>${param.name !== "" ? html`${RequestPayloadView.formatParameter(param.name, "payload-name", input.decodeRequestParameters)}${RequestPayloadView.formatParameter(
      param.value,
      "payload-value source-code",
      input.decodeRequestParameters
  ...
  const createPayload = (parsedFormData2) => {
    const object = new SDK.RemoteObject.LocalJSONObject(parsedFormData2);
  ...
  const onContextMenu = (viewSource, callback, includeURLDecodingOption = true) => (event) => {
    ...
    if (viewSource) {
      ...
      if (includeURLDecodingOption) {
        ...
        section.appendItem(
          viewURLEncodedText,
          toggleURLDecoding.bind(void 0, event),
...
export class RequestPayloadView extends UI.Widget.VBox {
  ...
  static formatParameter(value, className, decodeParameters) {
    ...
    return html`<div class=${classes}>
      ${errorDecoding ? html`<span class=payload-decode-error>${i18nString(UIStrings.unableToDecodeValue)}</span>` : value}
    </div>`;
  }
}
