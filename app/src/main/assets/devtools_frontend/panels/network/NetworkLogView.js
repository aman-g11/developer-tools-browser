...
export class NetworkLogView extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(filterBar, progressBarContainer, networkLogLargeRowsSetting) {
    ...
    this.networkHideChromeExtensions = Common.Settings.Settings.instance().createSetting("network-hide-chrome-extensions", false);
    this.networkShowBlockedCookiesOnlySetting = Common.Settings.Settings.instance().createSetting("network-show-blocked-cookies-only-setting", false);
    this.networkOnlyBlockedRequestsSetting = Common.Settings.Settings.instance().createSetting("network-only-blocked-requests", false);
    this.networkOnlyThirdPartySetting = Common.Settings.Settings.instance().createSetting("network-only-third-party-setting", false);
    this.networkResourceTypeFiltersSetting = Common.Settings.Settings.instance().createSetting("network-resource-type-filters", {});
    ...
    this.invertFilterUI = new UI.FilterBar.CheckboxFilterUI(
      i18nString(UIStrings.invertFilter),
      true,
    ...
    new UI.DropTarget.DropTarget(
      this.element,
      [UI.DropTarget.Type.File],
      i18nString(UIStrings.dropHarFilesHere),
      this.handleDrop.bind(this)
    );
    Common.Settings.Settings.instance().moduleSetting("network-color-code-resource-types").addChangeListener(this.invalidateAllItems.bind(this, false), this);
    ...
    Logs.NetworkLog.NetworkLog.instance().addEventListener(
      Logs.NetworkLog.Events.RequestAdded,
      this.onRequestUpdated,
      this
    );
    Logs.NetworkLog.NetworkLog.instance().addEventListener(
      Logs.NetworkLog.Events.RequestUpdated,
      this.onRequestUpdated,
      this
    );
    Logs.NetworkLog.NetworkLog.instance().addEventListener(
      Logs.NetworkLog.Events.RequestRemoved,
  ...
  resetSuggestionBuilder() {
    ...
    this.suggestionBuilder.addItem(
      NetworkForward.UIFilter.FilterType.Is,
  ...
  showRecordingHint() {
    ...
    const header = this.recording ? i18nString(UIStrings.recordingNetworkActivity) : i18nString(UIStrings.noNetworkActivityRecorded);
    ...
    this.dataGrid.element.addEventListener("keydown", (event) => {
      ...
      if (Platform.KeyboardUtilities.isEnterOrSpaceKey(event)) {
        this.dispatchEventToListeners(
          Events.RequestActivated,
          { showPanel: RequestPanelBehavior.ShowPanel, takeFocus: true }
        );
        event.consume(true);
      }
    });
  ...
  updateSummaryBar() {
    ...
    for (const request of Logs.NetworkLog.NetworkLog.instance().requests()) {
      ...
      if (networkManager && request.url() === networkManager.target().inspectedURL() && request.resourceType() === Common.ResourceType.resourceTypes.Document && networkManager.target().parentTarget()?.type() !== SDK.Target.Type.FRAME) {
  ...
  refreshRequest(request) {
    NetworkLogView.subdomains(request.domain).forEach(
    ...
    if (priority) {
      this.suggestionBuilder.addItem(
        NetworkForward.UIFilter.FilterType.Priority,
    ...
    if (request.mixedContentType !== Protocol.Security.MixedContentType.None) {
      this.suggestionBuilder.addItem(
        NetworkForward.UIFilter.FilterType.MixedContent,
    ...
    if (request.mixedContentType === Protocol.Security.MixedContentType.Blockable) {
      const suggestion = request.wasBlocked() ? NetworkForward.UIFilter.MixedContentFilterValues.BLOCKED : NetworkForward.UIFilter.MixedContentFilterValues.BLOCK_OVERRIDDEN;
  ...
  handleContextMenuForRequest(contextMenu, request) {
    ...
    if (request) {
      ...
      if (UI.ActionRegistry.ActionRegistry.instance().hasAction(openAiAssistanceId)) {
        ...
        appendSubmenuPromptAction2(
          submenu,
          action,
          i18nString(UIStrings.explainPurpose),
        ...
        appendSubmenuPromptAction2(
          submenu,
          action,
          i18nString(UIStrings.explainSlowness),
        ...
        appendSubmenuPromptAction2(
          submenu,
          action,
          i18nString(UIStrings.explainFailures),
        ...
        appendSubmenuPromptAction2(
          submenu,
          action,
          i18nString(UIStrings.assessSecurityHeaders),
      ...
      if (request.requestHeadersText()) {
        copyMenu.saveSection().appendItem(
          i18nString(UIStrings.copyRequestHeaders),
          NetworkLogView.copyRequestHeaders.bind(null, request),
          { jslogContext: "copy-request-headers" }
        );
      }
      if (request.responseHeadersText) {
        copyMenu.saveSection().appendItem(
          i18nString(UIStrings.copyResponseHeaders),
          NetworkLogView.copyResponseHeaders.bind(null, request),
          { jslogContext: "copy-response-headers" }
        );
      }
      if (request.finished) {
        copyMenu.saveSection().appendItem(
          i18nString(UIStrings.copyResponse),
          NetworkLogView.copyResponse.bind(null, request),
    ...
    if (this.networkShowOptionsToGenerateHarWithSensitiveData.get()) {
      copyMenu.footerSection().appendItem(
        filtered ? i18nString(UIStrings.copyAllListedAsHarWithSensitiveData) : i18nString(UIStrings.copyAllAsHarWithSensitiveData),
    ...
    contextMenu.overrideSection().appendItem(
      i18nString(UIStrings.overrideHeaders),
    ...
    if (request) {
      ...
      }, addRequestCondition2 = function(pattern, conditions) {
        const entry = manager.requestConditions.findCondition(pattern.constructorString);
        if (entry) {
      ...
      var removeRequestCondition = removeRequestCondition2, addRequestCondition = addRequestCondition2;
      const maxBlockedURLLength = 20;
      const manager = SDK.NetworkManager.MultitargetNetworkManager.instance();
      const blockingMenu = contextMenu.debugSection().appendSubMenuItem(
        i18nString(UIStrings.blockRequests),
        /* disabled=*/
        true
      );
      const throttlingMenu = contextMenu.debugSection().appendSubMenuItem(
        i18nString(UIStrings.throttleRequests),
        /* disabled=*/
        true
      );
      const urlWithoutScheme = request.parsedURL.urlWithoutScheme();
      const urlPattern = urlWithoutScheme && SDK.NetworkManager.RequestURLPattern.create(
        `*://${urlWithoutScheme}`
      );
      if (urlPattern) {
        ...
        const isThrottling = existingConditions && existingConditions.conditions !== SDK.NetworkManager.BlockingConditions && existingConditions.conditions !== SDK.NetworkManager.NoThrottlingConditions;
        ...
        blockingMenu.debugSection().appendItem(
          ...
          () => isBlocking ? removeRequestCondition2(urlPattern) : addRequestCondition2(urlPattern, SDK.NetworkManager.BlockingConditions),
          { jslogContext: "block-request-url" }
        );
        throttlingMenu.debugSection().appendItem(
          isThrottling ? i18nString(UIStrings.unthrottleS, { PH1: croppedURL }) : i18nString(UIStrings.throttleRequestUrl),
          () => isThrottling ? removeRequestCondition2(urlPattern) : addRequestCondition2(urlPattern, SDK.NetworkManager.Slow3GConditions),
          { jslogContext: "throttle-request-url" }
        );
      }
      const domain = request.parsedURL.domain();
      const domainPattern = domain && SDK.NetworkManager.RequestURLPattern.create(
        `*://${domain}`
      );
      if (domainPattern) {
        throttlingMenu.setEnabled(true);
        ...
        blockingMenu.debugSection().appendItem(
          ...
          () => isBlocking ? removeRequestCondition2(domainPattern) : addRequestCondition2(domainPattern, SDK.NetworkManager.BlockingConditions),
          { jslogContext: "block-request-domain" }
        );
        throttlingMenu.debugSection().appendItem(
          isThrottling ? i18nString(UIStrings.unthrottleS, { PH1: croppedURL }) : i18nString(UIStrings.throttleRequestDomain),
          () => isThrottling ? removeRequestCondition2(domainPattern) : addRequestCondition2(domainPattern, SDK.NetworkManager.Slow3GConditions),
    ...
    this.filters = descriptors.map((descriptor) => {
      ...
      if (key) {
        ...
        filter = NetworkLogView.requestPathFilter.bind(
          null,
          new RegExp(Platform.StringUtilities.escapeForRegExp(text), "i")
        );
      }
    ...
    switch (type) {
      ...
      case NetworkForward.UIFilter.FilterType.Priority:
        return NetworkLogView.requestPriorityFilter.bind(
          null,
          PerfUI.NetworkPriorities.uiLabelToNetworkPriority(value)
        );
...
export class MoreFiltersDropDownUI extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    super();
    this.networkHideDataURLSetting = Common.Settings.Settings.instance().createSetting("network-hide-data-url", false);
    this.networkHideChromeExtensionsSetting = Common.Settings.Settings.instance().createSetting("network-hide-chrome-extensions", false);
    this.networkShowBlockedCookiesOnlySetting = Common.Settings.Settings.instance().createSetting("network-show-blocked-cookies-only-setting", false);
    this.networkOnlyBlockedRequestsSetting = Common.Settings.Settings.instance().createSetting("network-only-blocked-requests", false);
    this.networkOnlyThirdPartySetting = Common.Settings.Settings.instance().createSetting("network-only-third-party-setting", false);
