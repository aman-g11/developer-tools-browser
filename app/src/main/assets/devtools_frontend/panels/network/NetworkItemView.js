...
const requestToResponseView = /* @__PURE__ */ new WeakMap();
const requestToPreviewView = /* @__PURE__ */ new WeakMap();
export class NetworkItemView extends UI.TabbedPane.TabbedPane {
  ...
  constructor(request, calculator, initialTab) {
    ...
    if (request.resourceType() === Common.ResourceType.resourceTypes.DirectSocket) {
      ...
      this.appendTab(
        NetworkForward.UIRequestLocation.UIRequestTabs.DIRECT_SOCKET_CONNECTION,
        i18nString(UIStrings.connectionInfo),
    ...
    if (request.resourceType() === Common.ResourceType.resourceTypes.WebSocket) {
      ...
      this.appendTab(
        NetworkForward.UIRequestLocation.UIRequestTabs.WS_FRAMES,
        i18nString(UIStrings.messages),
        frameView,
        i18nString(UIStrings.websocketMessages)
      );
    } else if (request.resourceType() === Common.ResourceType.resourceTypes.DirectSocket) {
      this.appendTab(
        NetworkForward.UIRequestLocation.UIRequestTabs.DIRECT_SOCKET_CHUNKS,
        i18nString(UIStrings.messages),
        new ResourceDirectSocketChunkView(request),
        i18nString(UIStrings.directsocketMessages)
      );
    } else if (request.mimeType === Platform.MimeType.MimeType.EVENTSTREAM) {
      this.appendTab(
        NetworkForward.UIRequestLocation.UIRequestTabs.EVENT_SOURCE,
        i18nString(UIStrings.eventstream),
    ...
    this.appendTab(
      NetworkForward.UIRequestLocation.UIRequestTabs.INITIATOR,
    ...
    if (request.trustTokenParams()) {
      ...
      this.appendTab(
        NetworkForward.UIRequestLocation.UIRequestTabs.TRUST_TOKENS,
  ...
  wasShown() {
    ...
    this.#request.addEventListener(
      SDK.NetworkRequest.Events.TRUST_TOKEN_RESULT_ADDED,
  ...
  async maybeAppendPayloadPanel() {
    ...
    if (this.#request.queryParameters || await this.#request.requestFormData()) {
      ...
      this.appendTab(
        NetworkForward.UIRequestLocation.UIRequestTabs.PAYLOAD,
