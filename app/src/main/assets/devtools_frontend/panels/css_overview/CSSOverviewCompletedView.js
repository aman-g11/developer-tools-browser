...
function renderUnusedDeclarations(unusedDeclarations) {
  return unusedDeclarations.length > 0 ? renderGroup(unusedDeclarations, "unused-declarations") : html`<div class="horizontally-padded">${i18nString(UIStrings.thereAreNoUnusedDeclarations)}</div>`;
}
function renderMediaQueries(mediaQueries) {
  return mediaQueries.length > 0 ? renderGroup(mediaQueries, "media-queries") : html`<div class="horizontally-padded">${i18nString(UIStrings.thereAreNoMediaQueries)}</div>`;
}
function renderFontInfo(fonts) {
  return fonts.length > 0 ? html`${fonts.map(({ font, fontMetrics }) => html`
    ...
    </section>`)}` : html`<div>${i18nString(UIStrings.thereAreNoFonts)}</div>`;
}
  ...
  return html`
    <td>
      <div class="contrast-container-in-grid">
          ...
          ${showAPCA ? html`
            <span>${i18nString(UIStrings.apca)}</span>${data.thresholdsViolated.apca ? createClearIcon() : createCheckIcon()}` : html`
