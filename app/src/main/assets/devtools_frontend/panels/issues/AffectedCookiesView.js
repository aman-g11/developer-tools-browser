...
export class AffectedCookiesView extends AffectedResourcesView {
  ...
  #appendAffectedCookies(cookies) {
    ...
    this.appendColumnTitle(
      header,
      i18nString(UIStrings.domain) + " & " + i18nString(UIStrings.path),
  ...
  #appendAffectedCookie(cookie, hasAssociatedRequest) {
    ...
    if (hasAssociatedRequest) {
      ...
      link.setAttribute(
        "jslog",
        `${VisualLogging.link("issues.filter-network-requests-by-cookie").track({ click: true })}`
      );
  ...
  update() {
    ...
    for (const cookie of cookieLinesWithRequestIndicator) {
      ...
      if (cookie.hasRequest) {
        ...
        link.setAttribute(
          "jslog",
          `${VisualLogging.link("issues.filter-network-requests-by-raw-cookie").track({ click: true })}`
        );
        link.addEventListener("click", () => {
          void Common.Revealer.reveal(NetworkForward.UIFilter.UIRequestFilter.filters([
            {
