...
export class AffectedSelectivePermissionsInterventionView extends AffectedResourcesView {
  ...
  #renderDetail(issue) {
    ...
    const stackTracePromise = details.stackTrace && issuesModel ? this.#resolveStackTrace(details.stackTrace, issuesModel) : Promise.resolve(html`<span>${i18nString(UIStrings.unknown)}</span>`);
