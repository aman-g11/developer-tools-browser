...
class IssueCategoryView extends UI.TreeOutline.TreeElement {
  ...
  constructor(category) {
    super(void 0, void 0, Platform.StringUtilities.toKebabCase(category));
...
export class IssuesPane extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.#aggregator.addEventListener(
      IssuesManager.IssueAggregator.Events.AGGREGATED_ISSUE_UPDATED,
      this.#issueUpdated,
      this
    );
    this.#aggregator.addEventListener(
      IssuesManager.IssueAggregator.Events.FULL_UPDATE_REQUIRED,
      this.#onFullUpdate,
      this
    );
  ...
  #createToolbars() {
    ...
    const groupByCategoryCheckbox = new UI.Toolbar.ToolbarSettingCheckbox(
      ...
      i18nString(UIStrings.groupDisplayedIssuesUnder),
      i18nString(UIStrings.groupByCategory)
    );
    ...
    const groupByKindSettingCheckbox = new UI.Toolbar.ToolbarSettingCheckbox(
      groupByKindSetting,
      i18nString(UIStrings.groupDisplayedIssuesUnderKind),
      i18nString(UIStrings.groupByKind)
    );
    ...
    this.#showThirdPartyCheckbox = new UI.Toolbar.ToolbarSettingCheckbox(
      thirdPartySetting,
      i18nString(UIStrings.includeCookieIssuesCausedBy),
  ...
  #updateItemPositionAndSize(parent) {
    ...
    for (let i = 0; i < childNodes.length; i++) {
      ...
      if (node.classList.contains("issue")) {
        ...
        UI.ARIAUtils.setSetSize(node, childNodes.length / 2);
      }
    }
  }
  ...
  #showIssuesTreeOrNoIssuesDetectedMessage(issuesCount, hiddenIssueCount) {
    if (issuesCount > 0 || hiddenIssueCount > 0) {
      ...
      this.#noIssuesMessageDiv.header = hasOnlyThirdPartyIssues ? i18nString(UIStrings.onlyThirdpartyCookieIssues) : i18nString(UIStrings.noIssues);
