...
class AffectedRequestsView extends AffectedResourcesView {
  #appendAffectedRequests(affectedRequests) {
    ...
    for (const affectedRequest of affectedRequests) {
      ...
      const tab = issueTypeToNetworkHeaderMap.get(category) || NetworkForward.UIRequestLocation.UIRequestTabs.HEADERS_COMPONENT;
...
class AffectedMixedContentView extends AffectedResourcesView {
  ...
  appendAffectedMixedContent(mixedContent) {
    ...
    if (mixedContent.request) {
      const networkTab = issueTypeToNetworkHeaderMap.get(this.issue.getCategory()) || NetworkForward.UIRequestLocation.UIRequestTabs.HEADERS_COMPONENT;
    ...
    this.appendIssueDetailCell(
      element,
      AffectedMixedContentView.translateStatus(mixedContent.resolutionStatus),
...
export class IssueView extends UI.TreeOutline.TreeElement {
  ...
  constructor(issue, description) {
    super(void 0, void 0, Platform.StringUtilities.toKebabCase(issue.getCategory()));
    this.#issue = issue;
  ...
  #createReadMoreLinks() {
    ...
    for (const description of this.#description.links) {
      ...
      render(
        html`<devtools-link class="link devtools-link" href=${description.link} jslogcontext="learn-more">${i18nString(UIStrings.learnMoreS, { PH1: description.linkTitle })}</devtools-link>`,
        linkListItem
      );
    }
