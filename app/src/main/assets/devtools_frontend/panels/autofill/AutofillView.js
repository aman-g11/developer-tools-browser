    ...
    return html`
      ...
      (field, index) => html`
        ...
        "background-color": highlightedGridRows.has(index) ? "var(--sys-color-state-hover-on-subtle)" : null
                  ...
                  <td>
                      ${field.autofillType}
                      ${field.fillingStrategy === FillingStrategy.AutocompleteAttribute ? html`<devtools-adorner .name=${field.fillingStrategy} title=${i18nString(UIStrings.autocompleteAttribute)}>
                              <span>${i18nString(UIStrings.attr)}</span>
                            </devtools-adorner>` : field.fillingStrategy === FillingStrategy.AutofillInferred ? html`<devtools-adorner .name=${field.fillingStrategy} title=${i18nString(UIStrings.inferredByHeuristics)}>
