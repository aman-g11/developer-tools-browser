  ...
  render(
    html`
      ...
      <div class="dom-breakpoints-container" jslog=${VisualLogging.section("sources.dom-breakpoints").track({ resize: true })}>
        ${hasBreakpoints ? html`<ul class="breakpoint-list"
      ...
      const linkified = PanelsCommon.DOMLinkifier.Linkifier.instance().linkify(
        breakpoint.node,
        { preventKeyboardFocus: true, tooltip: void 0 }
      );
      return html`
                <li class=${`breakpoint-entry ${item.isHighlighted ? "breakpoint-hit" : ""}`}
                  ...
                  <devtools-checkbox
                    ...
                    aria-description=${Directives.ifDefined(item.isHighlighted ? i18nString(UIStrings.breakpointHit) : void 0)}
...
export class DOMBreakpointsSidebarPane extends UI.Widget.VBox {
  ...
  constructor(view = DEFAULT_VIEW) {
    ...
    SDK.TargetManager.TargetManager.instance().addModelListener(
      ...
      SDK.DOMDebuggerModel.Events.DOM_BREAKPOINT_ADDED,
      this.breakpointAdded,
      this
    );
    SDK.TargetManager.TargetManager.instance().addModelListener(
      SDK.DOMDebuggerModel.DOMDebuggerModel,
      SDK.DOMDebuggerModel.Events.DOM_BREAKPOINT_TOGGLED,
      this.breakpointToggled,
      this
    );
    SDK.TargetManager.TargetManager.instance().addModelListener(
      SDK.DOMDebuggerModel.DOMDebuggerModel,
      SDK.DOMDebuggerModel.Events.DOM_BREAKPOINTS_REMOVED,
      this.breakpointsRemoved,
      this
    );
  ...
  onBreakpointContextMenu(breakpoint, event) {
    ...
    contextMenu.defaultSection().appendItem(
      i18nString(UIStrings.revealDomNodeInElementsPanel),
      () => Common.Revealer.reveal(breakpoint.node),
    ...
    for (const type of Object.values(allBreakpointTypes)) {
      ...
      if (label) {
        breakpointsMenu.defaultSection().appendCheckboxItem(
          label(),
          toggleBreakpoint.bind(null, type),
