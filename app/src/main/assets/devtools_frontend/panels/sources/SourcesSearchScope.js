...
export class SourcesSearchScope {
  ...
  projects() {
    ...
    return Workspace.Workspace.WorkspaceImpl.instance().projects().filter((project) => {
      ...
      if (!localOverridesEnabled && project.type() === Workspace.Workspace.projectTypes.FileSystem && Persistence.FileSystemWorkspaceBinding.FileSystemWorkspaceBinding.fileSystemType(project) === "overrides") {
        return false;
      }
      return true;
    });
  ...
  processMatchingFilesForProject(searchId, project, searchConfig, filesMatchingFileQuery, filesWithPreliminaryResult) {
    ...
    this.searchResultCandidates = Platform.ArrayUtilities.mergeOrdered(
      this.searchResultCandidates,
      uiSourceCodes,
      SourcesSearchScope.filesComparator
    );
  }
      ...
      if (uiSourceCode.isDirty()) {
        ...
        void uiSourceCode.requestContentData().then((contentData) => {
          contentLoaded.call(
            this,
            uiSourceCode,
            TextUtils.ContentData.ContentData.contentDataOrEmpty(contentData).textObj
          );
        });
      }
    }
