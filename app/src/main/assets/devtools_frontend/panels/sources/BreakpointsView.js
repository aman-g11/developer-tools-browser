...
export class BreakpointsSidebarController {
  ...
  #getBreakpointState(locations) {
    ...
    let status;
    if (hasEnabled) {
      status = hasDisabled ? "INDETERMINATE" /* INDETERMINATE */ : "ENABLED" /* ENABLED */;
    } else {
  ...
  render(html`
    ...
    <div jslog=${VisualLogging.section("sources.js-breakpoints")} id="devtools-breakpoint-view">
      ...
      <div class='pause-on-caught-exceptions'
            ...
            data-last-pause>
          <label class='checkbox-label'>
            <input data-pause-on-caught-checkbox type='checkbox' class="small" tabindex=-1 ?checked=${input.pauseOnCaughtExceptions} @change=${input.onPauseOnCaughtExceptionsStateChanged.bind(void 0)} jslog=${VisualLogging.toggle("pause-on-caught-exception").track({ change: true })}>
...
export class BreakpointsView extends UI.Widget.VBox {
  ...
  static instance({ forceNew } = { forceNew: false }) {
    if (!breakpointsViewInstance || forceNew) {
      breakpointsViewInstance = new BreakpointsView(void 0);
    }
  ...
  #onBreakpointEntryContextMenu(event, breakpointItem, editable) {
    ...
    const menu = new UI.ContextMenu.ContextMenu(event);
    const editBreakpointText = breakpointItem.type === SDK.DebuggerModel.BreakpointType.LOGPOINT ? i18nString(UIStrings.editLogpoint) : i18nString(UIStrings.editCondition);
