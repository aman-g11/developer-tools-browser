...
const TYPE_ORDERS = /* @__PURE__ */ new Map([
  [Types.Root, 1],
  [Types.Authored, 1],
  [Types.Deployed, 5],
  [Types.Domain, 10],
...
export class NavigatorView extends UI.Widget.VBox {
  ...
  constructor(jslogContext, enableAuthoredGrouping) {
    ...
    Bindings.NetworkProject.NetworkProjectManager.instance().addEventListener(
      Bindings.NetworkProject.Events.FRAME_ATTRIBUTION_ADDED,
      this.frameAttributionAdded,
      this
    );
    Bindings.NetworkProject.NetworkProjectManager.instance().addEventListener(
      Bindings.NetworkProject.Events.FRAME_ATTRIBUTION_REMOVED,
  ...
  static appendSearchItem(contextMenu, path) {
    ...
    contextMenu.viewSection().appendItem(
      searchLabel,
      () => Common.Revealer.reveal(searchSources),
  ...
  addUISourceCode(uiSourceCode) {
    if (Root.Runtime.experiments.isEnabled(Root.ExperimentNames.ExperimentName.JUST_MY_CODE) && Workspace.IgnoreListManager.IgnoreListManager.instance().isUserOrSourceMapIgnoreListedUISourceCode(
      uiSourceCode
    )) {
      return;
    }
  ...
  projectAdded(project) {
    ...
    const type = project instanceof Persistence.AutomaticFileSystemWorkspaceBinding.FileSystem || project instanceof Persistence.FileSystemWorkspaceBinding.FileSystem && project.fileSystem().automatic ? Types.AutomaticFileSystem : Types.FileSystem;
  ...
  removeProject(project) {
    ...
    if (project.type() !== Workspace.Workspace.projectTypes.ConnectableFileSystem && project.type() !== Workspace.Workspace.projectTypes.FileSystem) {
      return;
    }
  ...
  targetNode(project, target, isAuthored) {
    ...
    if (!targetNode) {
      targetNode = new NavigatorGroupTreeNode(
        ...
        project,
        "target:" + target.id(),
        target.type() === SDK.Target.Type.FRAME ? Types.Frame : Types.Worker,
        target.name()
      );
  ...
  removeUISourceCodeNode(node) {
    ...
    while (currentNode) {
      ...
      if ((parentNode === this.rootNode || parentNode === this.deployedNode) && project.type() === Workspace.Workspace.projectTypes.FileSystem) {
        break;
      }
      ...
      if (currentNode.type === Types.Frame) {
        ...
        const folderId = this.folderNodeId(
          ...
          currentNode instanceof NavigatorFolderTreeNode && currentNode.folderPath || Platform.DevToolsPath.EmptyEncodedPathString
        );
  ...
  async handleContextMenuExclude(project, path) {
    const shouldExclude = await UI.UIUtils.ConfirmDialog.show(
      i18nString(UIStrings.folderWillNotBeShown),
      i18nString(UIStrings.excludeThisFolder),
      void 0,
  ...
  async handleContextMenuDelete(uiSourceCode) {
    const shouldDelete = await UI.UIUtils.ConfirmDialog.show(
      ...
      i18nString(UIStrings.deleteThisFile),
      void 0,
  ...
  async handleDeleteFolder(node) {
    const shouldRemove = await UI.UIUtils.ConfirmDialog.show(
      i18nString(UIStrings.actionCannotBeUndone),
      i18nString(UIStrings.deleteFolder),
      void 0,
  ...
  async deleteDirectoryRecursively(node) {
    ...
    await Persistence.NetworkPersistenceManager.NetworkPersistenceManager.instance().project()?.deleteDirectoryRecursively(node.folderPath);
  }
  ...
  handleFolderContextMenu(event, node) {
    ...
    if (project.type() === Workspace.Workspace.projectTypes.FileSystem) {
      ...
      for (const { text, callback, jslogContext } of Workspace.IgnoreListManager.IgnoreListManager.instance().getIgnoreListFolderContextMenuItems(url, options)) {
        contextMenu.defaultSection().appendItem(text, callback, { jslogContext });
      }
    }
    if (project.canExcludeFolder(path)) {
      contextMenu.defaultSection().appendItem(
        i18nString(UIStrings.excludeFolder),
    ...
    if (project.type() === Workspace.Workspace.projectTypes.FileSystem) {
      if (Persistence.FileSystemWorkspaceBinding.FileSystemWorkspaceBinding.fileSystemType(project) !== Persistence.PlatformFileSystem.PlatformFileSystemType.OVERRIDES) {
        if (node instanceof NavigatorGroupTreeNode) {
          contextMenu.defaultSection().appendItem(i18nString(UIStrings.removeFolderFromWorkspace), async () => {
            ...
            const shouldRemove = await UI.UIUtils.ConfirmDialog.show(i18nString(UIStrings.workspaceStopSyncing), header, void 0, {
...
const boostOrderForNode = /* @__PURE__ */ new WeakSet();
...
export class NavigatorUISourceCodeTreeNode extends NavigatorTreeNode {
  ...
  constructor(navigatorView, uiSourceCode, frame) {
    ...
    if (uiSourceCode.contentType().isScript()) {
      ...
      this.recursiveProperties.exclusivelyContentScripts = uiSourceCode.project().type() === Workspace.Workspace.projectTypes.ContentScripts;
    }
  }
  frame() {
    return this.#frame;
  ...
  updateTitle(ignoreIsDirty) {
    const isIgnoreListed = Workspace.IgnoreListManager.IgnoreListManager.instance().isUserOrSourceMapIgnoreListedUISourceCode(
      this.#uiSourceCode
    );
  ...
  rename(callback) {
    ...
    this.treeElement.startEditingTitle(
      new UI.InplaceEditor.Config(commitHandler, () => afterEditing(false), void 0)
    );
  }
}
