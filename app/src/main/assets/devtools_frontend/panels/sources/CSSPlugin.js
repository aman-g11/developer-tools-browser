...
const doNotCompleteIn = /* @__PURE__ */ new Set(["ColorLiteral", "NumberLiteral", "StringLiteral", "Comment", "Important"]);
  ...
  findColorsAndCurves(
    ...
    (pos, parsedColor, colorText) => {
      builder.add(
        pos,
        pos,
        CodeMirror.Decoration.widget({ widget: new ColorSwatchWidget(parsedColor, colorText, pos) })
      );
    },
    (pos, curve, text) => {
  ...
  populateTextAreaContextMenu(contextMenu) {
    function addSourceMapURL(cssModel2, sourceUrl) {
      const dialog = AddDebugInfoURLDialog.createAddSourceMapURLDialog((sourceMapUrl) => {
        Bindings.CSSWorkspaceBinding.CSSWorkspaceBinding.instance().modelToInfo.get(cssModel2)?.addSourceMap(
          sourceUrl,
          sourceMapUrl
        );
      });
    ...
    if (this.uiSourceCode.project().type() === Workspace.Workspace.projectTypes.Network && cssModel && !Workspace.IgnoreListManager.IgnoreListManager.instance().isUserIgnoreListedURL(url)) {
