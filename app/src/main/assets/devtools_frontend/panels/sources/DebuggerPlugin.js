...
const MAX_CODE_SIZE_FOR_VALUE_DECORATIONS = 1e4;
const MAX_PROPERTIES_IN_SCOPE_FOR_VALUE_DECORATIONS = 500;
const debuggerPluginForUISourceCode = /* @__PURE__ */ new Map();
export class DebuggerPlugin extends Plugin {
  constructor(uiSourceCode, transformer) {
    ...
    this.loader.addEventListener(
      SDK.PageResourceLoader.Events.UPDATE,
  ...
  editorInitialized(editor) {
    ...
    this.popoverHelper = new UI.PopoverHelper.PopoverHelper(editor, this.getPopoverRequest.bind(this), "sources.object-properties");
  ...
  showIgnoreListInfobarIfNeeded() {
    ...
    const infobar = new UI.Infobar.Infobar(
      ...
      i18nString(UIStrings.thisScriptIsOnTheDebuggersIgnore),
      [
        {
  ...
  populateLineGutterContextMenu(contextMenu, editorLineNumber) {
    ...
    const supportsConditionalBreakpoints = Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().supportsConditionalBreakpoints(
      this.uiSourceCode
    );
  ...
  populateTextAreaContextMenu(contextMenu) {
    ...
    if (this.uiSourceCode.project().type() === Workspace.Workspace.projectTypes.Network && Common.Settings.Settings.instance().moduleSetting("js-source-maps-enabled").get() && !Workspace.IgnoreListManager.IgnoreListManager.instance().isUserIgnoreListedURL(this.uiSourceCode.url())) {
      if (this.scriptFileForDebuggerModel.size) {
        ...
        if (scriptFile.script?.isWasm() && !Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().pluginManager.hasPluginForScript(
          scriptFile.script
        )) {
          contextMenu.debugSection().appendItem(
            i18nString(UIStrings.addWasmDebugInfo),
  ...
  editBreakpointCondition(breakpointEditRequest) {
    ...
    dialog.oldCondition = oldCondition, dialog.breakpointType = isLogpointForDialog ? SDK.DebuggerModel.BreakpointType.LOGPOINT : SDK.DebuggerModel.BreakpointType.CONDITIONAL_BREAKPOINT;
    ...
    editor.dispatch({
      effects: CodeMirror.StateEffect.appendConfig.of(compartment.of(CodeMirror.EditorView.decorations.of(
        CodeMirror.Decoration.set([CodeMirror.Decoration.widget({
          block: true,
  ...
  async computeValueDecorations() {
    ...
    for (const [line, names] of variablesByLine) {
      ...
      decorations.push(CodeMirror.Decoration.widget({ widget: new ValueDecoration(newNames), side: 1 }).range(this.editor.state.doc.line(line + 1).to));
    }
  ...
  onInlineBreakpointMarkerContextMenu(event, breakpoint) {
    ...
    if (!SourceFrame.SourceFrame.isBreakableLine(editor.state, line) || // Editing breakpoints only make sense for conditional breakpoints
    // and logpoints.
  ...
  updateScriptFile(debuggerModel) {
    ...
    if (oldScriptFile) {
      oldScriptFile.removeEventListener(
        Bindings.ResourceScriptMapping.ResourceScriptFile.Events.DID_MERGE_TO_VM,
        this.didMergeToVM,
        this
      );
      oldScriptFile.removeEventListener(
        Bindings.ResourceScriptMapping.ResourceScriptFile.Events.DID_DIVERGE_FROM_VM,
        this.didDivergeFromVM,
        this
      );
  ...
  updateMissingDebugInfoInfobar(warning) {
    ...
    this.missingDebugInfoBar = UI.Infobar.Infobar.create(UI.Infobar.Type.ERROR, warning.details, [], void 0, "missing-debug-info");
    if (!this.missingDebugInfoBar) {
      return;
    }
  ...
  showSourceMapInfobarIfNeeded() {
    ...
    if (!resource) {
      this.sourceMapInfobar = UI.Infobar.Infobar.create(
        ...
        i18nString(UIStrings.sourceMapSkipped),
        [],
      ...
      this.sourceMapInfobar = UI.Infobar.Infobar.create(
        UI.Infobar.Type.INFO,
        i18nString(UIStrings.sourceMapLoaded),
        [],
      ...
      this.sourceMapInfobar = UI.Infobar.Infobar.create(
        UI.Infobar.Type.WARNING,
        i18nString(UIStrings.sourceMapFailed),
        [],
        void 0,
        "source-map-failed"
      );
...
export function computePopoverHighlightRange(state, mimeType, cursorPos) {
  ...
  const tree = CodeMirror.ensureSyntaxTree(state, cursorPos, 5 * 1e3);
  if (!tree) {
    return null;
  }
...
function inlineBreakpointArrow(color, outline, opacity = "1") {
  return `url('data:image/svg+xml,<svg width="11" height="12" viewBox="0 0 11 12" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0.5 0.5H5.80139C6.29382 0.5 6.7549 0.741701 7.03503 1.14669L10.392 6L7.03503 10.8533C6.7549 11.2583 6.29382 11.5 5.80139 11.5H0.5V0.5Z" fill="${encodeURIComponent(
    color
  )}" stroke="${encodeURIComponent(outline)}" fill-opacity="${encodeURIComponent(opacity)}"/></svg>')`;
}
...
function inlineLogpointArrow(color, outline, opacity = "1") {
  return `url('data:image/svg+xml,<svg width="11" height="12" viewBox="0 0 11 12" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0.5 0.5H5.80139C6.29382 0.5 6.7549 0.741701 7.03503 1.14669L10.392 6L7.03503 10.8533C6.7549 11.2583 6.29382 11.5 5.80139 11.5H0.5V0.5Z" fill="${encodeURIComponent(color)}" stroke="${encodeURIComponent(outline)}" fill-opacity="${encodeURIComponent(
    opacity
