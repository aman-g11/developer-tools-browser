...
export class WatchExpressionsSidebarPane extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.watchExpressionsSetting = Common.Settings.Settings.instance().createLocalSetting("watch-expressions", []);
    this.addButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.addWatchExpression),
      "plus",
      void 0,
    ...
    this.refreshButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.refreshWatchExpressions),
      "refresh",
      void 0,
  ...
  createWatchExpression(expression) {
    ...
    watchExpression.addEventListener("ExpressionUpdated" /* EXPRESSION_UPDATED */, this.watchExpressionUpdated, this);
  ...
  populateContextMenu(contextMenu, event) {
    ...
    if (this.watchExpressions.length > 1) {
      contextMenu.debugSection().appendItem(
        i18nString(UIStrings.deleteAllWatchExpressions),
        this.deleteAllButtonClicked.bind(this),
  ...
  appendApplicableItems(_event, contextMenu, target) {
    if (target instanceof ObjectUI.ObjectPropertiesSection.ObjectPropertyTreeElement) {
      if (!target.property.property.synthetic) {
        contextMenu.debugSection().appendItem(
          i18nString(UIStrings.addPropertyPathToWatch),
...
export class WatchExpression extends Common.ObjectWrapper.ObjectWrapper {
  ...
  updateExpression(newExpression) {
    ...
    this.update();
    this.dispatchEventToListeners("ExpressionUpdated" /* EXPRESSION_UPDATED */, this);
  }
  ...
  createWatchExpressionHeader(expressionValue, exceptionDetails) {
    ...
    if (Boolean(exceptionDetails) || !expressionValue) {
      ...
      const propertyValue = ObjectUI.ObjectPropertiesSection.ObjectPropertiesSection.createPropertyValueWithCustomSupport(
        expressionValue,
  ...
  populateContextMenu(contextMenu, event) {
    if (!this.isEditing()) {
      contextMenu.editSection().appendItem(
        i18nString(UIStrings.deleteWatchExpression),
    ...
    if (!this.isEditing() && this.result && (this.result.type === "number" || this.result.type === "string")) {
      contextMenu.clipboardSection().appendItem(
        ...
        this.copyValueButtonClicked.bind(this),
...
var Events = /* @__PURE__ */ ((Events2) => {
  Events2["EXPRESSION_UPDATED"] = "ExpressionUpdated";
  return Events2;
})(Events || {});
