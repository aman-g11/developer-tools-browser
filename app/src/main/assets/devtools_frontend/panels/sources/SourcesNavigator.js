...
export class NetworkNavigatorView extends NavigatorView {
  ...
  acceptProject(project) {
    return project.type() === Workspace.Workspace.projectTypes.Network && SDK.TargetManager.TargetManager.instance().isInScope(
...
export class FilesNavigatorView extends NavigatorView {
  ...
  constructor() {
    ...
    this.#automaticFileSystemNudge = uiI18n.getFormatLocalizedString(str_, UIStrings.automaticWorkspaceNudge, { PH1: link });
  ...
  wasShown() {
    ...
    this.#eventListeners = [
      this.#automaticFileSystemManager.addEventListener(
        Persistence.AutomaticFileSystemManager.Events.AUTOMATIC_FILE_SYSTEM_CHANGED,
        this.#automaticFileSystemChanged,
        this
      ),
      this.#automaticFileSystemManager.addEventListener(
        Persistence.AutomaticFileSystemManager.Events.AVAILABILITY_CHANGED,
  ...
  acceptProject(project) {
    ...
    return project.type() === Workspace.Workspace.projectTypes.FileSystem && Persistence.FileSystemWorkspaceBinding.FileSystemWorkspaceBinding.fileSystemType(project) !== "overrides" && !Snippets.ScriptSnippetFileSystem.isSnippetsProject(project);
  }
...
export class OverridesNavigatorView extends NavigatorView {
  ...
  constructor() {
    ...
    const placeholder = new UI.EmptyWidget.EmptyWidget(
      i18nString(UIStrings.noLocalOverrides),
    ...
    Persistence.NetworkPersistenceManager.NetworkPersistenceManager.instance().addEventListener(
      Persistence.NetworkPersistenceManager.Events.PROJECT_CHANGED,
  ...
  onProjectAddOrRemoved(event) {
    ...
    if (project && project.type() === Workspace.Workspace.projectTypes.FileSystem && Persistence.FileSystemWorkspaceBinding.FileSystemWorkspaceBinding.fileSystemType(project) !== "overrides") {
      return;
    }
    this.updateUI();
  }
...
export class ContentScriptsNavigatorView extends NavigatorView {
  constructor() {
    ...
    const placeholder = new UI.EmptyWidget.EmptyWidget(
      i18nString(UIStrings.noContentScripts),
...
export class SnippetsNavigatorView extends NavigatorView {
  constructor() {
    ...
    this.setPlaceholder(placeholder);
    placeholder.link = "https://developer.chrome.com/docs/devtools/javascript/snippets/";
    const toolbar = document.createElement("devtools-toolbar");
    toolbar.classList.add("navigator-toolbar");
    const newButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.newSnippet),
      "plus",
      i18nString(UIStrings.newSnippet),
      "sources.new-snippet"
    );
  ...
  async handleSaveAs(uiSourceCode) {
    ...
    if (TextUtils.ContentData.ContentData.isError(contentData)) {
      ...
      Common.Console.Console.instance().error(
        i18nString(UIStrings.saveAsFailed),
        /* show=*/
        false
      );
      return;
  ...
  handleAction(_context, actionId) {
    switch (actionId) {
      case "sources.create-snippet":
        void Snippets.ScriptSnippetFileSystem.findSnippetsProject().createFile(Platform.DevToolsPath.EmptyEncodedPathString, null, "").then((uiSourceCode) => Common.Revealer.reveal(uiSourceCode));
        return true;
