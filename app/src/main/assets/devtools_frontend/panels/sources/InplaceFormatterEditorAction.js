...
export class InplaceFormatterEditorAction {
  ...
  updateButton(uiSourceCode) {
    ...
    this.uiSourceCodeTitleChangedEvent = uiSourceCode ? uiSourceCode.addEventListener(
      Workspace.UISourceCode.Events.TitleChanged,
      (event) => this.updateButton(event.data),
      this
    ) : null;
  ...
  formatSourceInPlace() {
    ...
    if (uiSourceCode.isDirty()) {
      ...
      void uiSourceCode.requestContentData().then((contentDataOrError) => TextUtils.ContentData.ContentData.textOr(contentDataOrError, "")).then((content) => {
