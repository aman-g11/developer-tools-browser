...
const primitiveRemoteObjectTypes = /* @__PURE__ */ new Set(["number", "boolean", "bigint", "undefined"]);
...
export class SourcesPanel extends UI.Panel.Panel {
  ...
  constructor() {
    ...
    new UI.DropTarget.DropTarget(
      ...
      i18nString(UIStrings.dropWorkspaceFolderHere),
    ...
    this.toggleBreakpointsActiveAction = UI.ActionRegistry.ActionRegistry.instance().getAction("debugger.toggle-breakpoints-active");
    ...
    this.splitWidget = new UI.SplitWidget.SplitWidget(true, true, "sources-panel-split-view-state", initialDebugSidebarWidth);
    ...
    this.editorView = new UI.SplitWidget.SplitWidget(true, false, "sources-panel-navigator-split-view-state", initialNavigatorWidth);
    ...
    if (UI.ViewManager.ViewManager.instance().hasViewsForLocation("run-view-sidebar")) {
      ...
      const runViewTabbedPane = UI.ViewManager.ViewManager.instance().createTabbedLocation(this.revealNavigatorSidebar.bind(this), "run-view-sidebar").tabbedPane();
    ...
    Common.Settings.Settings.instance().moduleSetting("breakpoints-active").addChangeListener(this.breakpointsActiveStateChanged, this);
  ...
  static updateResizerAndSidebarButtons(panel) {
    ...
    const isInWrapper = UI.Context.Context.instance().flavor(QuickSourceView) && !UI.InspectorView.InspectorView.instance().isDrawerMinimized();
  ...
  debuggerPaused(event) {
    ...
    if (!this.#paused && Common.Settings.Settings.instance().moduleSetting("auto-focus-on-debugger-paused-enabled").get()) {
  ...
  showDebuggerPausedDetails(details) {
    ...
    if (this.threadsSidebarPane && this.#lastPausedTarget?.deref() !== pausedTarget && pausedTarget !== SDK.TargetManager.TargetManager.instance().primaryPageTarget()) {
      void this.sidebarPaneStack?.showView(this.threadsSidebarPane);
    }
    window.focus();
    Host.InspectorFrontendHost.InspectorFrontendHostInstance.bringToFront();
    const withOverlay = UI.Context.Context.instance().flavor(SDK.Target.Target)?.model(SDK.OverlayModel.OverlayModel) && !Common.Settings.Settings.instance().moduleSetting("disable-paused-state-overlay").get();
    if (withOverlay && !this.overlayLoggables) {
      ...
      VisualLogging.registerLoggable(
        ...
        `${VisualLogging.dialog("debugger-paused")}`,
        null,
        new DOMRect(0, 0, 200, 20)
      );
      VisualLogging.registerLoggable(
        this.overlayLoggables.resumeButton,
        `${VisualLogging.action("debugger.toggle-pause")}`,
        this.overlayLoggables.debuggerPausedMessage,
        new DOMRect(0, 0, 20, 20)
      );
      VisualLogging.registerLoggable(
        this.overlayLoggables.stepOverButton,
        `${VisualLogging.action("debugger.step-over")}`,
        this.overlayLoggables.debuggerPausedMessage,
        new DOMRect(0, 0, 20, 20)
      );
    }
    ...
    window.setTimeout(() => {
      ...
      if (byOverlayButton) {
        ...
        VisualLogging.logClick(
          this.#paused && details?.reason === Protocol.Debugger.PausedEventReason.Step ? this.overlayLoggables.stepOverButton : this.overlayLoggables.resumeButton,
  ...
  switchToPausedTarget(debuggerModel) {
    ...
    for (const debuggerModel2 of SDK.TargetManager.TargetManager.instance().models(SDK.DebuggerModel.DebuggerModel)) {
      if (debuggerModel2.isPaused()) {
        UI.Context.Context.instance().setFlavor(SDK.Target.Target, debuggerModel2.target());
        break;
      }
    }
  }
  ...
  editorSelected(event) {
    ...
    if (this.editorView.mainWidget() && Common.Settings.Settings.instance().moduleSetting("auto-reveal-in-navigator").get()) {
  ...
  toggleBreakpointsActive() {
    Common.Settings.Settings.instance().moduleSetting("breakpoints-active").set(!Common.Settings.Settings.instance().moduleSetting("breakpoints-active").get());
  }
  ...
  appendUISourceCodeItems(event, contextMenu, uiSourceCode) {
    ...
    if (!uiSourceCode.project().isServiceProject() && !eventTarget.isSelfOrDescendant(this.navigatorTabbedLocation.widget().element) && !(Root.Runtime.experiments.isEnabled(Root.ExperimentNames.ExperimentName.JUST_MY_CODE) && Workspace.IgnoreListManager.IgnoreListManager.instance().isUserOrSourceMapIgnoreListedUISourceCode(
      uiSourceCode
    ))) {
      contextMenu.revealSection().appendItem(
        i18nString(UIStrings.revealInSidebar),
    ...
    if (UI.ActionRegistry.ActionRegistry.instance().hasAction(openAiAssistanceId)) {
      ...
      if (!eventTarget.isSelfOrDescendant(editorElement) && uiSourceCode.contentType().isTextType()) {
        ...
        appendSubmenuPromptAction(
          submenu,
          action,
          i18nString(UIStrings.assessPerformance),
        ...
        appendSubmenuPromptAction(
          submenu,
          action,
          i18nString(UIStrings.explainThisScript),
        ...
        appendSubmenuPromptAction(
          submenu,
          action,
          i18nString(UIStrings.explainInputHandling),
          "Does the script handle user input safely",
          openAiAssistanceId + ".input"
        );
      }
    }
    if (uiSourceCode.contentType().hasScripts() && Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().scriptsForUISourceCode(uiSourceCode).every((script) => script.isJavaScript())) {
  ...
  appendUILocationItems(contextMenu, uiLocation) {
    ...
    if (!Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().scriptsForUISourceCode(uiSourceCode).every((script) => script.isJavaScript())) {
      return;
    }
    ...
    if (contentType.hasScripts()) {
      ...
      if (debuggerModel?.isPaused()) {
        contextMenu.debugSection().appendItem(
          i18nString(UIStrings.continueToHere),
  ...
  appendRemoteObjectItems(contextMenu, remoteObject) {
    ...
    contextMenu.debugSection().appendItem(
      ...
      () => executionContext?.target().model(SDK.ConsoleModel.ConsoleModel)?.saveToTempVariable(executionContext, remoteObject),
    ...
    if (remoteObject.type === "string") {
      ...
      ctxMenuClipboardSection.appendItem(
        i18nString(UIStrings.copyS, { PH1: String(copyContextMenuTitle) }),
        copyDecodedValueHandler,
        { jslogContext: "copy-object" }
      );
    } else if (remoteObject.type === "function") {
      contextMenu.debugSection().appendItem(
        i18nString(UIStrings.showFunctionDefinition),
  ...
  showFunctionDefinition(remoteObject) {
    void SDK.RemoteObject.RemoteFunction.objectAsFunction(remoteObject).targetFunction().then(
  ...
  async didGetFunctionDetails(response) {
    ...
    const uiLocation = await Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().rawLocationToUILocation(
      response.location
    );
    if (uiLocation) {
  ...
  handleAction(context, actionId) {
    ...
    switch (actionId) {
      case "debugger.toggle-pause": {
        const actionHandledInPausedOverlay = context.flavor(UI.ShortcutRegistry.ForwardedShortcut) && !Common.Settings.Settings.instance().moduleSetting("disable-paused-state-overlay").get();
