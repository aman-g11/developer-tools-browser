...
export class MediaQueryInspector extends UI.Widget.Widget {
  ...
  constructor(getWidthCallback, setWidthCallback, mediaThrottler, view = DEFAULT_VIEW) {
    ...
    UI.ZoomManager.ZoomManager.instance().addEventListener(
      UI.ZoomManager.Events.ZOOM_CHANGED,
  ...
  onContextMenu(event, locations) {
    ...
    for (let i = 0; i < locations.length; ++i) {
      ...
      const descriptor = typeof uiLocation.columnNumber === "number" ? Platform.StringUtilities.sprintf(
        "%s:%d:%d",
    ...
    const subMenuItem = contextMenu.defaultSection().appendSubMenuItem(
      i18nString(UIStrings.revealInSourceCode),
      void 0,
...
export var Section = /* @__PURE__ */ ((Section2) => {
...
export class MediaQueryUIModel {
  ...
  compareTo(other) {
    ...
    if (this.dimensionsEqual(other)) {
      ...
      return Platform.StringUtilities.compare(myLocation.url, otherLocation.url) || myLocation.lineNumber - otherLocation.lineNumber || myLocation.columnNumber - otherLocation.columnNumber;
    }
    const thisMaxWidthExpression = this.maxWidthExpression();
    const otherMaxWidthExpression = other.maxWidthExpression();
