  ...
  render(html`
    ...
    jslogContext: "search.clear"
      ...
      <div class="search-results" @keydown=${onPanelKeyDown}>
        ${searchResults.length ? html`<devtools-widget ${widget(SearchResultsPane, { searchResults, searchConfig })}
    ...
    output.collapseAllResults = () => void w.collapseAllResults();
            ...
            </devtools-widget>` : widget(UI.EmptyWidget.EmptyWidget, { header, text })}
      </div>
...
export class SearchView extends UI.Widget.VBox {
  ...
  #startSearch(searchConfig) {
    ...
    this.#progress = new Common.Progress.ProgressProxy(new Common.Progress.Progress(), void 0, this.requestUpdate.bind(this));
    this.#searchStarted();
    if (this.#searchScope) {
