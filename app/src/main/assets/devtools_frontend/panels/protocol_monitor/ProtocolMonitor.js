...
import { Events as JSONEditorEvents, JSONEditor } from "./JSONEditor.js";
    ...
    html`
      ...
      (message) => html`
                      <tr @select=${() => input.onSelect(message)}
                        ...
                        <td>
                          ${message.result ? html`<code>${JSON.stringify(message.result)}</code>` : message.error ? html`<code>${JSON.stringify(message.error)}</code>` : "id" in message ? "(pending)" : ""}
                        </td>
                        <td data-value=${message.elapsedTime || 0}>
                          ${!("id" in message) ? "" : message.elapsedTime ? i18nString(UIStrings.sMs, { PH1: String(message.elapsedTime) }) : "(pending)"}
                        </td>
      ...
      display: input.sidebarVisible ? "none" : "flex"
              ...
              <select class="target-selector"
                ...
                ${input.targets.map((target2) => html`
    ...
    tabs: [
      {
        ...
        view: input.type === void 0 ? new UI.EmptyWidget.EmptyWidget(
          i18nString(UIStrings.noMessageSelected),
          i18nString(UIStrings.selectAMessageToView)
