...
export class ElementsPanel extends UI.Panel.Panel {
  ...
  constructor() {
    ...
    if (Annotations.AnnotationRepository.annotationsEnabled()) {
      PanelCommon.AnnotationManager.instance().initializePlacementForAnnotationType(
        Annotations.AnnotationType.ELEMENT_NODE,
  ...
  evaluateTrackingComputedStyleUpdatesForNode = Common.Debouncer.debounce(() => {
    ...
    void selectedNode.domModel()?.cssModel()?.trackComputedStyleUpdatesForNode(
      shouldTrackComputedStyleUpdates ? selectedNode.id : void 0
    );
  }, 100);
...
${node.simpleSelector()} {}`, false);
  ...
  revealComputedStylesPane() {
    this.sidebarPaneView?.tabbedPane().selectTab("computed" /* COMPUTED */);
  }
    ...
    function revealPromise(resolve, reject) {
      ...
      function onNodeResolved(resolvedNode) {
        ...
        if (resolvedNode) {
          ...
          const promise = resolvedNode instanceof SDK.DOMModel.AdoptedStyleSheet ? panel.revealAndSelectAdoptedStyleSheet(resolvedNode, opts) : panel.revealAndSelectNode(resolvedNode, opts);
