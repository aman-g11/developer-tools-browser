  ...
  const renderElement = (element) => html`<div
    ...
    nodeClasses: element.domClasses
        ...
        <label
            ...
            jslog=${VisualLogging.showStyleEditor("color").track({
    click: true
  })}>
          <input
  ...
  render(
    html`
      ...
      click: true
          ...
          ${input.gridElements ? html`<div class="content-section" jslog=${VisualLogging.section("grid-overlays")}>
              <h3 class="content-section-title">
                ${input.gridElements.length ? i18nString(UIStrings.gridOrGridLanesOverlays) : i18nString(UIStrings.noGridOrGridLanesLayoutsFoundOnThisPage)}
              </h3>
              ${input.gridElements.length ? html`<div class="elements">${input.gridElements.map(renderElement)}</div>` : ""}
            </div>` : ""}
        </details>
        ${input.flexContainerElements !== void 0 ? html`
          <details open>
            <summary
                class="header"
                @keydown=${input.onSummaryKeyDown}
            ...
            ${input.flexContainerElements ? html`<div class="content-section" jslog=${VisualLogging.section("flexbox-overlays")}>
                <h3 class="content-section-title">
                  ${input.flexContainerElements.length ? i18nString(UIStrings.flexboxOverlays) : i18nString(UIStrings.noFlexboxLayoutsFoundOnThisPage)}
                </h3>
                ${input.flexContainerElements.length ? html`<div class="elements">${input.flexContainerElements.map(renderElement)}</div>` : ""}
              </div>` : ""}
          </details>` : ""}
      </div>`,
    // clang-format on
...
export class LayoutPane extends UI.Widget.Widget {
  ...
  modelAdded(domModel) {
    ...
    overlayModel.addEventListener(
      SDK.OverlayModel.Events.PERSISTENT_GRID_OVERLAY_STATE_CHANGED,
      this.requestUpdate,
      this
    );
    overlayModel.addEventListener(
