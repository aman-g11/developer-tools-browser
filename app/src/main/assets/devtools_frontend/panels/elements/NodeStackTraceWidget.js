...
export const DEFAULT_VIEW = (input, _output, target) => {
  ...
  render(
    ...
    ${target && stackTrace ? html`<devtools-widget
                class="stack-trace"
                .widgetConfig=${UI.Widget.widgetConfig(Components.JSPresentationUtils.StackTracePreviewContent, { stackTrace })}>
              </devtools-widget>` : html`<div class="gray-info-message">${i18nString(UIStrings.noStackTraceAvailable)}</div>`}`,
    target
  );
};
export class NodeStackTraceWidget extends UI.Widget.VBox {
  ...
  async performUpdate() {
    ...
    const stackTrace = runtimeStackTrace && target ? await Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().createStackTraceFromProtocolRuntime(
      runtimeStackTrace,
      target
    ) : void 0;
