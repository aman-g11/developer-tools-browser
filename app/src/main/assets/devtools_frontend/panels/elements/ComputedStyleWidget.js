...
function renderPropertyContents(node, cache, propertyName, propertyValue) {
  ...
  const value = Renderer.renderValueElement(
    ...
    matchProperty(propertyName, propertyValue),
...
const createTraceElement = (node, property, isPropertyOverloaded, matchedStyles, linkifier) => {
  ...
  const { valueElement } = Renderer.renderValueElement(
    property,
    matchProperty(property.name, property.value),
    [new ColorRenderer(), new URLRenderer(null, node), new StringRenderer()]
  );
...
export class ComputedStyleWidget extends UI.Widget.VBox {
  ...
  constructor(element, view = DEFAULT_VIEW) {
    ...
    this.showInheritedComputedStylePropertiesSetting = Common.Settings.Settings.instance().createSetting("show-inherited-computed-style-properties", false);
    ...
    return (node) => {
      ...
      if (data.tag === "property") {
        ...
        const propertyElement = createPropertyElement(
          domNode,
          this.#propertyElementsCache,
          data.propertyName,
          data.propertyValue,
          propertyTraces.has(data.propertyName),
          data.inherited,
          activeProperty,
          (event) => {
            if (activeProperty) {
  ...
  handleContextMenuEvent(matchedStyles, property, event) {
    ...
    contextMenu.defaultSection().appendItem(
      i18nString(UIStrings.navigateToStyle),
      () => Common.Revealer.reveal(property),
  ...
  async onFilterChanged(event) {
    ...
    if (event.detail && this.#computedStylesTree.data && this.#computedStylesTree.data.tree) {
      UI.ARIAUtils.LiveAnnouncer.alert(i18nString(
        UIStrings.filterUpdateAriaText,
        { PH1: event.detail, PH2: this.#computedStylesTree.data.tree.length }
      ));
    }
  }
...
const alwaysShownComputedProperties = /* @__PURE__ */ new Set(["display", "height", "width"]);
