  ...
  switch (node.nodeType()) {
    ...
    case Node.ELEMENT_NODE: {
      ...
      if (ElementsTreeElement.canShowInlineText(node)) {
        ...
        return html`${openingTag}<span class="webkit-html-text-node" jslog=${VisualLogging.value("text-node").track({ change: true, dblclick: true })} ${animateOn(
...
function renderAttribute(attr, updateRecord, isDiff, node) {
  ...
  if (!value) {
    if (name === "popovertarget") {
      relationRefDirective = relationRef(
        Protocol.DOM.GetElementByRelationRequestRelation.PopoverTarget,
        i18nString(UIStrings.showPopoverTarget)
      );
    } else if (name === "interesttarget") {
      relationRefDirective = relationRef(
        Protocol.DOM.GetElementByRelationRequestRelation.InterestTarget,
        i18nString(UIStrings.showInterestTarget)
      );
    } else if (name === "commandfor") {
      relationRefDirective = relationRef(
        Protocol.DOM.GetElementByRelationRequestRelation.CommandFor,
  ...
  const withEntitiesRef = valueType === 0 /* UNKNOWN */ ? ref((el) => {
    if (el) {
      setValueWithEntities(el, value);
    }
  }) : nothing;
  return html`<span class="webkit-html-attribute" jslog=${jslog}><span class="webkit-html-attribute-name"
    ...
    DOM_UPDATE_ANIMATION_CLASS_NAME
                        ...
                        ${valueType === 1 /* SRC */ ? renderLinkifiedValue(value, node) : nothing}
                        ${valueType === 2 /* SRCSET */ ? renderLinkifiedSrcset(Common.Srcset.parseSrcset(value), node) : nothing}
                </span>"` : nothing}</span>`;
}
...
export class ElementsTreeElement extends UI.TreeOutline.TreeElement {
  ...
  constructor(node, isClosingTag) {
    ...
    this.listItemElement.setAttribute(
      "jslog",
      `${VisualLogging.treeItem().parent("elementsTreeOutline").track({
  ...
  static populateForcedPseudoStateItems(contextMenu, node) {
    ...
    for (const pseudoClass of pseudoClasses) {
      ...
      stateMenu.defaultSection().appendCheckboxItem(
        ":" + pseudoClass,
        setPseudoStateCallback.bind(null, pseudoClass, !pseudoClassForced),
    ...
    DEFAULT_VIEW(
      {
        ...
        showPopoverAdorner: Boolean(Root.Runtime.hostConfig.devToolsAllowPopoverForcing?.enabled) && Boolean(this.node().attributes().find((attr) => attr.name === "popover")) && !this.isClosingTag(),
        ...
        aiButtonTitle: this.isAiButtonEnabled() ? UI.ActionRegistry.ActionRegistry.instance().getAction("freestyler.elements-floating-button").title() : void 0,
        onAiButtonClick: (ev) => {
          ev.stopPropagation();
  ...
  onbind() {
    ...
    if (this.treeOutline && !this.isClosingTag()) {
      ...
      this.nodeInternal.addEventListener(
        SDK.DOMModel.DOMNodeEvents.TOP_LAYER_INDEX_CHANGED,
        this.onTopLayerIndexChanged,
        this
      );
      this.nodeInternal.addEventListener(
        SDK.DOMModel.DOMNodeEvents.SCROLLABLE_FLAG_UPDATED,
        this.#onScrollableFlagUpdated,
        this
      );
      this.nodeInternal.addEventListener(
        SDK.DOMModel.DOMNodeEvents.AD_RELATED_STATE_UPDATED,
      ...
      this.nodeInternal.addEventListener(
        SDK.DOMModel.DOMNodeEvents.GRID_OVERLAY_STATE_CHANGED,
        this.#onPersistentGridOverlayStateChanged,
        this
      );
      this.nodeInternal.addEventListener(
        SDK.DOMModel.DOMNodeEvents.SCROLL_SNAP_OVERLAY_STATE_CHANGED,
  ...
  async populateTagContextMenu(contextMenu, event) {
    ...
    contextMenu.editSection().appendItem(
      ...
      treeElement.addNewAttribute.bind(treeElement),
    ...
    if (attribute && !newAttribute) {
      contextMenu.editSection().appendItem(
        i18nString(UIStrings.editAttribute),
        this.startEditingAttribute.bind(this, attribute, target),
  ...
  populateExpandRecursively(contextMenu) {
    contextMenu.viewSection().appendItem(
      ...
      this.expandRecursively.bind(this),
      { jslogContext: "expand-recursively" }
    );
  }
  populateScrollIntoView(contextMenu) {
    contextMenu.viewSection().appendItem(
      i18nString(UIStrings.scrollIntoView),
      () => this.nodeInternal.scrollIntoView(),
  ...
  async populateTextContextMenu(contextMenu, textNode) {
    if (!this.editing) {
      contextMenu.editSection().appendItem(
        i18nString(UIStrings.editText),
        this.startEditingTextNode.bind(this, textNode),
  ...
  async populateNodeContextMenu(contextMenu) {
    ...
    menuItem = contextMenu.clipboardSection().appendItem(
      i18nString(UIStrings.cut),
      treeOutline.performCopyOrCut.bind(treeOutline, true, this.nodeInternal),
    ...
    if (this.nodeInternal.nodeType() === Node.ELEMENT_NODE) {
      section.appendItem(
        i18nString(UIStrings.copySelector),
        this.copyCSSPath.bind(this),
        { jslogContext: "copy-selector" }
      );
      section.appendItem(
        i18nString(UIStrings.copyJsPath),
        this.copyJSPath.bind(this),
        { disabled: !canGetJSPath(this.nodeInternal), jslogContext: "copy-js-path" }
      );
      section.appendItem(
        i18nString(UIStrings.copyStyles),
        this.copyStyles.bind(this),
    ...
    if (!isShadowRoot) {
      ...
      menuItem = contextMenu.editSection().appendItem(
        ...
        treeOutline.duplicateNode.bind(treeOutline, this.nodeInternal),
        {
    ...
    menuItem = contextMenu.clipboardSection().appendItem(
      i18nString(UIStrings.paste),
      treeOutline.pasteNode.bind(treeOutline, this.nodeInternal),
      { disabled: !treeOutline.canPaste(this.nodeInternal), jslogContext: "paste" }
    );
    menuItem.setShortcut(createShortcut("V", modifier));
    menuItem = contextMenu.debugSection().appendCheckboxItem(
      i18nString(UIStrings.hideElement),
      treeOutline.toggleHideElement.bind(treeOutline, this.nodeInternal),
    ...
    if (isEditable) {
      contextMenu.editSection().appendItem(
        i18nString(UIStrings.deleteElement),
  ...
  async populateProcessingElementContextMenu(contextMenu) {
    ...
    contextMenu.editSection().appendItem(
      i18nString(UIStrings.editData),
      this.startEditingProcessingInstructionValue.bind(this),
  ...
  startEditingAttribute(attribute, elementForSelection) {
    ...
    const config = new UI.InplaceEditor.Config(
      this.attributeEditingCommitted.bind(this),
  ...
  startEditingTextNode(textNodeElement) {
    ...
    const config = new UI.InplaceEditor.Config(
      this.textNodeEditingCommitted.bind(this, textNode),
      this.editingCancelled.bind(this),
      null
    );
...
export const ForbiddenClosingTagElements = /* @__PURE__ */ new Set([
  "area",
  "base",
  "basefont",
  "br",
...
export const EditTagBlocklist = /* @__PURE__ */ new Set(["html", "head", "body"]);
