...
export class StylePropertiesSection {
  ...
  constructor(stylesContainer, matchedStyles, style, sectionIdx, computedStyles, parentsComputedStyles, computedStyleExtraFields, customHeaderText) {
    ...
    if (rule) {
      const newRuleButton = new UI.Toolbar.ToolbarButton(
        i18nString(UIStrings.insertStyleRuleBelow),
        "plus",
        void 0,
    ...
    if (Root.Runtime.experiments.isEnabled(Root.ExperimentNames.ExperimentName.FONT_EDITOR) && this.editable) {
      ...
      this.fontEditorButton = new UI.Toolbar.ToolbarButton("Font Editor", "custom-typography", void 0, "font-editor");
    ...
    this.selectorElement.setAttribute(
      "jslog",
      `${VisualLogging.cssRuleHeader("selector").track({ click: true, change: true })}`
    );
  ...
  resetToolbars() {
    if (this.stylesContainer.swatchPopoverHelper().isShowing() || this.styleInternal.type === SDK.CSSStyleDeclaration.Type.Inline) {
      return;
    }
    if (this.fontEditorToolbar) {
      this.fontEditorToolbar.classList.add("font-toolbar-hidden");
  ...
  static createRuleOriginNode(matchedStyles, linkifier, rule) {
    ...
    function linkifyNode(label) {
      if (header?.ownerNode) {
        ...
        link.widgetConfig = UI.Widget.widgetConfig((e) => new PanelsCommon.DOMLinkifier.DeferredDOMNodeLink(e, header.ownerNode));
      ...
      if (rule && rule.style.styleSheetId && rule.treeScope) {
        ...
        link.widgetConfig = UI.Widget.widgetConfig(
          (e) => new PanelsCommon.DOMLinkifier.DeferredDOMNodeLink(e, ownerNode, void 0, rule.style.styleSheetId)
        );
        link.textContent = label;
        return link;
      }
  ...
  onKeyDown(event) {
    ...
    if (UI.UIUtils.isEditing() || !this.editable || keyboardEvent.altKey || keyboardEvent.ctrlKey || keyboardEvent.metaKey) {
      return;
    }
  ...
  highlight(mode = "all") {
    ...
    const selectorList = this.styleInternal.parentRule && this.styleInternal.parentRule instanceof SDK.CSSRule.CSSStyleRule ? this.styleInternal.parentRule.selectorText() : void 0;
    ...
    containerElement.data = {
      ...
      onContainerLinkClick: (event) => {
        ...
        void ElementsPanel.instance().revealAndSelectNode(
          ...
          { showPanel: true, focusNode: true, highlightInOverlay: false }
        );
  ...
  #shouldCollapse() {
    ...
    const allOverloaded = properties.every((p) => this.matchedStyles.propertyState(p) === SDK.CSSMatchedStyles.PropertyState.OVERLOADED);
  ...
  handleQueryRuleClick(query, event) {
    ...
    const config = new UI.InplaceEditor.Config(
      this.editingMediaCommitted.bind(this, query),
      this.editingMediaCancelled.bind(this, element),
      void 0,
  ...
  handleSelectorClick(event) {
    ...
    if (UI.KeyboardShortcut.KeyboardShortcut.eventHasCtrlEquivalentKey(event) && this.navigable && target.classList.contains("simple-selector")) {
...
${allDeclarationText}
  ...
  startEditingSelector() {
    ...
    const config = new UI.InplaceEditor.Config(
      this.editingSelectorCommitted.bind(this),
      this.editingSelectorCancelled.bind(this),
      void 0
    );
...
export class BlankStylePropertiesSection extends StylePropertiesSection {
  ...
  constructor(stylesContainer, matchedStyles, defaultSelectorText, styleSheetHeader, ruleLocation, insertAfterStyle, sectionIdx) {
    ...
    this.selectorRefElement.appendChild(StylePropertiesSection.linkifyRuleLocation(
      cssModel,
      this.stylesContainer.linkifier,
      styleSheetHeader,
      this.actualRuleLocation()
    ));
