...
export class ThrottlingManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  createOfflineToolbarCheckbox() {
    const checkbox = new UI.Toolbar.ToolbarCheckbox(
      ...
      i18nString(UIStrings.forceDisconnectedFromNetwork),
    ...
    function forceOffline() {
      if (checkbox.checked()) {
        ...
        const newConditions = !this.lastNetworkThrottlingConditions.download && !this.lastNetworkThrottlingConditions.upload ? SDK.NetworkManager.NoThrottlingConditions : this.lastNetworkThrottlingConditions;
    ...
    function appendItems(contextMenu) {
      for (let index = 0; index < options.length; ++index) {
        ...
        contextMenu.defaultSection().appendCheckboxItem(
          conditions.title,
          selector.optionSelected.bind(selector, conditions),
    ...
    return {
      ...
      updateRecommendedOption(recommendedOption) {
        for (let i = 0; i < optionEls.length - 1; i++) {
          ...
          optionEls[i].text = option === recommendedOption ? i18nString(UIStrings.recommendedThrottling, { PH1: option.title() }) : option.title();
  ...
  createSaveDataOverrideSelector(className) {
    const reset = new Option(i18nString(UIStrings.noSaveDataOverride), void 0, true, true);
    ...
    const handler = (e) => {
      ...
      this.dispatchEventToListeners(ThrottlingManager.Events.SAVE_DATA_OVERRIDE_CHANGED, select2.selectedIndex);
    };
    ...
    this.addEventListener(
      ThrottlingManager.Events.SAVE_DATA_OVERRIDE_CHANGED,
      ({ data }) => select.setSelectedIndex(data)
    );
    return select;
  }
  ...
  createHardwareConcurrencySelector() {
    ...
    const checkbox = UI.UIUtils.CheckboxLabel.create(
      i18nString(UIStrings.hardwareConcurrency),
      false,
      i18nString(UIStrings.hardwareConcurrencySettingLabel),
...
((ThrottlingManager2) => {
  let Events;
  ((Events2) => {
    Events2["SAVE_DATA_OVERRIDE_CHANGED"] = "SaveDataOverrideChanged";
  })(Events = ThrottlingManager2.Events || (ThrottlingManager2.Events = {}));
