...
export class CPUThrottlingCard {
  ...
  updateState() {
    ...
    if (this.state === "cta") {
      ...
      this.calibrateButton.textContent = hasCalibrated ? i18nString(UIStrings.recalibrate) : i18nString(UIStrings.calibrate);
      if (!hasCalibrated) {
    ...
    const resultToString = (result2) => {
      ...
      if (typeof result2 === "string") {
        return SDK.CPUThrottlingManager.calibrationErrorToString(result2);
      }
      if (typeof result2 !== "number") {
        return `Invalid: ${result2}`;
      }
      return i18nString(UIStrings.dSlowdown, { PH1: result2.toFixed(1) });
    };
...
export class ThrottlingSettingsTab extends UI.Widget.VBox {
  ...
  renderItem(conditions, _editable) {
    ...
    element.createChild("div", "conditions-list-separator");
    element.createChild("div", "conditions-list-text").textContent = i18nString(UIStrings.dms, { PH1: conditions.latency });
    element.createChild("div", "conditions-list-separator");
    ...
    element.createChild("div", "conditions-list-separator");
    element.createChild("div", "conditions-list-text").textContent = conditions.packetReordering ? i18nString(UIStrings.on) : i18nString(UIStrings.off);
    return element;
  }
...
function throughputText(throughput) {
  ...
  return i18nString(UIStrings.fsmbits, { PH1: throughputInKbps / 1e3 | 0 });
}
