...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  const jslog = (group, condition) => `${VisualLogging.item(Platform.StringUtilities.toKebabCase(
    ...
    html`<select
      ...
      (group) => html`<optgroup
            ...
            ${group.items.map((condition) => html`<option
              ...
              ?selected=${selectedConditions ? SDK.NetworkManager.networkConditionsEqual(condition, selectedConditions) : group === input.throttlingGroups[0]}
              value=${title(condition)}
              aria-label=${i18nString(UIStrings.sS, { PH1: group.title, PH2: title(condition) })}
              jslog=${jslog(group, condition)}>
                ${condition === input.recommendedConditions ? i18nString(UIStrings.recommendedThrottling, { PH1: title(condition) }) : title(condition)}
            </option>`)}
        </optgroup>`
    )}
...
export class NetworkThrottlingSelect extends Common.ObjectWrapper.ObjectWrapper {
  ...
  #title;
  #view;
  #variant = NetworkThrottlingSelect.Variant.GLOBAL_CONDITIONS;
  #disabled = false;
  static createForGlobalConditions(element, title) {
    ...
    select.addEventListener(
      ...
      (ev) => !("block" in ev.data) && SDK.NetworkManager.MultitargetNetworkManager.instance().setNetworkConditions(ev.data)
    );
...
export class NetworkThrottlingSelectorWidget extends UI.Widget.VBox {
  ...
  constructor(element, view = DEFAULT_VIEW) {
    ...
    this.#select.addEventListener("conditionsChanged" /* CONDITIONS_CHANGED */, ({ data }) => this.#conditionsChangedHandler?.(data));
  }
  get disabled() {
