...
export var TargetPanel = /* @__PURE__ */ ((TargetPanel2) => {
...
function renderSettings({
  ...
  if (settings.viewportSettings) {
    environmentFragments.push(
      html`<div>${settings.viewportSettings.isMobile ? i18nString(UIStrings.mobile) : i18nString(UIStrings.desktop)}</div>`
    );
    environmentFragments.push(html`<div class="separator"></div>`);
    environmentFragments.push(
      html`<div>${settings.viewportSettings.width}×${settings.viewportSettings.height} px</div>`
    );
  }
    ...
    if (settings.networkConditionsSettings) {
      if (settings.networkConditionsSettings.title) {
        replaySettingsFragments.push(
          html`<div>${settings.networkConditionsSettings.title}</div>`
        );
      } else {
      ...
      replaySettingsFragments.push(
        html`<div>${SDK.NetworkManager.NoThrottlingConditions.title instanceof Function ? SDK.NetworkManager.NoThrottlingConditions.title() : SDK.NetworkManager.NoThrottlingConditions.title}</div>`
      );
    }
    ...
    <div class="settings-row">
      <div class="settings-container">
        <div
          ...
          ${isEditable ? html`<devtools-icon
                  class="chevron"
                  name="triangle-down">
                </devtools-icon>` : ""}
        </div>
        <div class=${Lit.Directives.classMap(replaySettingsClassMap)}>
          ${replaySettingsFragments.length ? replaySettingsFragments : html`<div>${i18nString(UIStrings.default)}</div>`}
        </div>
      </div>
      <div class="settings-container">
        <div class="settings-title">${i18nString(UIStrings.environment)}</div>
        <div class="settings">
          ${environmentFragments.length ? environmentFragments : html`<div>${i18nString(UIStrings.default)}</div>`}
        </div>
      </div>
    </div>
  `;
...
function renderTimelineArea(input, output) {
  ...
  return html`
    ...
    iconName: "cross"
            ...
            ${renderTextEditor(input, output)}` : Lit.nothing}
          </div>
        </devtools-split-view>
      `;
}
...
function renderReplayOrAbortButton(input) {
  if (input.replayState.isPlaying) {
    return html`
        <devtools-button .jslogContext=${"abort-replay"} @click=${input.onAbortReplay} .iconName=${"pause"} .variant=${Buttons.Button.Variant.OUTLINED}>
...
function renderSections(input) {
  return html`
      <div class="sections">
      ${!input.showCodeView ? html`<div class="section-toolbar">
        <devtools-button
          @click=${input.showCodeToggle}
          class="show-code"
          .data=${{
    ...
    (section, i) => html`
      ...
      isFirstSection: i === 0,
      isLastSection: i === input.sections.length - 1 && section.steps.length === 0,
      isSelected: input.selectedStep === (section.causingStep || null),
      sectionIndex: i,
      isRecording: input.isRecording,
      isPlaying: input.replayState.isPlaying,
      error: input.getSectionState(section) === State.ERROR ? input.currentError ?? void 0 : void 0,
      hasBreakpoint: false,
      ...
      jslogContext: "add-assertion"
                  ...
                  >${i18nString(UIStrings.addAssertion)}</devtools-button>` : void 0}
                  ${input.isRecording && i === input.sections.length - 1 ? html`<div class="step recording">${i18nString(
      UIStrings.recording
    )}</div>` : null}
                </div>
              </div>
...
function renderHeader(input) {
  ...
  return html`
    ...
    jslogContext: "edit-title"
        ...
        ${input.isTitleInvalid ? html`<div class="title-input-error-text">
          ${i18nString(UIStrings.requiredTitleError)}
        </div>` : Lit.nothing}
      </div>
      ${!input.isRecording && input.replayAllowed ? html`<div class="actions">
              <devtools-button
...
export const DEFAULT_VIEW = (input, output, target) => {
  ...
  const footerButtonTitle = input.recordingTogglingInProgress ? i18nString(UIStrings.recordingIsBeingStopped) : i18nString(UIStrings.endRecording);
  Lit.render(
    html`
    ...
    <div @click=${input.onWrapperClick} class=${Lit.Directives.classMap(
      ...
      <div class="recording-view main">
        ${renderHeader(input)}
        ${input.extensionDescriptor ? html`
...
export class RecordingView extends UI.Widget.Widget {
  ...
  #getSectionState(section) {
    ...
    const currentSection = this.sections.find(
      (section2) => section2.steps.includes(currentStep)
    );
    if (!currentSection) {
      if (this.currentError) {
        return State.ERROR;
  ...
  #onNetworkConditionsChange(event) {
    ...
    if (throttlingMenu instanceof HTMLSelectElement) {
      ...
      this.networkConditionsChanged?.(
        preset?.i18nTitleKey === SDK.NetworkManager.NoThrottlingConditions.i18nTitleKey ? void 0 : preset
      );
    }
  }
