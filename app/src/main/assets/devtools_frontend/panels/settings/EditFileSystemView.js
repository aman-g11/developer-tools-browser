...
export var ExcludedFolderStatus = /* @__PURE__ */ ((ExcludedFolderStatus2) => {
...
export const DEFAULT_VIEW = (input, _output, target) => {
  render(html`
      ...
      <div class="excluded-folder-header">
        ...
        ${input.excludedFolderPaths.filter(({ status }) => status !== 1 /* VALID */).map(({ status }) => html`<span class="excluded-folder-error">${statusString(status)}</span>`)}
    </div>`, target);
};
