...
export class KeybindsSettingsTab extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    const keybindsSetSelect = SettingsUI.SettingsUI.createControlForSetting(
      keybindsSetSetting,
      i18nString(UIStrings.matchShortcutsFromPreset)
    );
  ...
  createListItems() {
    const actions = UI.ActionRegistry.ActionRegistry.instance().actions().filter((action) => action.configurableBindings()).sort((actionA, actionB) => {
...
export class ShortcutListItem {
  ...
  constructor(item, settingsTab, isEditing) {
    ...
    this.element.setAttribute(
      "jslog",
      `${VisualLogging.item().context(item.id()).track({ keydown: "Escape", resize: true })}`
    );
  ...
  setupEditor() {
    ...
    const addShortcutButton = UI.UIUtils.createTextButton(
      i18nString(UIStrings.addAShortcut),
    ...
    this.element.appendChild(this.createIconButton(
      i18nString(UIStrings.ResetShortcutsForAction),
      "undo",
      "",
      "undo",
    ...
    this.confirmButton = this.createIconButton(
      i18nString(UIStrings.confirmChanges),
      "checkmark",
  ...
  addShortcut() {
    const shortcut = new UI.KeyboardShortcut.KeyboardShortcut([], this.item.id(), UI.KeyboardShortcut.Type.UNSET_SHORTCUT);
  ...
  createEditButton() {
    return this.createIconButton(
      i18nString(UIStrings.editShortcut),
      "edit",
  ...
  validateInputs() {
    ...
    this.shortcutInputs.forEach((shortcutInput, shortcut) => {
      ...
      const conflicts = UI.ShortcutRegistry.ShortcutRegistry.instance().actionsForDescriptors(userDescriptors).filter((actionId) => actionId !== this.item.id());
      if (conflicts.length) {
        confirmButton.disabled = true;
        shortcutInput.classList.add("error-input");
        UI.ARIAUtils.setInvalid(shortcutInput, true);
        ...
        errorMessageElement.textContent = i18nString(UIStrings.thisShortcutIsInUseByS, { PH1: actionCategory, PH2: actionTitle });
        return;
      }
