...
export const AI_SETTINGS_TAB_DEFAULT_VIEW = (input, _output, target) => {
  ...
  const sharedDisclaimer = html`
    ...
    (item) => html`<div><devtools-icon .name=${item.icon} class="medium"></devtools-icon>
...
export class AISettingsTab extends UI.Widget.VBox {
  ...
  constructor(view) {
    ...
    if (Root.Runtime.hostConfig.devToolsAiCodeCompletion?.enabled) {
      this.#aiCodeCompletionSetting = Common.Settings.Settings.instance().createSetting("ai-code-completion-enabled", false);
    }
  ...
  #initSettings() {
    ...
    if (this.#consoleInsightsSetting) {
      const consoleInsightsData = {
        ...
        toConsiderSettingItems: [{
          iconName: "google",
          text: noLogging ? i18nString(UIStrings.consoleInsightsSendsDataNoLogging) : i18nString(UIStrings.consoleInsightsSendsData)
        }],
        learnMoreLink: {
    ...
    if (this.#aiAssistanceSetting) {
      const aiAssistanceData = {
        ...
        toConsiderSettingItems: [{
          iconName: "google",
          text: noLogging ? i18nString(UIStrings.freestylerSendsDataNoLogging) : i18nString(UIStrings.freestylerSendsData)
        }],
        learnMoreLink: {
    ...
    if (this.#aiAnnotationsSetting) {
      const aiAnnotationsData = {
        ...
        toConsiderSettingItems: [{
          iconName: "google",
          text: noLogging ? i18nString(UIStrings.generatedAiAnnotationsSendDataNoLogging) : i18nString(UIStrings.generatedAiAnnotationsSendData)
        }],
        learnMoreLink: {
    ...
    if (this.#aiCodeCompletionSetting) {
      const settingItems = Root.Runtime.hostConfig.devToolsAiCodeGeneration?.enabled ? [
        { iconName: "code", text: i18nString(UIStrings.asYouTypeRelevantDataIsBeingSentToGoogle) },
        {
          ...
          text: Host.Platform.isMac() ? i18nString(UIStrings.describeCodeInCommentForMacOs) : i18nString(UIStrings.describeCodeInComment)
        }
      ...
      const aiCodeCompletionData = {
        ...
        toConsiderSettingItems: [{
          iconName: "google",
          text: noLogging ? i18nString(UIStrings.codeSuggestionsSendDataNoLogging) : i18nString(UIStrings.codeSuggestionsSendData)
        }],
        learnMoreLink: {
  ...
  #toggleSetting(setting, ev) {
    ...
    if (setting.name === "console-insights-enabled") {
      if (oldSettingValue) {
        ...
        Common.Settings.Settings.instance().createSetting("console-insights-skip-reminder", true, Common.Settings.SettingStorageType.SESSION).set(true);
      }
