...
export class HeapSnapshotGridNode extends Common.ObjectWrapper.eventMixin(
    ...
    if (dataGrid.snapshot && dataGrid.snapshot.totalSize !== 0) {
      ...
      if (percent) {
        ...
        this.setCellAccessibleName(
          i18nString(UIStrings.genericStringsTwoPlaceholders, { PH1: value, PH2: percent }),
          cell,
          columnId
        );
      } else {
...
export class HeapSnapshotDiffNode extends HeapSnapshotGridNode {
  ...
  constructor(dataGrid, classKey, diffForClass) {
    ...
    this.data = {
      ...
      countDelta: this.signForDelta(this.countDelta) + Platform.NumberUtilities.withThousandsSeparator(Math.abs(this.countDelta)),
