...
import { Events, HeapTimelineOverview, Samples } from "./HeapTimelineOverview.js";
...
const moduleI18nString = i18n.i18n.getLocalizedString.bind(void 0, moduleUIstr_);
export class HeapSnapshotView extends UI.View.SimpleView {
  ...
  constructor(dataDisplayDelegate, profile) {
    ...
    this.containmentDataGrid = new HeapSnapshotContainmentDataGrid(
      ...
      this,
      /* displayName */
      i18nString(UIStrings.containment)
    );
    ...
    this.constructorsWidget.element.setAttribute(
      "jslog",
      `${VisualLogging.pane("heap-snapshot.constructors-view").track({ resize: true })}`
    );
    ...
    if (isHeapTimeline && hasAllocationStacks) {
      ...
      this.allocationDataGrid.addEventListener(
        DataGrid.DataGrid.Events.SELECTED_NODE,
    ...
    this.retainmentWidget.element.setAttribute(
      "jslog",
      `${VisualLogging.pane("heap-snapshot.retaining-paths-view").track({ resize: true })}`
    );
    ...
    this.perspectiveSelect = new UI.Toolbar.ToolbarComboBox(
      ...
      i18nString(UIStrings.perspective),
      void 0,
    ...
    this.baseSelect = new UI.Toolbar.ToolbarComboBox(
      ...
      i18nString(UIStrings.baseSnapshot),
      void 0,
    ...
    this.resetRetainersButton = new UI.Toolbar.ToolbarButton(restoreIgnoredRetainers, "clear-list", restoreIgnoredRetainers);
  ...
  onProfileHeaderRemoved(event) {
    ...
    if (this.profile === profile) {
      ...
      this.profile.profileType().removeEventListener(
        ProfileTypeEvents.REMOVE_PROFILE_HEADER,
...
export class AllocationPerspective extends Perspective {
  ...
  constructor() {
    ...
    this.allocationSplitWidget = new UI.SplitWidget.SplitWidget(false, true, "heap-snapshot-allocation-split-view-state", 200, 200);
...
export class HeapSnapshotProfileType extends Common.ObjectWrapper.eventMixin(ProfileType) {
  ...
  constructor(id, title) {
    ...
    SDK.TargetManager.TargetManager.instance().addModelListener(
      ...
      SDK.HeapProfilerModel.Events.RESET_PROFILES,
      this.resetProfiles,
      this
    );
    SDK.TargetManager.TargetManager.instance().addModelListener(
      SDK.HeapProfilerModel.HeapProfilerModel,
      SDK.HeapProfilerModel.Events.ADD_HEAP_SNAPSHOT_CHUNK,
      this.addHeapSnapshotChunk,
      this
    );
    SDK.TargetManager.TargetManager.instance().addModelListener(
      SDK.HeapProfilerModel.HeapProfilerModel,
      SDK.HeapProfilerModel.Events.REPORT_HEAP_SNAPSHOT_PROGRESS,
...
export var HeapSnapshotProfileTypeEvents = /* @__PURE__ */ ((HeapSnapshotProfileTypeEvents2) => {
...
export class TrackingHeapSnapshotProfileType extends Common.ObjectWrapper.eventMixin(
  ...
  constructor() {
    ...
    this.recordAllocationStacksSettingInternal = Common.Settings.Settings.instance().createSetting("record-allocation-stacks", false);
    this.customContentInternal = null;
  ...
  modelRemoved(heapProfilerModel) {
    ...
    heapProfilerModel.removeEventListener(
      SDK.HeapProfilerModel.Events.LAST_SEEN_OBJECT_ID,
  ...
  customContent() {
    const checkboxSetting = SettingsUI.SettingsUI.createSettingCheckbox(
      i18nString(UIStrings.recordAllocationStacksExtra),
...
export var TrackingHeapSnapshotProfileTypeEvents = /* @__PURE__ */ ((TrackingHeapSnapshotProfileTypeEvents2) => {
...
export class HeapProfileHeader extends ProfileHeader {
  ...
  async saveToFile() {
    ...
    this.fileName = this.fileName || "Heap-" + Platform.DateUtilities.toISO8601Compact(/* @__PURE__ */ new Date()) + this.profileType().fileExtension();
...
export class HeapSnapshotStatisticsView extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.element.setAttribute(
      "jslog",
      `${VisualLogging.pane("profiler.heap-snapshot-statistics-view").track({ resize: true })}`
    );
  ...
  static valueFormatter(value) {
    ...
    return formatter.format(Math.round(value / 1e3));
  }
...
export class HeapAllocationStackView extends UI.Widget.Widget {
  ...
  async setAllocatedObject(snapshot, snapshotNodeIndex) {
    ...
    if (!frames) {
      ...
      UI.UIUtils.createTextChild(stackDiv2, i18nString(UIStrings.stackWasNotRecordedForThisObject));
      return;
    }
...
const stackFrameToURLElement = /* @__PURE__ */ new WeakMap();
