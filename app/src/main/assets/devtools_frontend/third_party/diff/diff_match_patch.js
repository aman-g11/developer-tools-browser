...
(function() {
  ...
  diff_match_patch.prototype.diff_compute_ = function(a, b, c, d) {
    ...
    return -1 != g ? (c = [[1, e.substring(0, g)], [0, f], [1, e.substring(g + f.length)]], a.length > b.length && (c[0][0] = c[2][0] = -1), c) : 1 == f.length ? [[-1, a], [1, b]] : (e = this.diff_halfMatch_(a, b)) ? (f = e[0], a = e[1], g = e[2], b = e[3], e = e[4], f = this.diff_main(f, g, c, d), c = this.diff_main(a, b, c, d), f.concat([[0, e]], c)) : c && 100 < a.length && 100 < b.length ? this.diff_lineMode_(
      a,
      b,
      d
  ...
  diff_match_patch.prototype.diff_cleanupSemantic = function(a) {
    for (var b = false, c = [], d = 0, e = null, f = 0, g = 0, h = 0, j = 0, i = 0; f < a.length; ) 0 == a[f][0] ? (c[d++] = f, g = j, h = i, i = j = 0, e = a[f][1]) : (1 == a[f][0] ? j += a[f][1].length : i += a[f][1].length, e && (e.length <= Math.max(g, h) && e.length <= Math.max(j, i)) && (a.splice(c[d - 1], 0, [-1, e]), a[c[d - 1] + 1][0] = 1, d--, d--, f = 0 < d ? c[d - 1] : -1, i = j = h = g = 0, e = null, b = true)), f++;
  ...
  diff_match_patch.prototype.diff_cleanupSemanticLossless = function(a) {
    function b(a2, b2) {
      if (!a2 || !b2) return 6;
      var c2 = a2.charAt(a2.length - 1), d2 = b2.charAt(0), e2 = c2.match(diff_match_patch.nonAlphaNumericRegex_), f2 = d2.match(diff_match_patch.nonAlphaNumericRegex_), g2 = e2 && c2.match(diff_match_patch.whitespaceRegex_), h2 = f2 && d2.match(diff_match_patch.whitespaceRegex_), c2 = g2 && c2.match(diff_match_patch.linebreakRegex_), d2 = h2 && d2.match(diff_match_patch.linebreakRegex_), i2 = c2 && a2.match(diff_match_patch.blanklineEndRegex_), j2 = d2 && b2.match(diff_match_patch.blanklineStartRegex_);
  ...
  diff_match_patch.prototype.diff_cleanupEfficiency = function(a) {
    for (var b = false, c = [], d = 0, e = null, f = 0, g = false, h = false, j = false, i = false; f < a.length; ) {
      if (0 == a[f][0]) a[f][1].length < this.Diff_EditCost && (j || i) ? (c[d++] = f, g = j, h = i, e = a[f][1]) : (d = 0, e = null), j = i = false;
      else if (-1 == a[f][0] ? i = true : j = true, e && (g && h && j && i || e.length < this.Diff_EditCost / 2 && 3 == g + h + j + i)) a.splice(c[d - 1], 0, [-1, e]), a[c[d - 1] + 1][0] = 1, d--, e = null, g && h ? (j = i = true, d = 0) : (d--, f = 0 < d ? c[d - 1] : -1, j = i = false), b = true;
      f++;
    }
    b && this.diff_cleanupMerge(a);
  };
  diff_match_patch.prototype.diff_cleanupMerge = function(a) {
    ...
    c = false;
    for (b = 1; b < a.length - 1; ) 0 == a[b - 1][0] && 0 == a[b + 1][0] && (a[b][1].substring(a[b][1].length - a[b - 1][1].length) == a[b - 1][1] ? (a[b][1] = a[b - 1][1] + a[b][1].substring(0, a[b][1].length - a[b - 1][1].length), a[b + 1][1] = a[b - 1][1] + a[b + 1][1], a.splice(b - 1, 1), c = true) : a[b][1].substring(
      0,
      a[b + 1][1].length
    ) == a[b + 1][1] && (a[b - 1][1] += a[b + 1][1], a[b][1] = a[b][1].substring(a[b + 1][1].length) + a[b + 1][1], a.splice(b + 1, 1), c = true)), b++;
  ...
  diff_match_patch.prototype.patch_make = function(a, b, c) {
    var d;
    if ("string" == typeof a && "string" == typeof b && "undefined" == typeof c) d = a, b = this.diff_main(d, b, true), 2 < b.length && (this.diff_cleanupSemantic(b), this.diff_cleanupEfficiency(b));
  ...
  diff_match_patch.prototype.patch_apply = function(a, b) {
    ...
    for (var d = 0, e = [], f = 0; f < a.length; f++) {
      ...
      if (-1 == j) e[f] = false, d -= a[f].length2 - a[f].length1;
      else if (e[f] = true, d = j - g, g = -1 == i ? b.substring(j, j + h.length) : b.substring(j, i + this.Match_MaxBits), h == g) b = b.substring(0, j) + this.diff_text2(a[f].diffs) + b.substring(j + h.length);
      else if (g = this.diff_main(h, g, false), h.length > this.Match_MaxBits && this.diff_levenshtein(g) / h.length > this.Patch_DeleteThreshold) e[f] = false;
      else {
        ...
        for (var h = 0, k, i = 0; i < a[f].diffs.length; i++) {
          ...
          1 === q[0] ? b = b.substring(
            0,
            j + k
          ) + q[1] + b.substring(j + k) : -1 === q[0] && (b = b.substring(0, j + k) + b.substring(j + this.diff_xIndex(g, h + q[1].length)));
          -1 !== q[0] && (h += q[1].length);
        }
      }
    }
  ...
  diff_match_patch.prototype.patch_splitMax = function(a) {
    for (var b = this.Match_MaxBits, c = 0; c < a.length; c++) if (!(a[c].length1 <= b)) {
      ...
      for (var e = d.start1, f = d.start2, g = ""; 0 !== d.diffs.length; ) {
        var h = new diff_match_patch.patch_obj(), j = true;
        h.start1 = e - g.length;
        h.start2 = f - g.length;
