...
class GreenDevFloaty {
  ...
  static instance(opts = { forceNew: null, document }) {
    ...
    if (!greenDevFloatyInstance || forceNew) {
      greenDevFloatyInstance = new GreenDevFloaty(document2);
    }
  ...
  setNode(node) {
    if (this.#node) {
      this.#node.domModel().overlayModel().removeEventListener(
        SDK.OverlayModel.Events.INSPECT_PANEL_SHOW_REQUESTED,
        this.handlePanelRequest
      );
    }
    if (this.#node === node) {
      return;
      ...
      await SDK.Connections.initMainConnection(async () => {
        ...
        if (overlayModel) {
          overlayModel.addEventListener(
            SDK.OverlayModel.Events.INSPECTED_ELEMENT_WINDOW_RESTORED,
            floaty.handleRestoreEvent,
            floaty
          );
        }
