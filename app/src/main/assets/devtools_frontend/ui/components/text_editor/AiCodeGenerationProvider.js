...
export var AiCodeGenerationTeaserMode = /* @__PURE__ */ ((AiCodeGenerationTeaserMode2) => {
...
export class AiCodeGenerationProvider {
  ...
  dispose() {
    ...
    Host.AidaClient.HostConfigTracker.instance().removeEventListener(
      ...
      this.#boundOnUpdateAiCodeGenerationState
    );
  }
...
function aiCodeGenerationTeaserExtension(teaser) {
  return CodeMirror.ViewPlugin.fromClass(class {
    ...
    eventHandlers: {
      ...
      keydown(event) {
        if (!UI.KeyboardShortcut.KeyboardShortcut.eventHasCtrlEquivalentKey(event) || teaser.displayState !== PanelCommon.AiCodeGenerationTeaser.AiCodeGenerationTeaserDisplayState.TRIGGER) {
          return false;
        }
        if (event.key === ".") {
          event.consume(true);
