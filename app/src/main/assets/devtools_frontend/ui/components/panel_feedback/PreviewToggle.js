  ...
  #render() {
    ...
    render(
      ...
      <div class="container">
        ...
        ${this.#feedbackURL && !this.#helperText ? html`<div class="feedback"><devtools-link class="devtools-link" href=${this.#feedbackURL} jslogContext=${"feedback"}>${i18nString(UIStrings.shortFeedbackLink)}</devtools-link></div>` : nothing}
        ${this.#learnMoreURL ? html`<div class="learn-more"><devtools-link class="devtools-link" href=${this.#learnMoreURL} jslogContext=${"learn-more"}>${i18nString(UIStrings.learnMoreLink)}</devtools-link></div>` : nothing}
        <div class="helper">
          ${this.#helperText && this.#feedbackURL ? html`<p>${this.#helperText} <devtools-link class="devtools-link" href=${this.#feedbackURL} jslogContext=${"feedback"}>${i18nString(UIStrings.previewTextFeedbackLink)}</devtools-link></p>` : nothing}
        </div>
      </div>`,
      this.#shadow,
      {
