...
export function installDragHandle(element, elementDragStart2, elementDrag, elementDragEnd, cursor, hoverCursor, startDelay, mouseDownPreventDefault = true) {
...
export function elementDragStart(targetElement, elementDragStart2, elementDrag, elementDragEnd, cursor, event) {
  const dragHandler = new DragHandler();
...
const elementsBeingEdited = /* @__PURE__ */ new Set();
...
export function animateFunction(window2, func, params, duration, animationComplete) {
...
export function initializeUIUtils(document2) {
...
export function createInput(className, type, jslogContext) {
  ...
  if (jslogContext) {
    element.setAttribute(
      "jslog",
      `${VisualLogging.textField().track({ keydown: "Enter", change: true }).context(jslogContext)}`
    );
  }
  return element;
}
...
export class CheckboxLabel extends HTMLElement {
  ...
  static createWithStringLiteral(title, checked, jslogContext, small) {
    ...
    return CheckboxLabel.create(stringLiteral, checked, void 0, jslogContext, small);
  }
  static lastId = 0;
}
...
export function trimTextMiddle(context, text, maxWidth) {
  return trimText(context, text, maxWidth, (text2, width) => Platform.StringUtilities.trimMiddle(text2, width));
}
export function trimTextEnd(context, text, maxWidth) {
  return trimText(context, text, maxWidth, (text2, width) => Platform.StringUtilities.trimEndWithMaxLength(text2, width));
}
...
export class ConfirmDialog {
  static async show(message, header, where, options) {
    ...
    const result = await new Promise((resolve) => {
      const okButton = createTextButton(
        /* text= */
        options?.okButtonLabel || i18nString(UIStrings.ok),
        /* clickHandler= */
        () => resolve(true),
        { jslogContext: "confirm", variant: Buttons.Button.Variant.PRIMARY }
      );
      ...
      buttonsBar.appendChild(createTextButton(
        options?.cancelButtonLabel || i18nString(UIStrings.cancel),
        () => resolve(false),
        { jslogContext: "cancel" }
      ));
      dialog.setOutsideClickCallback((event) => {
...
export const deepElementFromPoint = (document2, x, y) => {
...
export function measuredScrollbarWidth(document2) {
...
const MAX_DURATION = 60 * 24 * 60 * 60 * 1e3;
...
export const bindToSetting = (settingOrName, optionsOrValidator) => {
  const setting = typeof settingOrName === "string" ? Common.Settings.Settings.instance().moduleSetting(settingOrName) : settingOrName;
  let stringValidator;
  let jslog = true;
