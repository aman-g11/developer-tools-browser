...
const nodeToParentTreeElementMap = /* @__PURE__ */ new WeakMap();
const { render } = Lit;
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["ElementAttached"] = "ElementAttached";
  Events2["ElementsDetached"] = "ElementsDetached";
  Events2["ElementExpanded"] = "ElementExpanded";
  Events2["ElementCollapsed"] = "ElementCollapsed";
  Events2["ElementSelected"] = "ElementSelected";
  return Events2;
})(Events || {});
...
export var TreeVariant = /* @__PURE__ */ ((TreeVariant2) => {
...
export class TreeOutlineInShadow extends TreeOutline {
  ...
  disclosureElement;
  renderSelection;
  constructor(variant = "Other" /* OTHER */, element) {
    super();
...
export const treeElementBylistItemNode = /* @__PURE__ */ new WeakMap();
export class TreeElement {
  ...
  constructor(title, expandable, jslogContext) {
    ...
    this.listItemNode.setAttribute(
      "jslog",
      `${VisualLogging.treeItem().parent("parentTreeItem").context(jslogContext).track({
        click: true,
        resize: true,
...
class TreeViewTreeElement extends TreeElement {
  static CLONED_ATTRIBUTES = SDK.DOMModel.ARIA_ATTRIBUTES.union(/* @__PURE__ */ new Set(["jslog"]));
  ...
  remove() {
    removeNode(
      this,
      Boolean(this.parent && this.parent.configElement?.querySelector('ul[role="group"]'))
    );
    ...
    for (const node of getTreeNodes(nodes)) {
      if (node instanceof HTMLLIElement) {
        ...
        removeNode(
          node.treeElement,
          Boolean(
            node.treeElement.parent && node.treeElement.parent.configElement?.querySelector('ul[role="group"]')
          )
        );
      }
    }
