...
export class ContextMenu extends SubMenu {
  ...
  #show() {
    ...
    let useSoftMenu = this.useSoftMenu || ContextMenu.useSoftMenu || Host.InspectorFrontendHost.InspectorFrontendHostInstance.isHostedMode();
    ...
    if (this.openHostedMenu) {
      const itemWithId = (items, id2) => {
        for (const item2 of items) {
          if (item2.id === id2) {
            if (item2.featureName) {
              featuresUsed.push(item2.featureName);
            }
            return item2;
          }
