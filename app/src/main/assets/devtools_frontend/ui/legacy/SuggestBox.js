...
export class SuggestBox {
  ...
  applySuggestion(isIntermediateSuggestion) {
    if (this.onlyCompletion) {
      isIntermediateSuggestion ? ARIAUtils.LiveAnnouncer.alert(i18nString(
    ...
    if (suggestion?.text) {
      isIntermediateSuggestion ? ARIAUtils.LiveAnnouncer.alert(i18nString(UIStrings.sSuggestionSOfS, {
        ...
        UIStrings.sSuggestionSSelected,
        { PH1: suggestion.title || suggestion.text }
      ));
    }
  ...
  createElementForItem(item) {
    ...
    const displayText = Platform.StringUtilities.trimEndWithMaxLength((item.title || item.text).trim(), maxTextLength).replace(/\n/g, "\u21B5");
    ...
    if (item.subtitleRenderer) {
      ...
      subtitleElement.textContent = Platform.StringUtilities.trimEndWithMaxLength(item.subtitle, maxTextLength - displayText.length);
    }
    if (item.iconElement) {
