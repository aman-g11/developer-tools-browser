...
export class ListWidget extends VBox {
  ...
  startEditing(item, element, insertionPoint) {
    ...
    this.editor.beginEdit(
      item,
      index,
      element ? i18nString(UIStrings.saveString) : i18nString(UIStrings.addString),
...
export class Editor {
  ...
  constructor() {
    ...
    this.element.addEventListener(
      "keydown",
      onKeyDown.bind(null, Platform.KeyboardUtilities.isEscKey, this.cancelClicked.bind(this)),
      false
    );
  ...
  createSelect(name, options, validator, title) {
    ...
    for (let index = 0; index < options.length; ++index) {
      ...
      option.setAttribute(
        "jslog",
        `${VisualLogging.item(Platform.StringUtilities.toKebabCase(options[index])).track({ click: true })}`
      );
    }
    if (title) {
