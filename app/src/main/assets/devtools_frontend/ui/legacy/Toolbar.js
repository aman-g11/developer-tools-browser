...
export class Toolbar extends HTMLElement {
  ...
  onItemsChange(mutationList) {
    for (const mutation of mutationList) {
      ...
      for (const element of mutation.addedNodes) {
        ...
        if (this.items.some((item2) => item2.element === element)) {
          continue;
        }
        let item;
        if (element instanceof Buttons.Button.Button) {
          item = new ToolbarButton("", void 0, void 0, void 0, element);
        } else if (element instanceof ToolbarInputElement) {
          item = element.item;
        } else if (element instanceof HTMLSelectElement) {
          item = new ToolbarComboBox(null, element.title, void 0, void 0, element);
        } else {
  ...
  static createActionButton(actionOrActionId, options = {}) {
    ...
    return button;
    function makeButton() {
      const button2 = new ToolbarButton(action.title(), action.icon(), void 0, action.id());
      if (action.title()) {
        Tooltip.installWithActionBinding(button2.element, action.title(), action.id());
      }
      return button2;
    }
    function makeToggle() {
...
export var ToolbarItemWithCompactLayoutEvents = /* @__PURE__ */ ((ToolbarItemWithCompactLayoutEvents2) => {
...
((ToolbarButton2) => {
  ...
  ((Events2) => {
    Events2["CLICK"] = "Click";
  })(Events = ToolbarButton2.Events || (ToolbarButton2.Events = {}));
...
((ToolbarInput2) => {
  let Event;
  ((Event2) => {
    Event2["TEXT_CHANGED"] = "TextChanged";
    Event2["ENTER_PRESSED"] = "EnterPressed";
  })(Event = ToolbarInput2.Event || (ToolbarInput2.Event = {}));
...
export class ToolbarComboBox extends ToolbarItem {
  ...
  createOption(label, value, jslogContext) {
    ...
    if (!jslogContext) {
      jslogContext = value ? Platform.StringUtilities.toKebabCase(value) : void 0;
    }
...
export var ToolbarItemLocation = /* @__PURE__ */ ((ToolbarItemLocation2) => {
