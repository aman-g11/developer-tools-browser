  ...
  async handleKey(key, domKey, event, handlers) {
    ...
    function isPossiblyInputKey() {
      ...
      if (Host.Platform.isMac()) {
        ...
        if (!Host.Platform.isWin() && KeyboardShortcut.makeKey("z", modifiers.Ctrl.value | modifiers.Shift.value) === key) {
          return true;
        }
      }
    ...
    async function maybeExecuteActionForKey(event2) {
      ...
      for (const action of actions2) {
        let handled;
        if (event2) {
          void VisualLogging.logKeyDown(null, event2, action.id());
        }
...
export const ForwardedActions = /* @__PURE__ */ new Set([
  "main.toggle-dock",
