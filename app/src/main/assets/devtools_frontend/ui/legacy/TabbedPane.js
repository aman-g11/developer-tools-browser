...
export class TabbedPane extends Common.ObjectWrapper.eventMixin(VBox) {
  ...
  appendTab(id, tabTitle, view, tabTooltip, userGesture, isCloseable, isPreviewFeature, index, jslogContext) {
    ...
    tab.tabElement.setAttribute(
      "jslog",
      `${VisualLogging.panelTabHeader().track({ click: true, drag: true }).context(tab.jslogContext)}`
    );
    if (index !== void 0) {
...
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["TabInvoked"] = "TabInvoked";
  Events2["TabSelected"] = "TabSelected";
  Events2["TabClosed"] = "TabClosed";
  Events2["TabOrderChanged"] = "TabOrderChanged";
  Events2["PaneVisibilityChanged"] = "PaneVisibilityChanged";
  return Events2;
})(Events || {});
export class TabbedPaneTab {
  ...
  tabContextMenu(event) {
    ...
    if (tabIndex > 0) {
      contextMenu.defaultSection().appendItem(
        i18nString(UIStrings.moveTabLeft),
    ...
    if (tabIndex < this.tabbedPane.tabsElement.childNodes.length - 1) {
      contextMenu.defaultSection().appendItem(
        i18nString(UIStrings.moveTabRight),
...
const tabIcons = /* @__PURE__ */ new WeakMap();
const tabSuffixElements = /* @__PURE__ */ new WeakMap();
