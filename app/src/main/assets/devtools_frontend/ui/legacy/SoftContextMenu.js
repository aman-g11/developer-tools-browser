...
export class SoftContextMenu {
    ...
    if (item.jslogContext && item.label) {
      if (item.type === "checkbox") {
        menuItemElement.setAttribute(
          "jslog",
          `${VisualLogging.toggle().track({ click: true }).context(item.jslogContext)}`
        );
      } else {
        menuItemElement.setAttribute(
          "jslog",
          `${VisualLogging.action().track({ click: true }).context(item.jslogContext)}`
        );
      }
    }
  ...
  createSubMenu(item, menuContainsCheckbox) {
    ...
    if (item.jslogContext) {
      menuItemElement.setAttribute(
        "jslog",
        `${VisualLogging.item(item.jslogContext).track({ click: true, resize: true })}`
      );
    }
    return menuItemElement;
  }
  ...
  setChecked(item, checked) {
    ...
    const checkedState = item.checked ? i18nString(UIStrings.checked) : i18nString(UIStrings.unchecked);
    const accessibleName = item.shortcut ? i18nString(UIStrings.sSS, { PH1: String(item.label), PH2: item.shortcut, PH3: checkedState }) : i18nString(UIStrings.sS, { PH1: String(item.label), PH2: checkedState });
  ...
  triggerAction(menuItemElement, event) {
    ...
    if (this.keepOpen) {
      event.consume(true);
      const item = this.items.find((item2) => item2.id === detailsForElement.actionId);
      if (item?.id !== void 0) {
