...
var SpectrumGamut = /* @__PURE__ */ ((SpectrumGamut2) => {
  SpectrumGamut2["SRGB"] = "srgb";
  SpectrumGamut2["DISPLAY_P3"] = "display-p3";
  return SpectrumGamut2;
})(SpectrumGamut || {});
...
export class Spectrum extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(contrastInfo) {
    ...
    this.colorPickerButton = new UI.Toolbar.ToolbarToggle(
      i18nString(UIStrings.toggleColorPicker, { PH1: definedShortcutKey || "" }),
      "color-picker",
      "color-picker-filled",
      "color-eye-dropper"
    );
    ...
    const addColorButton = new UI.Toolbar.ToolbarButton(i18nString(UIStrings.addToPalette), "plus", void 0, "add-color");
  ...
  loadPalettes() {
    ...
    this.customPaletteSetting = Common.Settings.Settings.instance().createSetting("custom-color-palette", defaultCustomPalette);
    ...
    this.selectedColorPalette = Common.Settings.Settings.instance().createSetting("selected-color-palette", GeneratedPaletteTitle);
  ...
  updateInput() {
    if (this.colorFormat === Common.Color.Format.HEX) {
      ...
      this.hexValue.value = this.color.asString((this.color.alpha ?? 1) !== 1 ? Common.Color.Format.HEXA : Common.Color.Format.HEX);
    } else {
  ...
  updateUI() {
    ...
    this.alphaElementBackground.style.backgroundImage = Platform.StringUtilities.sprintf(
      ...
      noAlpha.asString(Common.Color.Format.LCH)
    );
  ...
  #getValueSteppingForInput(element) {
    ...
    switch (channel) {
      ...
      case Common.Color.ColorChannel.B:
        return this.color instanceof Common.Color.ColorFunction ? { step: 0.01, range: { min: 0, max: 1 } } : { step: 1, range: { min: 0, max: 255 } };
...
export var Events = /* @__PURE__ */ ((Events2) => {
  ...
  Events2["SIZE_CHANGED"] = "SizeChanged";
  return Events2;
})(Events || {});
...
export class PaletteGenerator {
  ...
  finish() {
    function hueComparator(a, b) {
      const hsva = paletteColors.get(a).as(Common.Color.Format.HSL).hsva();
      const hsvb = paletteColors.get(b).as(Common.Color.Format.HSL).hsva();
