...
export class CommandMenu {
  ...
  static createRevealViewCommand(options) {
    ...
    const executeHandler = () => {
      ...
      return UI.ViewManager.ViewManager.instance().showView(
        id,
        /* userGesture */
        true
      );
...
export var PanelOrDrawer = /* @__PURE__ */ ((PanelOrDrawer2) => {
...
export class CommandMenuProvider extends Provider {
  ...
  renderItem(itemIndex, query) {
    const command = this.commands[itemIndex];
    const badge = command.featurePromotionId ? UI.UIUtils.maybeCreateNewBadge(command.featurePromotionId) : void 0;
