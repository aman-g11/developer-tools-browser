...
export class Linkifier extends Common.ObjectWrapper.ObjectWrapper {
  ...
  maybeLinkifyScriptLocation(target, scriptId, sourceURL, lineNumber, options) {
    ...
    const rawLocation = scriptId ? debuggerModel.createRawLocationByScriptId(
      ...
      lineNumber || 0,
      columnNumber,
      linkifyURLOptions.inlineFrameIndex
  ...
  static linkHandlerSetting() {
    if (!linkHandlerSettingInstance) {
      linkHandlerSettingInstance = Common.Settings.Settings.instance().createSetting("open-link-handler", i18nString(UIStrings.auto));
    }
...
export class ContentProviderContextMenuProvider {
  appendApplicableItems(_event, contextMenu, contentProvider) {
    ...
    if (!Common.ParsedURL.schemeIs(contentUrl, "file:")) {
      contextMenu.revealSection().appendItem(
        ...
        () => Host.InspectorFrontendHost.InspectorFrontendHostInstance.openInNewTab(
          contentUrl.endsWith(":formatted") ? Common.ParsedURL.ParsedURL.slice(contentUrl, 0, contentUrl.lastIndexOf(":")) : contentUrl
        ),
        { jslogContext: "open-in-new-tab" }
      );
    }
    for (const origin of linkHandlers.keys()) {
      ...
      contextMenu.revealSection().appendItem(
        i18nString(UIStrings.openUsingS, { PH1: title }),
