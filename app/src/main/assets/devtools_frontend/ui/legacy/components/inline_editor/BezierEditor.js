...
export class BezierEditor extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(model) {
    ...
    this.contentElement.setAttribute(
      "jslog",
      `${VisualLogging.dialog("bezierEditor").parent("mapped").track({ keydown: "Enter|Escape" })}`
    );
    ...
    this.animationTimingUI.element().setAttribute(
      "jslog",
      `${VisualLogging.bezierCurveEditor().track({ click: true, drag: true })}`
    );
  ...
  createCategory(presetGroup) {
    ...
    presetElement.setAttribute(
      "jslog",
      `${VisualLogging.bezierPresetCategory().track({ click: true }).context(presetGroup[0].name)}`
    );
...
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["BEZIER_CHANGED"] = "BezierChanged";
  return Events2;
})(Events || {});
