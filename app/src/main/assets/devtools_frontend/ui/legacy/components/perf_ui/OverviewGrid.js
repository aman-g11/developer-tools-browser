...
const WindowScrollSpeedFactor = 0.3;
...
export class Window extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor(parentElement, dividersLabelBarElement, calculator) {
    ...
    UI.UIUtils.installDragHandle(
      ...
      this.endWindowSelectorDragging.bind(this),
      "text",
      null
    );
    ...
    this.createBreadcrumbButton.setAttribute(
      "jslog",
      `${VisualLogging.action("timeline.create-breadcrumb").track({ click: true })}`
    );
    this.reset();
  }
  enableCreateBreadcrumbsButton() {
  ...
  setWindowRatio(windowLeftRatio, windowRightRatio) {
    ...
    this.updateCurtains();
    if (this.calculator) {
      this.dispatchEventToListeners("WindowChangedWithPosition" /* WINDOW_CHANGED_WITH_POSITION */, this.calculateWindowValue());
    }
    this.dispatchEventToListeners("WindowChanged" /* WINDOW_CHANGED */);
  ...
  #createBreadcrumb() {
    this.dispatchEventToListeners("BreadcrumbAdded" /* BREADCRUMB_ADDED */, this.calculateWindowValue());
  }
  updateCurtains() {
...
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["WINDOW_CHANGED"] = "WindowChanged";
  Events2["WINDOW_CHANGED_WITH_POSITION"] = "WindowChangedWithPosition";
  Events2["BREADCRUMB_ADDED"] = "BreadcrumbAdded";
  return Events2;
})(Events || {});
