...
export var HoverType = /* @__PURE__ */ ((HoverType2) => {
...
export var GroupCollapsibleState = /* @__PURE__ */ ((GroupCollapsibleState2) => {
  ...
  return GroupCollapsibleState2;
})(GroupCollapsibleState || {});
export var FilterAction = /* @__PURE__ */ ((FilterAction2) => {
...
export class FlameChart extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor(dataProvider, flameChartDelegate, optionalConfig = {}) {
    ...
    this.chartViewport.setWindowTimes(
      dataProvider.minimumBoundary(),
      dataProvider.minimumBoundary() + dataProvider.totalTime()
    );
  ...
  highlightEntry(entryIndex) {
    ...
    this.dispatchEventToListeners("EntryHovered" /* ENTRY_HOVERED */, entryIndex);
  }
  hideHighlight() {
    ...
    this.highlightedEntryIndex = -1;
    this.updateElementPosition(this.highlightElement, this.highlightedEntryIndex);
    this.dispatchEventToListeners("EntryHovered" /* ENTRY_HOVERED */, -1);
  }
  ...
  onMouseMove(mouseEvent) {
    ...
    this.dispatchEventToListeners("MouseMove" /* MOUSE_MOVE */, {
      mouseEvent,
    ...
    switch (hoverType) {
      case "TRACK_CONFIG_UP_BUTTON" /* TRACK_CONFIG_UP_BUTTON */:
      case "TRACK_CONFIG_DOWN_BUTTON" /* TRACK_CONFIG_DOWN_BUTTON */:
      case "TRACK_CONFIG_HIDE_BUTTON" /* TRACK_CONFIG_HIDE_BUTTON */:
      case "TRACK_CONFIG_SHOW_BUTTON" /* TRACK_CONFIG_SHOW_BUTTON */: {
        this.hideHighlight();
      ...
      case "INSIDE_TRACK_HEADER" /* INSIDE_TRACK_HEADER */:
        this.updateHighlight();
        this.viewportElement.style.cursor = "pointer";
        return;
      case "INSIDE_TRACK" /* INSIDE_TRACK */:
      case "OUTSIDE_TRACKS" /* OUTSIDE_TRACKS */:
        this.updateHighlight();
        return;
      case "ERROR" /* ERROR */:
        return;
  ...
  updateHighlight() {
    ...
    if (entryIndex === -1) {
      ...
      if (hoverType === "INSIDE_TRACK_HEADER" /* INSIDE_TRACK_HEADER */) {
  ...
  #onDblClick(mouseEvent) {
    ...
    if (this.highlightedEntryIndex !== -1) {
      ...
      this.dispatchEventToListeners(
        ...
        { entryIndex: this.highlightedEntryIndex, withLinkCreationButton: true }
      );
    ...
    switch (hoverType) {
      ...
      case "OUTSIDE_TRACKS" /* OUTSIDE_TRACKS */: {
        ...
        if (mouseEvent.shiftKey && this.highlightedEntryIndex !== -1 && timelineData) {
          ...
          this.dispatchEventToListeners("EntryInvoked" /* ENTRY_INVOKED */, this.highlightedEntryIndex);
        }
        return;
      }
    }
  ...
  onContextMenu(event) {
    ...
    if (hoverType === "INSIDE_TRACK_HEADER" /* INSIDE_TRACK_HEADER */ && this.#hasTrackConfigurationMode()) {
    ...
    if (!isFakedFromKeyboardPress) {
      this.dispatchEventToListeners("EntryInvoked" /* ENTRY_INVOKED */, entryIndexToUse);
  ...
  handleSelectionNavigation(event) {
    ...
    if (keyboardEvent.keyCode === keys.Left.code || keyboardEvent.keyCode === keys.Right.code) {
      ...
      if (indexOnLevel >= 0 && indexOnLevel < levelIndexes.length) {
        this.dispatchEventToListeners("EntrySelected" /* ENTRY_SELECTED */, levelIndexes[indexOnLevel]);
      }
      return true;
    }
    ...
    if (event.key === "Enter") {
      event.consume(true);
      this.dispatchEventToListeners("EntryInvoked" /* ENTRY_INVOKED */, this.selectedEntryIndex);
      this.dispatchEventToListeners("EntryLabelAnnotationAdded" /* ENTRY_LABEL_ANNOTATION_ADDED */, {
  ...
  coordinatesToEntryIndex(x, y) {
    ...
    const indexOnLevel = Math.max(
      Platform.ArrayUtilities.upperBound(
        entriesOnLevel,
        cursorTime,
        (time, entryIndex2) => time - entryStartTimes[entryIndex2]
      ) - 1,
      0
    );
  ...
  isMouseOverRevealChildrenArrow(x, index) {
    if (!this.entryHasDecoration(index, "HIDDEN_DESCENDANTS_ARROW" /* HIDDEN_DESCENDANTS_ARROW */)) {
      return false;
    }
    const timelineData = this.timelineData();
    if (!timelineData) {
  ...
  coordinatesToGroupIndexAndHoverType(x, y) {
    if (!this.rawTimelineData?.groups || !this.groupOffsets) {
      return { groupIndex: -1, hoverType: "ERROR" /* ERROR */ };
    }
    if (x < 0 || y < 0) {
      return { groupIndex: -1, hoverType: "ERROR" /* ERROR */ };
    }
          ...
          if (this.#inTrackConfigEditMode) {
            if (mouseInHeaderRow) {
              if (UP_ICON_LEFT <= x && x < UP_ICON_LEFT + EDIT_ICON_WIDTH) {
                return { groupIndex, hoverType: "TRACK_CONFIG_UP_BUTTON" /* TRACK_CONFIG_UP_BUTTON */ };
              }
              if (DOWN_ICON_LEFT <= x && x < DOWN_ICON_LEFT + EDIT_ICON_WIDTH) {
                return { groupIndex, hoverType: "TRACK_CONFIG_DOWN_BUTTON" /* TRACK_CONFIG_DOWN_BUTTON */ };
              }
              if (HIDE_ICON_LEFT <= x && x < HIDE_ICON_LEFT + EDIT_ICON_WIDTH) {
                return {
                  groupIndex,
                  hoverType: groups[groupIndex].hidden ? "TRACK_CONFIG_SHOW_BUTTON" /* TRACK_CONFIG_SHOW_BUTTON */ : "TRACK_CONFIG_HIDE_BUTTON" /* TRACK_CONFIG_HIDE_BUTTON */
                };
              }
              if (mouseInHeaderRow && x <= headerRight) {
                return { groupIndex, hoverType: "INSIDE_TRACK_HEADER" /* INSIDE_TRACK_HEADER */ };
              }
            }
          } else {
            if (mouseInHeaderRow && x <= headerRight) {
            ...
            return { groupIndex, hoverType: "INSIDE_TRACK" /* INSIDE_TRACK */ };
          }
        }
      }
    }
    return { groupIndex: -1, hoverType: "OUTSIDE_TRACKS" /* OUTSIDE_TRACKS */ };
  }
  enterTrackConfigurationMode() {
    ...
    this.dispatchEventToListeners("TracksReorderStateChange" /* TRACKS_REORDER_STATE_CHANGED */, true);
    this.updateLevelPositions();
    this.draw();
    this.scrollGroupIntoView(0);
  }
  ...
  #exitEditMode() {
    ...
    this.dispatchEventToListeners("TracksReorderStateChange" /* TRACKS_REORDER_STATE_CHANGED */, false);
    this.updateLevelPositions();
    this.draw();
  }
  markerIndexBeforeTime(time) {
    ...
    return Platform.ArrayUtilities.lowerBound(
      ...
      time,
      (markerTimestamp, marker) => markerTimestamp - marker.startTime()
    );
  }
  /**
  ...
  draw() {
    ...
    this.resetCanvas();
    this.dispatchEventToListeners("LatestDrawDimensions" /* LATEST_DRAW_DIMENSIONS */, {
      chart: {
    ...
    for (let i = 0; i < indexes.length; ++i) {
      ...
      for (const decoration of decorationsForEvent) {
        switch (decoration.type) {
          ...
          case "HIDDEN_DESCENDANTS_ARROW" /* HIDDEN_DESCENDANTS_ARROW */: {
            context.save();
            context.beginPath();
            context.rect(barX, barY, barWidth, barHeight);
  ...
  getDrawBatches(context, timelineData) {
    ...
    for (let level = 0; level < this.dataProvider.maxStackDepth(); ++level) {
      ...
      const rightIndexOnLevel = Platform.ArrayUtilities.lowerBound(
        levelIndexes,
        this.chartViewport.windowRightTime(),
  ...
  drawGroupHeaders(width, height) {
    ...
    this.forEachGroupInViewport((offset, groupIndex, group) => {
      ...
      if (this.isGroupCollapsible(groupIndex) && !group.expanded || group.style.shareHeaderLine) {
        ...
        context.fillRect(
          iconsWidth + HEADER_LEFT_PADDING,
          offset + HEADER_LABEL_Y_PADDING,
          labelBackgroundWidth,
          group.style.height - 2 * HEADER_LABEL_Y_PADDING
        );
      }
      context.fillStyle = this.#inTrackConfigEditMode && group.hidden ? ThemeSupport.ThemeSupport.instance().getComputedValue("--sys-color-token-subtle", this.contentElement) : group.style.color;
      ...
      if (this.#inTrackConfigEditMode && group.hidden) {
        context.fillRect(
          ...
          UI.UIUtils.measureTextWidth(context, group.name),
          1
        );
      }
      if (trackConfigurationAllowed) {
        if (this.#inTrackConfigEditMode) {
          ...
          drawIcon(
            ...
            offset,
            EDIT_ICON_WIDTH,
            group.hidden ? showIconPath : hideIconPath,
    ...
    this.forEachGroupInViewport((offset, index, group) => {
      if (this.isGroupCollapsible(index)) {
        drawExpansionArrow(
          context,
          iconsWidth + EXPANSION_ARROW_INDENT * (group.style.nestingLevel + 1),
    ...
    this.forEachGroupInViewport((offset, index, group, _isFirst, groupHeight) => {
      if (this.isGroupFocused(index)) {
        ...
        context.fillStyle = ThemeSupport.ThemeSupport.instance().getComputedValue("--selected-group-border", this.contentElement);
  ...
  #drawCustomSymbols(context, timelineData) {
    ...
    for (const [entryIndex, drawOverride] of this.#indexToDrawOverride.entries()) {
      ...
      const pos = drawOverride(
        ...
        width,
        height,
        (time) => this.chartViewport.timeToPosition(time),
      ...
      for (let entryIndexOnLevel = rightIndexOnLevel; entryIndexOnLevel >= 0; --entryIndexOnLevel) {
        ...
        if (group.style.useDecoratorsForOverview && this.dataProvider.forceDecoration(entryIndex)) {
          ...
          this.dataProvider.decorateEntry(
            ...
            unclippedBarX,
            timeToPixel,
            (color2) => this.#transformColor(entryIndex, color2)
          );
          continue;
        }
  ...
  drawCircleAroundCollapseArrow(entryIndex, context, timelineData) {
    ...
    if (!decorationsForEvent?.find(
      (decoration) => decoration.type === "HIDDEN_DESCENDANTS_ARROW" /* HIDDEN_DESCENDANTS_ARROW */
    )) {
      return {};
    }
    const { entryStartTimes, entryLevels } = timelineData;
  ...
  updateGroupTree(groups, root) {
    ...
    function traverse(treeNode) {
      ...
      if (index < 0) {
        ...
        if (!groups[index]) {
          console.warn(
            "The |groups| is changed. Please make sure the flamechart is reset after data change in the data provider"
          );
          return;
        }
  ...
  #traverseGroupTreeAndUpdateLevelPositionsForTheGroup(groupNode, currentOffset, parentGroupIsVisible) {
    ...
    if (groupNode.index >= groups.length) {
      console.warn(
        "The data from the group tree is outdated. Please make sure the flamechart is reset after data change in the data provider"
      );
      return currentOffset;
    }
  ...
  isGroupCollapsible(index) {
    ...
    if (style.collapsible === 1 /* NEVER */) {
      return false;
    }
    if (!style.shareHeaderLine) {
      return style.collapsible === 0 /* ALWAYS */;
    }
    const isLastGroup = index + 1 >= groups.length;
    if (style.collapsible === 2 /* IF_MULTI_ROW */) {
  ...
  setSelectedEntry(entryIndex) {
    if (this.isMouseOverRevealChildrenArrow(this.lastMouseOffsetX, entryIndex)) {
      this.modifyTree("RESET_CHILDREN" /* RESET_CHILDREN */, entryIndex);
    }
...
export var FlameChartDecorationType = /* @__PURE__ */ ((FlameChartDecorationType2) => {
  ...
  return FlameChartDecorationType2;
})(FlameChartDecorationType || {});
...
export class FlameChartTimelineData {
  ...
  static create(data) {
    return new FlameChartTimelineData(
      data.entryLevels,
      data.entryTotalTimes,
      data.entryStartTimes,
      data.groups,
      data.entryDecorations || [],
      data.initiatorsData || []
    );
  }
  // TODO(crbug.com/1501055) Thinking about refactor this class, so we can avoid create a new object when modifying the
...
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["CANVAS_FOCUSED"] = "CanvasFocused";
  Events2["ENTRY_INVOKED"] = "EntryInvoked";
  Events2["ENTRY_LABEL_ANNOTATION_ADDED"] = "EntryLabelAnnotationAdded";
  Events2["ENTRIES_LINK_ANNOTATION_CREATED"] = "EntriesLinkAnnotationCreated";
  Events2["TRACKS_REORDER_STATE_CHANGED"] = "TracksReorderStateChange";
  Events2["ENTRY_SELECTED"] = "EntrySelected";
  Events2["ENTRY_HOVERED"] = "EntryHovered";
  Events2["LATEST_DRAW_DIMENSIONS"] = "LatestDrawDimensions";
  Events2["MOUSE_MOVE"] = "MouseMove";
  return Events2;
})(Events || {});
