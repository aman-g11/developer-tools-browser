...
export class ContrastOverlay {
  ...
  constructor(contrastInfo, colorElement) {
    ...
    if (Root.Runtime.experiments.isEnabled(Root.ExperimentNames.ExperimentName.APCA)) {
      this.contrastRatioLines.set(
        "APCA",
        UI.UIUtils.createSVGChild(this.contrastRatioSVG, "path", "spectrum-contrast-line")
      );
    } else {
      this.contrastRatioLines.set(
        "aa",
  ...
  update() {
    ...
    if (Root.Runtime.experiments.isEnabled(Root.ExperimentNames.ExperimentName.APCA) && this.contrastInfo.contrastRatioAPCA() === null) {
      return;
    }
...
export class ContrastRatioLineBuilder {
  ...
  drawContrastRatioLine(width, height, level) {
    ...
    const desiredLuminance = isAPCA ? Common.ColorUtils.desiredLuminanceAPCA(bgLuminance, requiredContrast, fgIsLighter) : Common.Color.desiredLuminance(bgLuminance, requiredContrast, fgIsLighter);
    if (isAPCA && Math.abs(Math.round(Common.ColorUtils.contrastRatioByLuminanceAPCA(desiredLuminance, bgLuminance))) < requiredContrast) {
      return null;
    }
    let lastV = fgHSVA[V];
    let currentSlope = 0;
    ...
    let candidateLuminance = (candidateHSVA2) => {
      return Common.ColorUtils.luminance(
        Common.ColorUtils.blendColors(Common.Color.Legacy.fromHSVA(candidateHSVA2).rgba(), bgRGBA)
      );
    };
    if (Root.Runtime.experiments.isEnabled(Root.ExperimentNames.ExperimentName.APCA)) {
