...
const objectPropertiesSectionMap = /* @__PURE__ */ new WeakMap();
export class ObjectTreeNodeBase extends Common.ObjectWrapper.ObjectWrapper {
  constructor(parent, propertiesMode = 1 /* OWN_AND_INTERNAL_AND_INHERITED */) {
    super();
    this.parent = parent;
  ...
  *#walk(maxDepth = -1) {
  ...
  async populateChildrenIfNeededImpl() {
    ...
    if (this.arrayLength > ARRAY_LOAD_THRESHOLD) {
      ...
      const { properties: objectProperties2, internalProperties: objectInternalProperties2 } = await SDK.RemoteObject.RemoteObject.loadFromObjectPerProto(
        this.object,
        true,
        true
    ...
    switch (this.propertiesMode) {
      ...
      case 1 /* OWN_AND_INTERNAL_AND_INHERITED */:
        ({ properties: objectProperties, internalProperties: objectInternalProperties } = await SDK.RemoteObject.RemoteObject.loadFromObjectPerProto(
          object,
          true
          /* generatePreview */
        ));
...
export class ObjectTree extends ObjectTreeNodeBase {
  #object;
  constructor(object, propertiesMode = 1 /* OWN_AND_INTERNAL_AND_INHERITED */) {
...
class ArrayGroupTreeNode extends ObjectTreeNodeBase {
  #object;
  #range;
  constructor(object, range, parent, propertiesMode = 1 /* OWN_AND_INTERNAL_AND_INHERITED */) {
...
export class ObjectTreeNode extends ObjectTreeNodeBase {
  constructor(property, propertiesMode = 1 /* OWN_AND_INTERNAL_AND_INHERITED */, parent, nonSyntheticParent) {
    super(parent, propertiesMode);
  ...
  async invokeGetter(getter) {
    ...
    const result = await this.parent?.object?.callFunction(invokeGetter, [SDK.RemoteObject.RemoteObject.toCallArgument(getter)]);
    if (!result?.object) {
      return;
    }
...
export class ObjectPropertiesSection extends UI.TreeOutline.TreeOutlineInShadow {
  ...
  static defaultObjectPropertiesSection(object, linkifier, skipProto, readOnly) {
    ...
    const propertyValue = ObjectPropertiesSection.createPropertyValue(
      object,
      /* wasThrown */
      false,
      /* showPreview */
    ...
    const objectPropertiesSection = new ObjectPropertiesSection(object, titleElement, linkifier, void 0, !readOnly);
    if (skipProto) {
  ...
  static valueElementForFunctionDescription(description, includePreview, defaultName, className) {
    ...
    return html`<span
      ...
      title=${Platform.StringUtilities.trimEndWithMaxLength(description ?? "", 500)}>${prefix && html`<span class=object-value-function-prefix>${prefix} </span>`}${includePreview ? Platform.StringUtilities.trimEndWithMaxLength(body.trim(), maxFunctionBodyLength) : abbreviation.replace(/\n/g, " ")}</span>`;
  ...
  static createPropertyValue(value, wasThrown, showPreview, linkifier, isSyntheticProperty = false, variableName, includeNullOrUndefined) {
    ...
    const contents = () => {
      ...
      if (type === "string" && typeof description === "string") {
        ...
        return html`<span class="value object-value-string" title=${ifDefined(tooLong ? void 0 : description)}>${tooLong ? widget(ExpandableTextPropertyValue, { text }) : text}</span>`;
      }
      if (type === "object" && subtype === "trustedtype") {
        const text = `${className} '${description}'`;
        const tooLong = text.length > maxRenderableStringLength;
        return html`<span class="value object-value-trustedtype" title=${ifDefined(tooLong ? void 0 : text)}>${tooLong ? widget(ExpandableTextPropertyValue, { text }) : html`${className} <span class=object-value-string title=${description}>${JSON.stringify(description)}</span>`}</span>`;
      }
      if (type === "function") {
        return ObjectPropertiesSection.valueElementForFunctionDescription(description, void 0, void 0, "value");
      }
      ...
      return html`<span class="value object-value-${subtype || type}" title=${description}>${hasPreview ? new RemoteObjectPreviewFormatter().renderObjectPreview(value.preview, includeNullOrUndefined) : description}${isSyntheticProperty ? nothing : this.getMemoryIcon(value, variableName)}</span>`;
    };
    if (wasThrown) {
      render(
        html`<span class="error value">${uiI18n.getFormatLocalizedStringTemplate(str_, UIStrings.exceptionS, { PH1: contents() })}</span>`,
        propertyValue
      );
    } else {
...
const maxRenderableStringLength = 1e4;
...
export var ObjectPropertiesMode = /* @__PURE__ */ ((ObjectPropertiesMode2) => {
...
export class RootElement extends UI.TreeOutline.TreeElement {
  ...
  constructor(object, linkifier, emptyPlaceholder) {
    ...
    this.object = object;
    this.object.addEventListener("children-changed" /* CHILDREN_CHANGED */, this.onpopulate, this);
  ...
  render(
    html`<span class=name-and-value><span
      ...
      output.nameElement = e;
          ...
          class=${nameClasses}
          title=${input.node.path}>${property.private ? html`<span class="private-property-hash">${property.name[0]}</span>${property.name.substring(1)}</span>` : quotedName}</span>${isInternalEntries ? nothing : html`<span class='separator'>: </span><devtools-prompt
                ...
                ?editing=${input.editing}>
                  ${input.expanded && isExpandable && property.value ? html`<span
                      class="value object-value-${property.value.subtype || property.value.type}"
                      title=${ifDefined(property.value.description)}>${property.value.description === "Object" ? "" : Platform.StringUtilities.trimMiddle(
      property.value.description ?? "",
      maxRenderableStringLength
    )}${property.synthetic ? nothing : ObjectPropertiesSection.getMemoryIcon(property.value)}</span>` : value()}
...
export class ObjectPropertyTreeElement extends UI.TreeOutline.TreeElement {
  ...
  static populateWithProperties(treeNode, { properties, internalProperties, accessors }, skipProto, skipGettersAndSetters, linkifier, emptyPlaceholder) {
    ...
    for (const property of properties ?? []) {
      if (treeNode instanceof ObjectPropertyTreeElement && !ObjectPropertiesSection.isDisplayableProperty(property.property, treeNode.property?.property)) {
        continue;
      }
  ...
  getContextMenu(event) {
    ...
    if (!this.property.property.synthetic && this.property.path) {
      ...
      contextMenu.clipboardSection().appendItem(
        i18nString(UIStrings.copyPropertyPath),
        copyPathHandler,
...
export const EXPANDABLE_TEXT_DEFAULT_VIEW = (input, output, target) => {
  ...
  const onContextMenu = (e) => {
    ...
    if (canExpand && !input.expanded) {
      contextMenu.clipboardSection().appendItem(
        i18nString(UIStrings.showMoreS, { PH1: totalBytesText }),
        input.expandText,
  ...
  render(
    ...
    html`<span title=${croppedText + "\u2026"} @contextmenu=${onContextMenu}>
               ...
               <button
                 ...
                 jslog=${ifDefined(canExpand ? VisualLogging.action("expand").track({ click: true }) : void 0)}
                 class=${canExpand ? "expandable-inline-button" : "undisplayable-text"}
                 data-text=${canExpand ? i18nString(UIStrings.showMoreS, { PH1: totalBytesText }) : i18nString(UIStrings.longTextWasTruncatedS, { PH1: totalBytesText })}
                 ></button>
               <button
