...
export class FilteredListWidget extends Common.ObjectWrapper.eventMixin(
  UI.Widget.VBox
) {
  promptHistory;
  scoringTimer;
  ...
  createElementForItem(item) {
    ...
    if (this.provider) {
      ...
      wrapperElement.setAttribute(
        "jslog",
        `${VisualLogging.item(this.provider.jslogContextAt(item)).track({ click: true, resize: true })}`
      );
    }
