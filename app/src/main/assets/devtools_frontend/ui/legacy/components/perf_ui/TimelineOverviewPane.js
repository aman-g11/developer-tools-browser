...
export class TimelineOverviewPane extends Common.ObjectWrapper.eventMixin(
  UI.Widget.VBox
) {
  overviewCalculator;
  overviewGrid;
  ...
  constructor(prefix) {
    ...
    this.overviewGrid.element.setAttribute(
      "jslog",
      `${VisualLoggging.timeline(`${prefix}-overview`).track({ click: true, drag: true, hover: true })}`
    );
  ...
  onMouseMove(event) {
    ...
    if (Trace.Types.Timing.Milli(timeWindow.rawStartValue) <= timeInMilliSeconds && timeInMilliSeconds <= Trace.Types.Timing.Milli(timeWindow.rawEndValue)) {
      const timeInMicroSeconds = Trace.Helpers.Timing.milliToMicro(timeInMilliSeconds);
      this.dispatchEventToListeners("OverviewPaneMouseMove" /* OVERVIEW_PANE_MOUSE_MOVE */, { timeInMicroSeconds });
    } else {
      this.dispatchEventToListeners("OverviewPaneMouseLeave" /* OVERVIEW_PANE_MOUSE_LEAVE */);
    }
    if (!this.hasPointer) {
  ...
  onBreadcrumbAdded() {
    this.dispatchEventToListeners("OverviewPaneBreadcrumbAdded" /* OVERVIEW_PANE_BREADCRUMB_ADDED */, {
  ...
  onWindowChanged(event) {
    ...
    this.dispatchEventToListeners("OverviewPaneWindowChanged" /* OVERVIEW_PANE_WINDOW_CHANGED */, windowTimes);
  }
  setWindowTimes(startTime, endTime) {
    ...
    this.updateWindow();
    this.dispatchEventToListeners("OverviewPaneWindowChanged" /* OVERVIEW_PANE_WINDOW_CHANGED */, {
  ...
  #initializeDimHighlightSVG() {
    ...
    ThemeSupport.ThemeSupport.instance().addEventListener(ThemeSupport.ThemeChangeEvent.eventName, () => {
      ...
      desaturateRect2?.setAttribute("fill", ThemeSupport.ThemeSupport.instance().getComputedValue("--color-background"));
      const bracket2 = this.#dimHighlightSVG.querySelector("polygon");
      bracket2?.setAttribute(
        "fill",
        ThemeSupport.ThemeSupport.instance().getComputedValue("--sys-color-state-on-header-hover")
      );
    });
  }
  #addBracket(left, right) {
    ...
    bracket?.setAttribute(
      "points",
      `${left},0 ${left},${TRIANGLE_SIZE} ${left + TRIANGLE_SIZE - 1},1 ${right - TRIANGLE_SIZE - 1},1 ${right},${TRIANGLE_SIZE} ${right},0`
    );
...
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["OVERVIEW_PANE_WINDOW_CHANGED"] = "OverviewPaneWindowChanged";
  Events2["OVERVIEW_PANE_BREADCRUMB_ADDED"] = "OverviewPaneBreadcrumbAdded";
  Events2["OVERVIEW_PANE_MOUSE_MOVE"] = "OverviewPaneMouseMove";
  Events2["OVERVIEW_PANE_MOUSE_LEAVE"] = "OverviewPaneMouseLeave";
  return Events2;
})(Events || {});
...
export class OverviewInfo {
  ...
  constructor(anchor) {
    ...
    this.visible = false;
    this.element = UI.UIUtils.createShadowRootWithCoreStyles(this.glassPane.contentElement, { cssFile: timelineOverviewInfoStyles }).createChild("div", "overview-info");
  }
