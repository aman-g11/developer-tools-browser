...
export function processEventForIntuitiveDebugging(event, state, extraInfo) {
  const entry = {
    event,
    ve: state ? VisualElements[state?.config.ve] : void 0,
    veid: state?.veid,
...
const veTestKeys = /* @__PURE__ */ new Map();
...
const adHocAnalysisEntries = /* @__PURE__ */ new Map();
function processImpressionsForAdHocAnalysisDebugLog(states) {
  for (const state of states) {
    const buildVe = (state2) => {
      const ve = {
      ...
      if (state2.parent) {
        ve.parent = buildVe(state2.parent);
      }
      return ve;
    };
...
export var DebugLoggingFormat = /* @__PURE__ */ ((DebugLoggingFormat2) => {
...
export function setVeDebugLoggingEnabled(enabled, format = "Intuitive" /* INTUITIVE */) {
  if (enabled) {
...
const EVENT_EXPECTATION_TIMEOUT = 5e3;
