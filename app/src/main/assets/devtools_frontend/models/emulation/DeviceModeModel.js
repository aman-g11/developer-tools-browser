...
export class DeviceModeModel extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    ...
    this.#uaSetting = Common.Settings.Settings.instance().createSetting("emulation.device-ua", "Mobile" /* MOBILE */);
    this.#uaSetting.addChangeListener(this.uaSettingChanged, this);
    this.#deviceScaleFactorSetting = Common.Settings.Settings.instance().createSetting("emulation.device-scale-factor", 0);
    ...
    if (type === "Device" /* Device */ && device && mode) {
      ...
      if (this.#initialized) {
        ...
        this.#scaleSetting.set(
          scale || this.calculateFitScale(orientation.width, orientation.height, this.currentOutline(), this.currentInsets())
        );
      }
    } else {
      this.#device = null;
  ...
  modelAdded(emulationModel) {
    if (emulationModel.target() === SDK.TargetManager.TargetManager.instance().primaryPageTarget() && emulationModel.supportsDeviceEmulation()) {
      ...
      emulationModel.addEventListener(
        SDK.EmulationModel.EmulationModelEvents.SCREEN_ORIENTATION_LOCK_CHANGED,
  ...
  applyOrientationLock(orientation) {
    const wantsLandscape = orientation.type === Protocol.Emulation.ScreenOrientationType.LandscapePrimary || orientation.type === Protocol.Emulation.ScreenOrientationType.LandscapeSecondary;
  ...
  calculateAndEmulate(resetPageScaleFactor) {
    ...
    if (this.#type === "Device" /* Device */ && this.#device && this.#mode) {
      ...
      this.applyDeviceMetrics(
        ...
        mobile,
        screenHeight >= screenWidth ? Protocol.Emulation.ScreenOrientationType.PortraitPrimary : Protocol.Emulation.ScreenOrientationType.LandscapePrimary,
        resetPageScaleFactor
      );
  ...
  applyUserAgent(userAgent, userAgentMetadata) {
    SDK.NetworkManager.MultitargetNetworkManager.instance().setUserAgentOverride(
      userAgent,
      userAgent ? userAgentMetadata : null
    );
  }
  applyDeviceMetrics(screenSize, insets, outline, scale, deviceScaleFactor, mobile, screenOrientation, resetPageScaleFactor) {
    ...
    this.#outlineRect = new Rect(
      this.#screenRect.left - outline.left * scale,
      0,
  ...
  async captureScreenshot(fullSize, clip) {
    ...
    const screenshot = await screenCaptureModel.captureScreenshot(
      Protocol.Page.CaptureScreenshotRequestFormat.Png,
      100,
      screenshotMode,
      clip
    );
...
export var Type = /* @__PURE__ */ ((Type2) => {
  Type2["None"] = "None";
  Type2["Responsive"] = "Responsive";
  Type2["Device"] = "Device";
  return Type2;
})(Type || {});
export var UA = /* @__PURE__ */ ((UA2) => {
  UA2["MOBILE"] = "Mobile";
