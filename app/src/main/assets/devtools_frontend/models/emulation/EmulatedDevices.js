...
export class EmulatedDevice {
  ...
  constructor() {
    ...
    this.capabilities = ["touch" /* TOUCH */, "mobile" /* MOBILE */];
    this.userAgent = "";
    ...
    this.#show = "Default" /* Default */;
    this.#showByDefault = true;
  }
  static fromJSONV1(json) {
    try {
      let parseValue2 = function(object, key, type2, defaultValue) {
        ...
        const outlineInsets = parseValue2(json2["outline"], "insets", "object", null);
        if (outlineInsets) {
          result2.outlineInsets = parseInsets2(outlineInsets);
          if (result2.outlineInsets.left < 0 || result2.outlineInsets.top < 0) {
            throw new Error("Emulated device has wrong outline insets");
          }
          result2.outlineImage = parseValue2(json2["outline"], "image", "string");
        }
        if (json2["hinge"]) {
          result2.hinge = parseHinge2(parseValue2(json2, "hinge", "object", void 0));
        }
        return result2;
      };
      var parseValue = parseValue2, parseIntValue = parseIntValue2, parseInsets = parseInsets2, parseRGBA = parseRGBA2, parseHinge = parseHinge2, parseOrientation = parseOrientation2;
      const result = new EmulatedDevice();
      result.title = parseValue2(json, "title", "string");
      const type = parseValue2(json, "type", "string");
      if (!Object.values(Type).includes(type)) {
        throw new Error("Emulated device has wrong type: " + type);
      }
      result.type = type;
      result.order = parseValue2(json, "order", "number", 0);
      const rawUserAgent = parseValue2(json, "user-agent", "string");
      result.userAgent = SDK.NetworkManager.MultitargetNetworkManager.patchUserAgentWithChromeVersion(rawUserAgent);
      const userAgentMetadata = parseValue2(json, "user-agent-metadata", "object", null);
      result.userAgentMetadata = result.userAgent ? userAgentMetadata : null;
      const capabilities = parseValue2(json, "capabilities", "object", []);
      ...
      result.deviceScaleFactor = parseValue2(json["screen"], "device-pixel-ratio", "number");
      ...
      result.vertical = parseOrientation2(parseValue2(json["screen"], "vertical", "object"));
      result.horizontal = parseOrientation2(parseValue2(json["screen"], "horizontal", "object"));
      result.isDualScreen = parseValue2(json, "dual-screen", "boolean", false);
      result.isFoldableScreen = parseValue2(json, "foldable-screen", "boolean", false);
      if (result.isDualScreen || result.isFoldableScreen) {
        result.verticalSpanned = parseOrientation2(parseValue2(json["screen"], "vertical-spanned", "object", null));
        result.horizontalSpanned = parseOrientation2(parseValue2(json["screen"], "horizontal-spanned", "object", null));
      }
      ...
      const modes = parseValue2(json, "modes", "object", [
      ...
      for (let i = 0; i < modes.length; ++i) {
        const mode = {};
        mode.title = parseValue2(modes[i], "title", "string");
        mode.orientation = parseValue2(modes[i], "orientation", "string");
        ...
        mode.insets = parseInsets2(parseValue2(modes[i], "insets", "object", { left: 0, top: 0, right: 0, bottom: 0 }));
        ...
        mode.image = parseValue2(modes[i], "image", "string", null);
        result.modes.push(mode);
      }
      result.#showByDefault = parseValue2(json, "show-by-default", "boolean", void 0);
      const show = parseValue2(json, "show", "string", "Default" /* Default */);
...
var Type = /* @__PURE__ */ ((Type2) => {
  Type2["Phone"] = "phone";
  Type2["Tablet"] = "tablet";
  Type2["Notebook"] = "notebook";
  Type2["Desktop"] = "desktop";
...
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["CUSTOM_DEVICES_UPDATED"] = "CustomDevicesUpdated";
  Events2["STANDARD_DEVICES_UPDATED"] = "StandardDevicesUpdated";
  return Events2;
})(Events || {});
