...
class NetworkAnalyzer {
  ...
  static estimateRTTByOrigin(records, options) {
    ...
    for (const [origin, originRequests] of groupedByOrigin.entries()) {
      let collectEstimates2 = function(estimator, multiplier = 1) {
        for (const request of originRequests) {
          const timing = request.timing;
      ...
      var collectEstimates = collectEstimates2;
      const originEstimates = [];
      if (!forceCoarseEstimates) {
        collectEstimates2(this.estimateRTTViaConnectionTiming);
      }
      if (!originEstimates.length) {
        if (useDownloadEstimates) {
          collectEstimates2(this.estimateRTTViaDownloadTiming, coarseEstimateMultiplier);
        }
        if (useSendStartEstimates) {
          collectEstimates2(this.estimateRTTViaSendStartTiming, coarseEstimateMultiplier);
        }
        if (useHeadersEndEstimates) {
          collectEstimates2(this.estimateRTTViaHeadersEndTiming, coarseEstimateMultiplier);
        }
      }
      if (originEstimates.length) {
