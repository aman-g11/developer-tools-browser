...
export class DebuggerWorkspaceBinding {
  ...
  constructor(resourceMapping, targetManager, ignoreListManager, workspace) {
    ...
    this.ignoreListManager.addEventListener(
      Workspace.IgnoreListManager.Events.IGNORED_SCRIPT_RANGES_UPDATED,
  ...
  async updateLocations(script) {
    const stackTraceUpdatePromise = script.target().model(StackTraceImpl.StackTraceModel.StackTraceModel)?.scriptInfoChanged(script, this.#translateRawFrames.bind(this));
  ...
  async shouldPause(debuggerPausedDetails, autoSteppingContext) {
    ...
    if (!autoSteppingContext || debuggerPausedDetails.reason !== Protocol.Debugger.PausedEventReason.Step || !functionLocation || !frame.script.isWasm() || !Common.Settings.moduleSetting("wasm-auto-stepping").get() || !this.pluginManager.hasPluginForScript(frame.script)) {
      return true;
    }
