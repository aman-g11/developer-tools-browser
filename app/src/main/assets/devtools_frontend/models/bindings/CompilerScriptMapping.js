...
export class CompilerScriptMapping {
  ...
  sourceMapAttached(event) {
    ...
    for (const url of sourceMap.sourceURLs()) {
      ...
      const contentProvider = content !== null ? TextUtils.StaticContentProvider.StaticContentProvider.fromString(url, contentType, content) : new SDK.CompilerSourceMappingContentProvider.CompilerSourceMappingContentProvider(
        url,
        contentType,
