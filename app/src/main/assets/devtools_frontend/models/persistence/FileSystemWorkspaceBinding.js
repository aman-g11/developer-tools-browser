...
export class FileSystem extends Workspace.Workspace.ProjectStore {
  ...
  constructor(fileSystemWorkspaceBinding, isolatedFileSystem, workspace) {
    ...
    this.#fileSystemParentURL = Common.ParsedURL.ParsedURL.substr(this.fileSystemBaseURL, 0, fileSystemPath.lastIndexOf("/") + 1);
  ...
  rename(uiSourceCode, newName, callback) {
    ...
    function innerCallback(success, newName2) {
      ...
      filePath = Common.ParsedURL.ParsedURL.encodedFromParentPathAndName(parentPath, newName2);
  ...
  async findFilesMatchingSearchRequest(searchConfig, filesMatchingFileQuery, progress) {
    ...
    for (const query of queriesToRun) {
      ...
      workingFileSet = Platform.ArrayUtilities.intersectOrdered(
        workingFileSet,
        files,
        Platform.StringUtilities.naturalOrderComparator
      );
      ++progress.worked;
    }
...
const sourceCodeToMetadataMap = /* @__PURE__ */ new WeakMap();
