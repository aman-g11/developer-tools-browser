...
export class NetworkPersistenceManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  async updateActiveProject() {
    ...
    this.#active = Boolean(this.#enabledSetting.get() && SDK.TargetManager.TargetManager.instance().rootTarget() && this.#project);
  ...
  fileUrlFromNetworkUrl(url, ignoreInactive) {
    ...
    return Common.ParsedURL.ParsedURL.concatenate(
      this.#project.fileSystemPath(),
      "/",
      this.encodedPathFromUrl(url, ignoreInactive)
    );
  }
  ...
  isUISourceCodeOverridable(uiSourceCode) {
    return uiSourceCode.project().type() === Workspace.Workspace.projectTypes.Network && !NetworkPersistenceManager.isForbiddenNetworkUrl(uiSourceCode.url());
  }
  ...
  async networkUISourceCodeAdded(uiSourceCode) {
    if (uiSourceCode.project().type() !== Workspace.Workspace.projectTypes.Network || !this.canHandleNetworkUISourceCode(uiSourceCode)) {
      return;
    }
  ...
  async generateHeaderPatterns(uiSourceCode) {
    ...
    const relativePath = Common.ParsedURL.ParsedURL.slice(
      Common.ParsedURL.ParsedURL.join(relativePathParts, "/"),
      0,
    ...
    if (relativePathParts.length > 2 && relativePathParts[1] === "longurls" && headerOverrides.length) {
      ...
      patterns = this.#generateHeaderPatternsForFileUrl(
        Common.ParsedURL.ParsedURL.substring(decodedPath, "file:/".length),
        headerOverrides
      );
    } else {
  ...
  async onProjectAdded(project) {
    if (project.type() !== Workspace.Workspace.projectTypes.FileSystem || FileSystemWorkspaceBinding.fileSystemType(project) !== "overrides") {
      return;
    }
...
const RESERVED_FILENAMES = /* @__PURE__ */ new Set([
  "con",
  "prn",
  "aux",
  "nul",
...
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["PROJECT_CHANGED"] = "ProjectChanged";
  Events2["REQUEST_FOR_HEADER_OVERRIDES_FILE_CHANGED"] = "RequestsForHeaderOverridesFileChanged";
  Events2["LOCAL_OVERRIDES_PROJECT_UPDATED"] = "LocalOverridesProjectUpdated";
  Events2["LOCAL_OVERRIDES_REQUESTED"] = "LocalOverridesRequested";
  return Events2;
})(Events || {});
