...
export class IsolatedFileSystemManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  requestFileSystems() {
    ...
    Host.InspectorFrontendHost.InspectorFrontendHostInstance.events.addEventListener(
      Host.InspectorFrontendHostAPI.Events.FileSystemsLoaded,
      onFileSystemsLoaded,
      this
    );
  ...
  addFileSystem(type) {
    Host.userMetrics.actionTaken(
      type === "overrides" ? Host.UserMetrics.Action.OverrideTabAddFolder : Host.UserMetrics.Action.WorkspaceTabAddFolder
    );
  ...
  removeFileSystem(fileSystem) {
    Host.userMetrics.actionTaken(
      fileSystem.type() === PlatformFileSystemType.OVERRIDES ? Host.UserMetrics.Action.OverrideTabRemoveFolder : Host.UserMetrics.Action.WorkspaceTabRemoveFolder
    );
  ...
  #addFileSystem(fileSystem, dispatchEvent) {
    ...
    function storeFileSystem(fileSystem2) {
      ...
      fileSystem2.addEventListener(PlatformFileSystemEvents.FILE_SYSTEM_ERROR, this.#onFileSystemError, this);
      if (dispatchEvent) {
...
export var Events = /* @__PURE__ */ ((Events2) => {
  ...
  Events2["FileSystemError"] = "FileSystemError";
  return Events2;
})(Events || {});
let lastRequestId = 0;
