          ...
          if (Annotations.AnnotationRepository.annotationsEnabled()) {
            ...
            if (layoutShiftData?.impacted_nodes && layoutShiftData.impacted_nodes?.length > 0) {
              Annotations.AnnotationRepository.instance().addElementsAnnotation(
                "This element is impacted by a layout shift",
                layoutShiftData.impacted_nodes[0].node_id.toString()
              );
            }
          }
        }
...
`);
  ...
  #serializeRelatedInsightsForEvents(events) {
    ...
    for (const [insightKey, events2] of insightNameToRelatedEvents) {
      const eventsString = events2.slice(0, 5).map((e) => Trace.Name.forEntry(e) + " " + this.serializeEvent(e)).join(", ");
  ...
  async formatMainThreadTrackSummary(bounds) {
    ...
    const insightSet = this.#parsedTrace.insights?.values().find(
      (insightSet2) => Trace.Helpers.Timing.boundsIncludeTimeRange({ bounds, timeRange: insightSet2.bounds })
    );
