...
var ScorePriority = /* @__PURE__ */ ((ScorePriority2) => {
...
export class PerformanceAgent extends AiAgent {
  ...
  get userTier() {
    return Boolean(Root.Runtime.hostConfig.devToolsGreenDevUi?.enabled) ? "TESTERS" : Root.Runtime.hostConfig.devToolsAiAssistancePerformanceAgent?.userTier;
  }
  get options() {
  ...
  #declareFunctions(context) {
    ...
    this.declareFunction("getInsightDetails", {
      ...
      handler: async (params) => {
        ...
        if (!insightSet) {
          const valid = [...parsedTrace.insights?.values() ?? []].map(
            (insightSet2) => `id: ${insightSet2.id}, url: ${insightSet2.url}, bounds: ${this.#formatter?.serializeBounds(insightSet2.bounds)}`
          ).join("; ");
        ...
        const widgets = [];
        if (Trace.Insights.Models.LCPDiscovery.isLCPDiscoveryInsight(insight) || Trace.Insights.Models.LCPBreakdown.isLCPBreakdownInsight(insight)) {
