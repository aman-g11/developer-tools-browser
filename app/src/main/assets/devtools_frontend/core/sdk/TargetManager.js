...
export class TargetManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  static instance({ forceNew } = { forceNew: false }) {
    if (!Root.DevToolsContext.globalInstance().has(TargetManager) || forceNew) {
      Root.DevToolsContext.globalInstance().set(
        TargetManager,
        new TargetManager(Root.DevToolsContext.globalInstance())
      );
    }
  ...
  createTarget(id, name, type, parentTarget, sessionId, waitForDebuggerInPage, connection, targetInfo) {
    ...
    if (target === target.outermostTarget() && (target.type() !== TargetType.FRAME || target === this.primaryPageTarget()) && !this.#defaultScopeSet) {
...
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["AVAILABLE_TARGETS_CHANGED"] = "AvailableTargetsChanged";
  Events2["INSPECTED_URL_CHANGED"] = "InspectedURLChanged";
  Events2["NAME_CHANGED"] = "NameChanged";
  Events2["SUSPEND_STATE_CHANGED"] = "SuspendStateChanged";
  return Events2;
})(Events || {});
