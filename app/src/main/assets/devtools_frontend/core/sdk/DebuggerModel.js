...
export class DebuggerModel extends SDKModel {
  ...
  constructor(target) {
    ...
    this.#sourceMapManager = new SourceMapManager(
      target,
      (compiledURL, sourceMappingURL, payload, script) => new SourceMap(compiledURL, sourceMappingURL, payload, script)
    );
    Common.Settings.Settings.instance().moduleSetting("pause-on-exception-enabled").addChangeListener(this.pauseOnExceptionStateChanged, this);
    Common.Settings.Settings.instance().moduleSetting("pause-on-caught-exception").addChangeListener(this.pauseOnExceptionStateChanged, this);
    Common.Settings.Settings.instance().moduleSetting("pause-on-uncaught-exception").addChangeListener(this.pauseOnExceptionStateChanged, this);
    Common.Settings.Settings.instance().moduleSetting("disable-async-stack-traces").addChangeListener(this.asyncStackTracesStateChanged, this);
    Common.Settings.Settings.instance().moduleSetting("breakpoints-active").addChangeListener(this.breakpointsActiveChanged, this);
    if (!target.suspended()) {
    ...
    Common.Settings.Settings.instance().moduleSetting("js-source-maps-enabled").addChangeListener((event) => this.#sourceMapManager.setEnabled(event.data));
  ...
  parsedScriptSource(scriptId, sourceURL, startLine, startColumn, endLine, endColumn, executionContextId, hash, executionContextAuxData, isLiveEdit, sourceMapURL, hasSourceURLComment, hasSyntaxError, length, isModule, originStackTrace, codeOffset, scriptLanguage, debugSymbols, embedderName, buildId) {
    ...
    if ((!selectedDebugSymbol || selectedDebugSymbol.type === Protocol.Debugger.DebugSymbolsType.SourceMap) && script.sourceMapURL && !hasSyntaxError) {
  ...
  dispose() {
    ...
    Common.Settings.Settings.instance().moduleSetting("pause-on-exception-enabled").removeChangeListener(this.pauseOnExceptionStateChanged, this);
    Common.Settings.Settings.instance().moduleSetting("pause-on-caught-exception").removeChangeListener(this.pauseOnExceptionStateChanged, this);
    Common.Settings.Settings.instance().moduleSetting("disable-async-stack-traces").removeChangeListener(this.asyncStackTracesStateChanged, this);
  }
  async suspendModel() {
  ...
  async *iterateAsyncParents(stackTraceOrPausedDetails) {
    ...
    while (true) {
      if (stackTrace.parent) {
        ...
        const model = stackTrace.parentId.debuggerId ? await DebuggerModel.modelForDebuggerId(stackTrace.parentId.debuggerId) : this;
        if (!model) {
          return;
        }
...
const debuggerIdToModel = /* @__PURE__ */ new Map();
...
export var Events = /* @__PURE__ */ ((Events2) => {
  ...
  Events2["DebugInfoAttached"] = "DebugInfoAttached";
  ...
  Events2["ScriptSourceWasEdited"] = "ScriptSourceWasEdited";
  return Events2;
})(Events || {});
  ...
  static fromPayloadArray(debuggerModel, callFrames, exception) {
    ...
    for (let i = 0; i < callFrames.length; ++i) {
      ...
      if (script) {
        ...
        result.push(new CallFrame(debuggerModel, script, callFrame, void 0, void 0, ex));
      }
    }
    return result;
  }
...
export class Scope {
  ...
  object() {
    ...
    const declarativeScope = this.#type !== Protocol.Debugger.ScopeType.With && this.#type !== Protocol.Debugger.ScopeType.Global;
    if (declarativeScope) {
  ...
  extraProperties() {
    ...
    if (exception) {
      extraProperties.push(new RemoteObjectProperty(
        i18nString(UIStrings.exception),
        exception,
        void 0,
        void 0,
        void 0,
